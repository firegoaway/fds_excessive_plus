...
NAMELIST /TIME/ DT,DT_END_FILL,DT_END_MINIMUM,DT_EXTERNAL,DT_EXTERNAL_HEARTBEAT,EXTERNAL_HEARTBEAT_FILENAME,HEARTBEAT_FAIL,&
                ...
                CFL_RELAXED,CFL_RELAXED_MAX,VN_RELAXED_MAX
...
! CFL_RELAXED mode initialization - simple and robust
IF (CFL_RELAXED) THEN
   CFL_MAX = CFL_RELAXED_MAX
   VN_MAX = VN_RELAXED_MAX
   WRITE(LU_ERR,'(A)') 'INFO: CFL_RELAXED mode enabled'
   WRITE(LU_ERR,'(A,F8.1,A)') '  CFL_MAX: ', CFL_RELAXED_MAX, ' (relaxed from 1.1)'
   WRITE(LU_ERR,'(A,F8.1,A)') '  VN_MAX: ', VN_RELAXED_MAX, ' (relaxed from 0.8)'
   WRITE(LU_ERR,'(A)') '  WARNING: Results less accurate - for rapid assessments only'
   WRITE(LU_ERR,'(A)') '  EXPECTED SPEEDUP: 5-10x for typical scenarios'
ENDIF

! LAZY-SOAP solver initialization
IF (PRES_FLAG == LAZY_SOAP_FLAG) THEN
   WRITE(LU_ERR,'(/A)') 'INFO: LAZY-SOAP pressure solver enabled'
   WRITE(LU_ERR,'(A,F6.2)') '  LAZY_SOAP_THRESHOLD: ', LAZY_SOAP_THRESHOLD, ' (aggressive skip threshold)'
   WRITE(LU_ERR,'(A,F6.2)') '  LAZY_SOAP_EXTRAP_ALPHA: ', LAZY_SOAP_EXTRAP_ALPHA, ' (damping factor)'
   WRITE(LU_ERR,'(A,F6.2)') '  LAZY_SOAP_EXTRAP_BETA: ', LAZY_SOAP_EXTRAP_BETA, ' (2nd order coefficient)'
   WRITE(LU_ERR,'(A,I0)') '  LAZY_MAX_SKIP_CONSECUTIVE: ', LAZY_MAX_SKIP_CONSECUTIVE, ' (max consecutive skips)'
   WRITE(LU_ERR,'(A,F6.3)') '  LAZY_DIVERGENCE_TOLERANCE: ', LAZY_DIVERGENCE_TOLERANCE, ' (divergence fallback)'
   WRITE(LU_ERR,'(A)') '  EXPECTED SPEEDUP: 3-5x with 90-95% accuracy'
ENDIF

! FIXED_DT_OVERRIDE initialization - FDS 5 legacy mode
IF (DT_OVERRIDE > 0._EB) THEN
   WRITE(LU_ERR,'(/A)') 'INFO: FIXED_DT_OVERRIDE mode enabled (FDS 5 legacy)'
   WRITE(LU_ERR,'(A,F8.5,A)') '  DT_OVERRIDE: ', DT_OVERRIDE, ' s (fixed time step)'
   IF (DISABLE_VN_CHECK) THEN
      WRITE(LU_ERR,'(A)') '  DISABLE_VN_CHECK: .TRUE. (Von Neumann check disabled)'
   END IF
   IF (DISABLE_PARTICLE_CFL) THEN
      WRITE(LU_ERR,'(A)') '  DISABLE_PARTICLE_CFL: .TRUE. (Particle CFL disabled)'
   END IF
   WRITE(LU_ERR,'(A,F6.3)') '  DIVERGENCE_TOLERANCE: ', DIVERGENCE_TOLERANCE, ' (acceptable divergence)'
   WRITE(LU_ERR,'(A)') '  WARNING: Very fast but highly inaccurate - use only for rough estimates!'
   WRITE(LU_ERR,'(A)') '  EXPECTED SPEEDUP: 8-12x with 70-85% accuracy'
ENDIF
...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE, &
                BNDF_DEFAULT,CC_IBM,CCVOL_LINK,CCC_DEFAULT,C_DEARDORFF,&
                CFL_MAX,CFL_MIN,CFL_VELOCITY_NORM,CHECK_FO,CHECK_HT,CHECK_VN, &
                CNF_CUTOFF,CONSTANT_SPECIFIC_HEAT_RATIO,FDS5_HT_CFL,FDS5_DT_RESTRICT,FDS5_IBM_CFL_CHECK,FDS5_CFL_NORM,FDS5_PRESSURE_MODE,FDS5_WALL_BC_PASS,FDS5_DIVG_REDUCE,FDS5_DIVG_ADVECTION,FDS5_CHEM_LOOP,DIVERGENCE_LIMIT,HT_CFL_LIMIT,PARTICLE_VELOCITY_LIMIT,VON_NEUMANN_LIMIT,FDS5_CCC,&
                CHECK_STABILITY_OMP_THRESHOLD,PRESSURE_RHS_OMP_THRESHOLD,CACHE_VISCOSITY_CORRECTOR,CACHE_VISCOSITY_PREDICTOR,SKIP_DEARDORFF_FILTER_CORRECTOR,&
                FDS5_FLAME_SPEED_MODEL,FDS5_COMBUSTION_EARLY_SKIP,&
                ...
                LAZY_SOAP_THRESHOLD,LAZY_SOAP_EXTRAP_ALPHA,LAZY_SOAP_EXTRAP_BETA,LAZY_MAX_SKIP_CONSECUTIVE,LAZY_DIVERGENCE_TOLERANCE,LAZY_SOAP_GLOBAL_CONTEXT,&
                ...
                MINIMUM_ZONE_VOLUME,MLR_BASED_HRR,MPI_TIMEOUT,NEAR_WALL_PARTICLE_INTERPOLATION,NEIGHBOR_SEPARATION_DISTANCE,NORTH_BEARING,&
                ...
                DT_OVERRIDE,DISABLE_VN_CHECK,DISABLE_PARTICLE_CFL,DIVERGENCE_TOLERANCE
...
TURBULENCE_MODEL     = 'EXPLICIT_EULER'
MAX_LEAK_PATHS       = 200
FLUX_LIMITER         = 'null'
...
! FDS5 CFL norm: force max-norm without divergence term (fastest mode) for all simulation modes

IF (FDS5_CFL_NORM) THEN
   CFL_VELOCITY_NORM = 3
ENDIF
...
REAL(EB) :: MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            ...
            CONVERSION,PR_GAS,CONDUCTIVITY_LIQUID,VISCOSITY_LIQUID,BETA_LIQUID,THERMOPHORETIC_DIAMETER,C,H,O,N,CL,&
...
NAMELIST /SPEC/ AEROSOL,ALT_ID,BACKGROUND,BETA_LIQUID,C,CL,COMPLEX_REFRACTIVE_INDEX,CONDUCTIVITY,CONDUCTIVITY_LIQUID,&
...
DO N1=1,N_SPEC_READ
   ...
   IF (ANY(MASS_FRACTION>0._EB) .AND. ANY(VOLUME_FRACTION>0._EB)) THEN
      ...
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (FORMULA/='null' .AND. (C >0._EB .OR. H>0._EB .OR. O>0._EB .OR. N>0._EB .OR. CL>0._EB)) THEN
      WRITE(MESSAGE,'(A,A,A)') 'ERROR(160): SPEC ' ,TRIM(ID),', cannot specify both FORMULA and C,H,O,N, or Cl.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
ENDDO
...
PREDEF_SPEC_IF: IF (SIMPLE_CHEMISTRY) THEN
   ...
   IF (.NOT. ANY(SPEC_ID_READ=='HYDROGEN CHLORIDE')) N_PREDEFINED = N_PREDEFINED + 1
   ...
   ENDIF
   IF (.NOT. ANY(SPEC_ID_READ=='HYDROGEN CHLORIDE')) THEN
      N_PREDEFINED = N_PREDEFINED + 1
      PREDEFINED_SPEC_ID(N_PREDEFINED) = 'HYDROGEN CHLORIDE'
   ENDIF
   DO NR=1,N_REACTIONS
...
PRIMITIVE_LOOP: DO N1=1,N_SPECIES - N_COPY_PRIMITIVE
   ...
   ENDIF
   IF (SS%PROP_ID == 'HCl' .OR. SS%PROP_ID == 'HYDROGEN CHLORIDE') THEN
      MW_HCL = SS%MW
   ENDIF
   ...
   IF (TRIM(SS%FORMULA)=='null') THEN
      ...
      IF (C >0._EB .OR. H>0._EB .OR. O>0._EB .OR. N>0._EB) THEN
         ...
         IF (CL > 0._EB) THEN
            SS%ATOMS(17) = CL
            SS%MW = SS%MW + CL * ELEMENT(17)%MASS
         ENDIF
      ENDIF
   ENDIF
   ...
   IF (SIMPLE_CHEMISTRY) THEN
      DO NR=1,N_REACTIONS
         IF (TRIM(ID)==TRIM(REACTION(NR)%FUEL)) THEN
            ...
            IF (TRIM(SS%FORMULA)==TRIM(FORMULA) .AND. .NOT. (C>0._EB .OR. H>0._EB .OR. N>0._EB .OR. O>0._EB .OR. CL>0._EB)) &
...
IF (SIMPLE_CHEMISTRY) THEN
   DO NR=1,N_REACTIONS
      ...
      IF (ANY(SPECIES_MIXTURE(N_PREDEFINED_SMIX+1:N_TRACKED_SPECIES)%ID==REACTION(NR)%FUEL)) THEN
         ...
         IF (ANY(SM%ATOMS(2:5)>0._EB) .OR. ANY(SM%ATOMS(9:16)>0._EB) .OR. ANY(SM%ATOMS(18:)>0._EB)) THEN
            WRITE(MESSAGE,'(A,I0,A)') 'ERROR(163): REAC ',NR,': FORMULA limited to C,H,O,N,Cl for simple chemistry.'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ELSE
...
ELSE BACKGROUND_IF ! Mixture is fuel or products
   ...
   N_SIMPLE_CHEM_RXN: SELECT CASE (RN%N_SIMPLE_CHEMISTRY_REACTIONS)
      CASE (1) N_SIMPLE_CHEM_RXN
         ...
         SPEC_ID(6)         = 'NITROGEN'
         SPEC_ID(7)         = 'HYDROGEN CHLORIDE'
         RN%NU_CO           = (FUEL_MW/MW_CO)   * RN%CO_YIELD
         RN%NU_HCN          = (FUEL_MW/MW_HCN)  * RN%HCN_YIELD
         RN%NU_HCL          = RN%CL * (FUEL_MW/MW_HCL) * RN%HCL_YIELD
         RN%NU_SOOT         = (FUEL_MW/MW_SOOT) * RN%SOOT_YIELD
         RN%NU_H2O          = 0.5_EB*RN%H - 0.5_EB*(RN%NU_SOOT*SOOT_H_FRACTION + RN%NU_HCN + RN%NU_HCL)
         ...
         IF (RN%NU_H2O <0._EB) THEN
            WRITE(MESSAGE,'(A)') 'ERROR(172): REAC, Not enough hydrogen for the SOOT_YIELD, HCN_YIELD, and/or HCL_YIELD.'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
      ...
      CASE (2) N_SIMPLE_CHEM_RXN
         ...
         RN%NU_H2O          = 0.5_EB*(RN%H - 2._EB*RN%NU_H2 - RN%NU_HCN - RN%NU_SOOT*SOOT_H_FRACTION - RN%NU_HCL)
         IF (ABS(RN%NU_H2O)<TWO_EPSILON_EB) RN%NU_H2O = 0._EB
         ...
         RN2%NU_HCL          = RN%CL * (FUEL_MW/MW_HCL) * RN%HCL_YIELD
         RN2%NU_SOOT         = (FUEL_MW/MW_SOOT) * RN%SOOT_YIELD
         RN2%NU_H2O          = 0.5_EB*RN%H - 0.5_EB*(RN2%NU_SOOT*SOOT_H_FRACTION + RN2%NU_HCN + RN2%NU_HCL)
...
REAL(EB) :: SOOT_YIELD,CO_YIELD,HCN_YIELD,HCL_YIELD,EPUMO2,A,LOWER_OXYGEN_LIMIT, &
            CRITICAL_FLAME_TEMPERATURE,HEAT_OF_COMBUSTION,HOC_COMPLETE,E,C,H,N,O,CL, &
            ...
            E_LOW_PR,N_T_LOW_PR, A_TROE, T1_TROE, T2_TROE, T3_TROE, COMBUSTION_COMPLETENESS=-1._EB, &
...
CHARACTER(LABEL_LENGTH) :: RAMP_FS
...
NAMELIST /REAC/ A, A_TROE, A_LOW_PR, &
                ...
                AUTO_IGNITION_TEMPERATURE,C,CHECK_ATOM_BALANCE,CO_YIELD,COMBUSTION_COMPLETENESS,CRITICAL_FLAME_TEMPERATURE,E,E_LOW_PR, &
                EPUMO2,EQUATION,FLAME_SPEED,FLAME_SPEED_EXPONENT,FLAME_SPEED_TEMPERATURE, &
                FORMULA,FUEL,FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,FUEL_N_TO_HCN_FRACTION,&
                FUEL_RADCAL_ID,FYI,H,HCL_YIELD,HCN_YIELD,HEAT_OF_COMBUSTION,HOC_COMPLETE,&
                ID,IDEAL,LOWER_OXYGEN_LIMIT,N,N_S,N_SIMPLE_CHEMISTRY_REACTIONS,N_T, N_T_LOW_PR, NU,O,PRIORITY,RADIATIVE_FRACTION,&
                RAMP_CHI_R,RAMP_FS,REAC_ATOM_ERROR,REAC_MASS_ERROR,REACTYPE,REVERSE,CL,SOOT_YIELD,&
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   A_E_IF: IF (A < 0._EB .AND. E < 0._EB .AND. TRIM(SPEC_ID_NU(1))=='null' .AND. TRIM(EQUATION)=='null') THEN
      ...
      C_H_IF: IF(C<=TWO_EPSILON_EB .AND. H<=TWO_EPSILON_EB) THEN
         ...
         IF (L_TMP) THEN
            ...
            IF (ANY(ATOM_COUNTS(2:5)>0._EB) .OR. ANY(ATOM_COUNTS(9:16)>0._EB) .OR. ANY(ATOM_COUNTS(18:)>0._EB)) THEN
               WRITE(MESSAGE,'(A,I0,A)') 'ERROR(191): REAC ',NR,' Fuel FORMULA for SIMPLE_CHEMISTRY can only contain C,H,O,N, and Cl.'
               CALL SHUTDOWN(MESSAGE) ; RETURN
            ELSE
               C = ATOM_COUNTS(6)
               H = ATOM_COUNTS(1)
   ...
   ! FDS5 flame speed model parameters
   RN%FLAME_SPEED = FLAME_SPEED
   RN%FLAME_SPEED_EXPONENT = FLAME_SPEED_EXPONENT
   RN%FLAME_SPEED_TEMPERATURE = FLAME_SPEED_TEMPERATURE
   ...
   RN%RAMP_FS = RAMP_FS
   IF (RAMP_FS/='null') CALL GET_RAMP_INDEX(RN%RAMP_FS,'EQUIVALENCE_RATIO',RN%RAMP_FS_INDEX)
   ...
   ! Set fixed CCC if specified by user (0-1). Default -1 means calculate from ZETA and CHI_R
   ! Use COMBUSTION_COMPLETENESS from namelist, store in CCC_FIXED
   ! If CCC_DEFAULT is .TRUE. and user did not specify COMBUSTION_COMPLETENESS, use 0.93
   IF (COMBUSTION_COMPLETENESS >= 0._EB .AND. COMBUSTION_COMPLETENESS <= 1._EB) THEN
      RN%CCC_FIXED = COMBUSTION_COMPLETENESS
   ELSEIF (COMBUSTION_COMPLETENESS > 1._EB) THEN
      WRITE(MESSAGE,'(A,I0,A)') 'ERROR(XXX): REAC ',NR,' CCC must be between 0 and 1.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ELSEIF (CCC_DEFAULT) THEN
      RN%CCC_FIXED = 0.93_EB
   ENDIF

   ! TIER 8: FDS5_CCC — use fixed CCC (complete combustion) instead of procedural formula
   ! If CCC_FIXED is still unset (-1), set it to 1.0 (like FDS5: no mixing model for prescribed burner)
   IF (FDS5_CCC .AND. RN%CCC_FIXED < 0._EB) THEN
      RN%CCC_FIXED = 1.0_EB
   ENDIF
...
FLAME_SPEED_TEMPERATURE     = -1._EB
TURBULENT_FLAME_SPEED_ALPHA = 1._EB
...
CHARACTER(LABEL_LENGTH) :: SOLVER='SOAP'
INTEGER :: NM
...
SELECT CASE(TRIM(SOLVER))
   ...
   CASE('SOAP','SOAP_ADAPTIVE','SMOOTHNESS_BASED')
      PRES_METHOD = 'SOAP'
      PRES_FLAG   = SOAP_FLAG
      ! SOAP (Smoothness-Based Adaptive Poisson) использует адаптивную экстраполяцию на основе гладкости RHS
      ! Рекомендуется для инженерных расчетов с CFL 5-15

   CASE('LAZY_SOAP','LAZY-SOAP','LAZY','LAZY_ADAPTIVE')
      PRES_METHOD = 'LAZY_SOAP'
      PRES_FLAG   = LAZY_SOAP_FLAG
      ! LAZY-SOAP (Lazy Smoothness-Based Adaptive Poisson) — оптимизированная версия SOAP
      ! Ключевые особенности:
      ! 1. Глобальное решение о пропуске для всех сеток
      ! 2. Пропуск вычисления RHS при экстраполяции (экономия 60-70% времени)
      ! 3. Экстраполяция 2-го порядка с использованием 3 предыдущих шагов
      ! 4. Агрессивный порог 0.35 (вместо 0.15 у SOAP)
      ! 5. Контроль ошибки дивергенции для отката к FFT
      ! Рекомендуется для быстрых инженерных расчетов с CFL 8-20
