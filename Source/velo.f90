...
USE CC_SCALARS, ONLY : CC_COMPUTE_KRES,CC_COMPUTE_KRES_W,CC_COMPUTE_VISCOSITY,CC_COMPUTE_VISCOSITY_W,CUTFACE_VELOCITIES
...
SELECT_TURB: SELECT CASE (TURB_MODEL)
   ...
   CASE (DEARDORFF) SELECT_TURB ! Deardorff (1980) eddy viscosity model (current default)
      ...
      MU_TURB => WORK7  !< Store/reuse turbulent kinematic viscosity (NU_EDDY) across predictor-corrector

      IF (CORRECTOR .AND. SKIP_DEARDORFF_FILTER_CORRECTOR) THEN
         ! Skip expensive test-filter — reuse NU_EDDY from predictor step.
         ! MU_DNS is already fresh (recomputed above), only the turbulent part is cached.
         ! KSGS changes slowly (~0.1%/timestep) so this is numerically stable.
         MU = MU_DNS + RHOP * MU_TURB
      ELSE
         ! Full Deardorff computation with test filter
         ...
         DO K=1,KBAR
            DO J=1,JBAR
               DO I=1,IBAR
                  ...
                  MU_TURB(I,J,K) = NU_EDDY  !< Store for corrector reuse
               ENDDO
            ENDDO
         ENDDO
         !$OMP END PARALLEL DO
...
CALL CC_COMPUTE_KRES_W(APPLY_TO_ESTIMATED_VARIABLES,NM)
...
CALL CC_COMPUTE_VISCOSITY_W(0._EB,NM)
...
USE CC_SCALARS, ONLY : CC_VELOCITY_FLUX,CC_VELOCITY_BC,CC_VELOCITY_BC_W,ROTATED_CUBE_VELOCITY_FLUX,CUTFACE_VELOCITIES
INTEGER, INTENT(IN) :: NM
...
CALL CC_VELOCITY_BC_W(T,NM,APPLY_TO_ESTIMATED_VARIABLES,DO_IBEDGES=.FALSE.)
...
USE CC_SCALARS, ONLY : CC_PROJECT_VELOCITY,CC_PROJECT_VELOCITY_W
...
ELSE FREEZE_VELOCITY_IF
   ...
   CALL CC_PROJECT_VELOCITY_W(NM,DT,STORE_FLG=.FALSE.)
   SELECT CASE(PRES_FLAG)
...
ELSE FREEZE_VELOCITY_IF
   ...
   CALL CC_PROJECT_VELOCITY_W(NM,DT,.TRUE.)
   SELECT CASE(PRES_FLAG)
   ...
   CALL CC_PROJECT_VELOCITY_W(NM,DT,.FALSE.)
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,ULMAT_FLAG)
...
USE CC_SCALARS, ONLY : CC_VELOCITY_BC,CC_VELOCITY_BC_W,GET_OPENBC_TANGENTIAL_CUTFACE_VEL,GET_OPENBC_TANGENTIAL_CUTFACE_VEL_W
   ...
   SIGN_LOOP: DO I_SGN=-1,1,2
      ORIENTATION_LOOP: DO IS=1,3
         ...
         OPEN_AND_WIND_BC: IF ((IWM==0 .OR. WALL(IWM)%BOUNDARY_TYPE==OPEN_BOUNDARY) .AND. &
            ...
            IF (CC_IBM) CALL GET_OPENBC_TANGENTIAL_CUTFACE_VEL_W(APPLY_TO_ESTIMATED_VARIABLES,UPWIND_BOUNDARY,&
...
IF(CC_IBM) CALL CC_VELOCITY_BC_W(T,NM,APPLY_TO_ESTIMATED_VARIABLES,DO_IBEDGES=.TRUE.)
...
USE CC_SCALARS, ONLY : CC_MATCH_VELOCITY,CC_MATCH_VELOCITY_W
...
IF(CC_IBM) THEN
   CALL CC_MATCH_VELOCITY_W(NM,PREDICTOR,.TRUE.)
   RETURN
ENDIF
...
REAL(EB) :: UODX,VODY,WODZ,UVW,UVWMAX,R_DX2,MU_MAX,MUTRM,PART_CFL,MU_TMP, UVWMAX_TMP, DT_CLIP, T_NOW, CP, DIVMX_TMP, DIV_CFL_TMP
REAL(EB) :: DT_NEW(NMESHES)
...
!$OMP PARALLEL IF(REAL(IBAR,EB)*JBAR*KBAR > CHECK_STABILITY_OMP_THRESHOLD) PRIVATE(ICFL_TMP, JCFL_TMP, KCFL_TMP, UODX, VODY, WODZ, UVW, UVWMAX_TMP) SHARED(UVWMAX, ICFL, JCFL, KCFL)
UVWMAX_TMP = 0._EB
!$OMP DO SCHEDULE(STATIC)
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         ...
         SELECT CASE (CFL_VELOCITY_NORM)
            ...
            CASE(3) ; UVW = MAX(UODX,VODY,WODZ)  ! FDS5-style: exclude divergence from CFL velocity
         END SELECT
...
HEAT_TRANSFER_IF: IF (CHECK_HT) THEN
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC=>WALL(IW)
      IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY) CYCLE WALL_LOOP
      ! Use cached B1 properties — avoids B1=>BOUNDARY_PROP1(WC%B1_INDEX) indirection
      IF (FDS5_HT_CFL) THEN
         ! Still need BC for gas cell coords (required for GET_SPECIFIC_HEAT)
         BC=>BOUNDARY_COORD(WC%BC_INDEX)
         IIG = BC%IIG
         JJG = BC%JJG
         KKG = BC%KKG
         ...
         UVW = ABS(WALL_Q_CON_F_CACHE(IW))/(WALL_RHO_F_CACHE(IW)*CP)
      ELSE
         UVW = (ABS(WALL_Q_CON_F_CACHE(IW))/WALL_RHO_F_CACHE(IW))**ONTH * 2._EB*WALL_RDN_CACHE(IW)
      ENDIF
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         BC=>BOUNDARY_COORD(WC%BC_INDEX)
...
PARABOLIC_IF: IF (CHECK_VN) THEN
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         I_LOOP: DO I=1,IBAR
            IF (SOLID_CELL(I,J,K)) CYCLE I_LOOP
...
IF (CC_IBM .AND. FDS5_IBM_CFL_CHECK) THEN
   T_USED(4)=T_USED(4)+CURRENT_TIME()-T_NOW
   CALL CHECK_CFLVN_LINKED_CELLS(NM,DT,UVWMAX,R_DX2,MUTRM)
   T_NOW = CURRENT_TIME()
ENDIF

! Attempt DT restriction to avoid clippings (FDS6 behavior only)

DT_CLIP = HUGE(1._EB)
IF (FDS5_DT_RESTRICT .AND. (CLIP_RHOMIN .OR. CLIP_RHOMAX)) THEN
...
ELSE RAMP_TIME_IF
   ...
   ! === FIXED_DT_OVERRIDE MODE: FDS 5 legacy behavior ===
   IF (DT_OVERRIDE > 0._EB) THEN
      DT_NEW(NM) = DT_OVERRIDE
      CHANGE_TIME_STEP_INDEX(NM) = 0  ! Do not change time step
      ! Skip all stability checks when DT_OVERRIDE is active
   ELSE
   ...
   IF ((CFL<CFL_MAX .AND. VN<VN_MAX .AND. PART_CFL<PARTICLE_CFL_MAX) .OR. LOCK_TIME_STEP) THEN
      ...
      ENDIF
   ELSE
      ! === DISABLE_VN_CHECK and DISABLE_PARTICLE_CFL support ===
      IF (DISABLE_VN_CHECK) VN = 0._EB  ! Fake passing VN check
      IF (DISABLE_PARTICLE_CFL) PART_CFL = 0._EB  ! Fake passing Particle CFL check
   ...
   END IF  ! End of DT_OVERRIDE mode

   ! === TIER 13: FDS5-style divergence-based DT limiter ===
   ! When CFL_VELOCITY_NORM=3, divergence is excluded from CFL check.
   ! This limiter prevents runaway divergence from destabilizing the simulation
   ! at large time steps. Acts as a safety net regardless of CFL_VELOCITY_NORM.
   IF (FDS5_DIV_LIMITER) THEN
      DIVMX_TMP = 0._EB
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               DIVMX_TMP = MAX(DIVMX_TMP, ABS(DS(I,J,K)))
            ENDDO
         ENDDO
      ENDDO
      DIV_CFL_TMP = DIVMX_TMP * DT_NEW(NM)
      IF (DIV_CFL_TMP > DIVERGENCE_LIMIT) THEN
         DT_NEW(NM) = MIN(DT_NEW(NM), DIVERGENCE_LIMIT / MAX(DIVMX_TMP, DT_EPS))
         CHANGE_TIME_STEP_INDEX(NM) = -1
      ENDIF
   ENDIF
   ! === End TIER 13 ===
...
USE CC_SCALARS, ONLY: CC_BAROCLINIC_CORRECTION,CC_BAROCLINIC_CORRECTION_W
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
...
IF(CC_IBM) CALL CC_BAROCLINIC_CORRECTION_W(T,NM)
