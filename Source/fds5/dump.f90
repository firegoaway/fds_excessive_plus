...
ELSEIF (SPEC_INDEX >= DERIVED_SPEC_OFFSET .AND. N_PRODUCTS_COMP>0) THEN
   ! Derived species: compute mass fraction from PRODUCTS mixture components
   ! Y_component = Y_products_total * w_component_in_PRODUCTS
   COMP_IDX = SPEC_INDEX - DERIVED_SPEC_OFFSET
   IF (COMP_IDX >= 1 .AND. COMP_IDX <= N_PRODUCTS_COMP) THEN
      YY_GET(:) = YY(II,JJ,KK,:)
      Y_PROD_TOTAL = 0._EB
      ! Sum combustion product state species: CO2(4), H2O(5), CO(6), SOOT(7)
      DO NS = 4, MIN(7, N_STATE_SPECIES)
...
IF (COLUMN_DUMP_LIMIT) THEN
   ...
   ENDDO
ELSE
   WRITE(TCFORM,'(A,I4.4,A)') "(ES15.7E3,",N_CTRL,"(',',I2))"
