...
!                  STOP
      ...
      IF (USE_MPI) THEN
         ...
         BLOCK
            ...
            TMP_I_MIN(1:COUNTS2D(MYID)) = RESHAPE(SLLO%I_MIN(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS2D(MYID)/))
            TMP_I_MAX(1:COUNTS2D(MYID)) = RESHAPE(SLLO%I_MAX(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS2D(MYID)/))
            TMP_J_MIN(1:COUNTS2D(MYID)) = RESHAPE(SLLO%J_MIN(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS2D(MYID)/))
            TMP_J_MAX(1:COUNTS2D(MYID)) = RESHAPE(SLLO%J_MAX(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS2D(MYID)/))
            TMP_K_MIN(1:COUNTS2D(MYID)) = RESHAPE(SLLO%K_MIN(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS2D(MYID)/))
            TMP_K_MAX(1:COUNTS2D(MYID)) = RESHAPE(SLLO%K_MAX(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS2D(MYID)/))
            TMP_NIC(1:COUNTS2D(MYID))   = RESHAPE(SLLO%NIC(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS2D(MYID)/))
            
            CALL MPI_ALLGATHERV(TMP_I_MIN,COUNTS2D(MYID),MPI_INTEGER,SLLO%I_MIN,COUNTS2D,DISPLS2D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_I_MAX,COUNTS2D(MYID),MPI_INTEGER,SLLO%I_MAX,COUNTS2D,DISPLS2D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_J_MIN,COUNTS2D(MYID),MPI_INTEGER,SLLO%J_MIN,COUNTS2D,DISPLS2D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_J_MAX,COUNTS2D(MYID),MPI_INTEGER,SLLO%J_MAX,COUNTS2D,DISPLS2D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_K_MIN,COUNTS2D(MYID),MPI_INTEGER,SLLO%K_MIN,COUNTS2D,DISPLS2D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_K_MAX,COUNTS2D(MYID),MPI_INTEGER,SLLO%K_MAX,COUNTS2D,DISPLS2D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_NIC,  COUNTS2D(MYID),MPI_INTEGER,SLLO%NIC,  COUNTS2D,DISPLS2D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
...
!                  STOP
      ...
      IF (USE_MPI) THEN
         ...
         BLOCK
            ...
            ALLOCATE(TMP_I_MIN(COUNTS3D(MYID)))
            ALLOCATE(TMP_I_MAX(COUNTS3D(MYID)))
            ALLOCATE(TMP_J_MIN(COUNTS3D(MYID)))
            ALLOCATE(TMP_J_MAX(COUNTS3D(MYID)))
            ALLOCATE(TMP_K_MIN(COUNTS3D(MYID)))
            ALLOCATE(TMP_K_MAX(COUNTS3D(MYID)))
            ALLOCATE(TMP_NIC(COUNTS3D(MYID)))

            ! Copy data in column-major order (Fortran storage order)
            TMP_I_MIN(1:COUNTS3D(MYID)) = RESHAPE(SLLO%I_MIN(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS3D(MYID)/))
            TMP_I_MAX(1:COUNTS3D(MYID)) = RESHAPE(SLLO%I_MAX(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS3D(MYID)/))
            TMP_J_MIN(1:COUNTS3D(MYID)) = RESHAPE(SLLO%J_MIN(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS3D(MYID)/))
            TMP_J_MAX(1:COUNTS3D(MYID)) = RESHAPE(SLLO%J_MAX(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS3D(MYID)/))
            TMP_K_MIN(1:COUNTS3D(MYID)) = RESHAPE(SLLO%K_MIN(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS3D(MYID)/))
            TMP_K_MAX(1:COUNTS3D(MYID)) = RESHAPE(SLLO%K_MAX(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS3D(MYID)/))
            TMP_NIC(1:COUNTS3D(MYID))   = RESHAPE(SLLO%NIC(:,DISPLS(MYID)+1:DISPLS(MYID)+COUNTS(MYID)),(/COUNTS3D(MYID)/))
            
            CALL MPI_ALLGATHERV(TMP_I_MIN,COUNTS3D(MYID),MPI_INTEGER,SLLO%I_MIN,COUNTS3D,DISPLS3D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_I_MAX,COUNTS3D(MYID),MPI_INTEGER,SLLO%I_MAX,COUNTS3D,DISPLS3D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_J_MIN,COUNTS3D(MYID),MPI_INTEGER,SLLO%J_MIN,COUNTS3D,DISPLS3D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_J_MAX,COUNTS3D(MYID),MPI_INTEGER,SLLO%J_MAX,COUNTS3D,DISPLS3D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_K_MIN,COUNTS3D(MYID),MPI_INTEGER,SLLO%K_MIN,COUNTS3D,DISPLS3D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_K_MAX,COUNTS3D(MYID),MPI_INTEGER,SLLO%K_MAX,COUNTS3D,DISPLS3D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
            CALL MPI_ALLGATHERV(TMP_NIC,  COUNTS3D(MYID),MPI_INTEGER,SLLO%NIC,  COUNTS3D,DISPLS3D,MPI_INTEGER,MPI_COMM_WORLD,IERR)
...
IF (USE_MPI) THEN
   BLOCK
      ...
      ALLOCATE(TMP_TUSED(COUNTS_SCARC_TIMERS(MYID)))
      TMP_TUSED(:) = RESHAPE(TUSED_SCARC(:,DISPLS_SCARC(MYID)+1:DISPLS_SCARC(MYID)+COUNTS_SCARC(MYID)),(/COUNTS_SCARC_TIMERS(MYID)/))
      CALL MPI_GATHERV(TMP_TUSED,COUNTS_SCARC_TIMERS(MYID), &
