         ...
         SUM_MASSFLUX_LOOP: DO N=0,N_SPECIES
            IF (SF%MASS_FLUX(N) > 0._EB) THEN  ! Use user-specified ramp-up of mass flux
               ...
               ENDIF
                ...
                ! BUG#11: троттлинг впрыска топлива при газофазном дефиците горения
                IF (GOVERN_HRR) MASSFLUX(IW,N) = MASSFLUX(IW,N)*F_INJECT
...
IF (VIRTUAL_PARTICLES .AND. CORRECTOR) THEN
   DROPLET_LOOP: DO I=1,NLP
      ...
      ITMP = MAX(0, MIN(5000, NINT(TMP(II,JJ,KK))))
      CP = Y2CP_C(ITMP)
      H_G = CP*TMP(II,JJ,KK)
      DELTA_H_G = 0._EB
      DO N=1,N_SPECIES
...
WALL_CELL_LOOP: DO IW=1,NWC+NVWC
   ...
   NEW_EVAP_IF: IF (NEW_EVAP) THEN
      MATERIAL_LOOP2B: DO N=1,SF%N_MATL
         ...
         ELSE
            MU_AIR = Y2MU_C(MAX(0, MIN(5000, NINT(TMP_G))))*SPECIES(0)%MW
