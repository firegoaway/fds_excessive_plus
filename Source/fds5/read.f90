...
IF (FN_INPUT(1:1)==' ') THEN
   IF (MYID==0) THEN
      ...
      IF (.NOT.USE_MPI .AND. .NOT.USE_OPENMP)   &
         WRITE(LU_OUTPUT,'(/A,A,A)') "Version: ",TRIM(VERSION_STRING),"; MPI Disabled; OpenMP Disabled"
      IF (     USE_MPI .AND. .NOT.USE_OPENMP)   &
         WRITE(LU_OUTPUT,'(/A,A,A)') "Version: ",TRIM(VERSION_STRING),"; MPI Enabled; OpenMP Disabled"
      IF (.NOT.USE_MPI .AND.      USE_OPENMP)   &
         WRITE(LU_OUTPUT,'(/A,A,A)') "Version: ",TRIM(VERSION_STRING),"; MPI Disabled; OpenMP Enabled"
      IF (     USE_MPI .AND.      USE_OPENMP)   &
         WRITE(LU_OUTPUT,'(/A,A,A)') "Version: ",TRIM(VERSION_STRING),"; MPI Enabled; OpenMP Enabled"
   ENDIF
   IF (USE_OPENMP .and. .NOT.USE_MPI) &
      WRITE(LU_OUTPUT,'(A,I3)') 'Number of available OpenMP threads: ',OPENMP_AVAILABLE_THREADS
   IF (MYID==0) THEN
      WRITE(LU_OUTPUT,'(A,I4)') "SVN Revision Number: ",SVN_REVISION_NUMBER
      WRITE(LU_OUTPUT,'(A,A)') "Compile Date: ",TRIM(COMPILE_DATE)
      WRITE(LU_OUTPUT,'(/A)')  "Consult FDS Users Guide Chapter, Running FDS, for further instructions."
      WRITE(LU_OUTPUT,'(/A)')  "Hit Enter to Escape..."
      READ(5,*,ERR=2,END=2)
   ENDIF
 2 STOP
ENDIF
...
IF (.NOT.EX) THEN
   IF (MYID==0) WRITE(LU_OUTPUT,'(A,A,A)') "ERROR: The file, ", TRIM(FN_INPUT),", does not exist in the current directory"
   STOP
ENDIF
...
CALL READ_HEAD
CALL READ_MISC
CALL READ_WIND    ! FDS6 compatibility stub (reads and ignores &WIND)
CALL READ_COMB    ! FDS6 compatibility stub (reads and ignores &COMB)
CALL READ_MULT
CALL READ_MESH
CALL READ_TRAN
CALL READ_TIME

! Enable external control file reading if configured (FDS6/Fenix+ compatibility)
IF (TRIM(EXTERNAL_FILENAME) /= 'null' .AND. DT_EXTERNAL > 0._EB) THEN
   READ_EXTERNAL = .TRUE.
ENDIF
...
CALL BUILD_FS_BURNERS  ! R26: список prescribed-очагов для FLAME_SHEET (проверки совместимости)
CALL READ_DUMP
CALL READ_CLIP
CALL READ_INIT
CALL READ_ZONE
...
SUBROUTINE BUILD_FS_BURNERS
USE MPI   ! R28b: MPI_ALLREDUCE для глобального числа FS-очагов

! R26 FLAME_SHEET (режим A): сборка глобального списка prescribed-очагов из VENT-записей
! с SURF%HRRPUA>0. R26e: горизонтальные (IOR=±3) — как прежде (footprint = грань);
! вертикальные (IOR=±1,±2) — горизонтальной проекцией: footprint — колонка клеток
! вдоль нормали (±полклетки), конус стартует от верхней кромки грани (ZV=Z2),
! мощность берётся по истинной площади грани AREA_EQ (сохранение суммарной мощности).
! VENT дублируется на каждый меш: дедупликация по проекционной геометрии (первая копия задаёт группу).

CHARACTER(80) :: MESSAGE
INTEGER :: NB_GLOBAL
INTEGER :: NM,NV,NB,N_TOT,N_BURN,N_I3,N_VERT
REAL(EB) :: DXM,DYM,XV,YV,X1E,X2E,Y1E,Y2E,ZVE,AREA_EQ
LOGICAL :: DUP
TYPE (VENTS_TYPE), POINTER :: VT=>NULL()
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()

N_FS_BURNERS = 0
IF (.NOT.FLAME_SHEET) RETURN

IF (CCC_FIXED_GLOBAL>=0._EB .AND. CCC_FIXED_GLOBAL<=1._EB) &
   CALL SHUTDOWN('ERROR: FLAME_SHEET incompatible with CCC_FIXED_GLOBAL (MLR-injection is CCC-scaled)')
IF (CO_PRODUCTION) &
   CALL SHUTDOWN('ERROR: FLAME_SHEET V1 does not support CO_PRODUCTION')

ALLOCATE(FS_BURNERS(MAX(1,NMESHES*16)))

! R26b: диагностический скан (сводка по рангам); R26e: + счёт вертикальных граней
N_TOT = 0
N_BURN = 0
N_I3 = 0
N_VERT = 0
DO NM=1,NMESHES
   ...
   DO NV=1,MESHES(NM)%N_VENT
      ...
      IF (VT%IBC<0 .OR. VT%IBC>N_SURF) CYCLE
      SF => SURFACE(VT%IBC)
      IF (SF%HRRPUA>0._EB .OR. SF%MLRPUA>0._EB) THEN
         N_BURN = N_BURN + 1
         IF (ABS(VT%IOR)==3) N_I3 = N_I3 + 1
         IF (ABS(VT%IOR)==1 .OR. ABS(VT%IOR)==2) N_VERT = N_VERT + 1
      ENDIF
   ENDDO
ENDDO
WRITE(LU_ERR,'(A,I3,A,I6,A,I6,A,I6,A,I6)') ' FS-scan rank ',MYID,': vents= ',N_TOT, &
   ' burning= ',N_BURN,' IOR3= ',N_I3,' vert= ',N_VERT

VENT_LOOP: DO NM=1,NMESHES
   ! IOR определяется только на ранге-владельце меша (MESH_LOOP_2 в READ_VENT):
   ! регистрацию и проверки ведём только для своих мешей
   IF (PROCESS(NM)/=MYID) CYCLE VENT_LOOP
   DXM = (MESHES(NM)%XF-MESHES(NM)%XS)/REAL(MESHES(NM)%IBAR,EB)
   DYM = (MESHES(NM)%YF-MESHES(NM)%YS)/REAL(MESHES(NM)%JBAR,EB)
   DO NV=1,MESHES(NM)%N_VENT
      VT => MESHES(NM)%VENTS(NV)
      IF (VT%IBC<0 .OR. VT%IBC>N_SURF) CYCLE
      SF => SURFACE(VT%IBC)
      IF (SF%HRRPUA<=0._EB) THEN
         IF (SF%MLRPUA>0._EB) &
            CALL SHUTDOWN('ERROR: FLAME_SHEET V1 supports HRRPUA burners only (MLRPUA-only surface found)')
         CYCLE
      ENDIF
      ! R26e: проекция грани на горизонтальный footprint
      AREA_EQ = -1._EB
      SELECT CASE (VT%IOR)
      CASE (3,-3)
         X1E = VT%X1; X2E = VT%X2; Y1E = VT%Y1; Y2E = VT%Y2; ZVE = VT%Z1
      CASE (1,-1)
         XV  = 0.5_EB*(VT%X1+VT%X2)
         X1E = XV-0.5_EB*DXM; X2E = XV+0.5_EB*DXM
         Y1E = VT%Y1;         Y2E = VT%Y2
         ZVE = VT%Z2
         AREA_EQ = (VT%Y2-VT%Y1)*(VT%Z2-VT%Z1)
      CASE (2,-2)
         YV  = 0.5_EB*(VT%Y1+VT%Y2)
         Y1E = YV-0.5_EB*DYM; Y2E = YV+0.5_EB*DYM
         X1E = VT%X1;         X2E = VT%X2
         ZVE = VT%Z2
         AREA_EQ = (VT%X2-VT%X1)*(VT%Z2-VT%Z1)
      CASE DEFAULT
         WRITE(MESSAGE,'(A,I5,A)') 'ERROR: FLAME_SHEET: vent ',VT%ORDINAL,' has no orientation (IOR=0)'
         CALL SHUTDOWN(MESSAGE)
      END SELECT
      DUP = .FALSE.
      DO NB=1,N_FS_BURNERS
         IF (FS_BURNERS(NB)%X1==X1E .AND. FS_BURNERS(NB)%X2==X2E .AND. &
             FS_BURNERS(NB)%Y1==Y1E .AND. FS_BURNERS(NB)%Y2==Y2E .AND. &
             FS_BURNERS(NB)%ZV==ZVE) THEN
            DUP = .TRUE.
            EXIT
         ENDIF
      ENDDO
      IF (DUP) CYCLE
      N_FS_BURNERS = N_FS_BURNERS + 1
      FS_BURNERS(N_FS_BURNERS)%X1         = X1E
      FS_BURNERS(N_FS_BURNERS)%X2         = X2E
      FS_BURNERS(N_FS_BURNERS)%Y1         = Y1E
      FS_BURNERS(N_FS_BURNERS)%Y2         = Y2E
      FS_BURNERS(N_FS_BURNERS)%ZV         = ZVE
      FS_BURNERS(N_FS_BURNERS)%QPP        = SF%HRRPUA
      FS_BURNERS(N_FS_BURNERS)%AREA_EQ    = AREA_EQ
      FS_BURNERS(N_FS_BURNERS)%RAMP_INDEX = SF%RAMP_INDEX(TIME_HEAT)
      FS_BURNERS(N_FS_BURNERS)%TAU_Q      = SF%TAU(TIME_HEAT)
      FS_BURNERS(N_FS_BURNERS)%T_IGN      = SF%T_IGN
   ENDDO
ENDDO VENT_LOOP

! R28b: N_FS_BURNERS перранговый (очаг регистрирует ранг-владелец меша) —
! сообщение о числе групп и предупреждение печатаем по глобальной сумме,
! иначе ранг 0 без очагов печатает ложное «no burner vents found»
NB_GLOBAL = N_FS_BURNERS
IF (USE_MPI) CALL MPI_ALLREDUCE(N_FS_BURNERS,NB_GLOBAL,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,IZERO)
N_FS_BURNERS_GLOBAL = NB_GLOBAL
IF (MYID==0) THEN
   IF (NB_GLOBAL==0) THEN
      WRITE(LU_OUTPUT,'(A)') ' WARNING: FLAME_SHEET is active but no burner vents (HRRPUA, IOR=3) found'
   ELSE
      WRITE(LU_OUTPUT,'(A,I5,A)') ' FLAME_SHEET mode A: ',NB_GLOBAL,' burner group(s) registered'
   ENDIF
ENDIF

END SUBROUTINE BUILD_FS_BURNERS
...
INTEGER :: IJK(3),NM,NOM,CURRENT_MPI_PROCESS,MPI_PROCESS,RGB(3),LEVEL,N_MESH_NEW,N,II,JJ,KK,NMESHES_READ,NNN
LOGICAL :: EVACUATION, EVAC_HUMANS, EMBEDDED
...
NAMELIST /MESH/ IJK,FYI,ID,SYNCHRONIZE,EVACUATION,EVAC_HUMANS,CYLINDRICAL,XB,RGB,COLOR,EVAC_Z_OFFSET, &
                MPI_PROCESS,LEVEL,MULT_ID,EMBEDDED
TYPE (MESH_TYPE), POINTER :: M
...
! Initialize OBSTRUCTION pointer to NULL for all meshes
DO NM=1,NMESHES
   MESHES(NM)%OBSTRUCTION => NULL()
   MESHES(NM)%N_OBST = 0
ENDDO
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            ! FDS6 compatibility: MPI_PROCESS parameter is read but ignored
            ! Always use automatic distribution for optimal load balancing

            IF (     USE_MPI) CURRENT_MPI_PROCESS = MOD(NM-1,NUMPROCS)
            ...
            M => MESHES(NM)
            M%MESH_LEVEL = LEVEL
            IF (EMBEDDED .OR. LEVEL > 0) THEN
               M%EMBEDDED_MESH_FLAG = .TRUE.
               M%EMBEDDED_RATIO = EMBEDDED_MESH_RATIO
            ENDIF
            M%IBAR = IJK(1)
            M%JBAR = IJK(2)
            M%KBAR = IJK(3)
            ...
            IF (MYID==0 .AND. USE_MPI) WRITE(LU_OUTPUT,'(A,I3,A,I3)') 'Mesh ',NM,' is assigned to Process ',PROCESS(NM)
...
! Auto-detect embedded meshes: if mesh NM is strictly inside another mesh,
! mark it as embedded automatically (no need for user to specify EMBEDDED=.TRUE.)
AUTO_EMBED_LOOP: DO NM=1,NMESHES
   IF (MESHES(NM)%EMBEDDED_MESH_FLAG) CYCLE AUTO_EMBED_LOOP
   DO NOM=1,NMESHES
      IF (NOM==NM) CYCLE
      ! Check strict containment: child must be strictly smaller than parent on at least one side
      IF (MESHES(NM)%XS >= MESHES(NOM)%XS .AND. MESHES(NM)%XF <= MESHES(NOM)%XF .AND. &
          MESHES(NM)%YS >= MESHES(NOM)%YS .AND. MESHES(NM)%YF <= MESHES(NOM)%YF .AND. &
          MESHES(NM)%ZS >= MESHES(NOM)%ZS .AND. MESHES(NM)%ZF <= MESHES(NOM)%ZF) THEN
         ! Require at least one face to be strictly inside (not coincident boundaries)
         IF (MESHES(NM)%XS > MESHES(NOM)%XS .OR. MESHES(NM)%XF < MESHES(NOM)%XF .OR. &
             MESHES(NM)%YS > MESHES(NOM)%YS .OR. MESHES(NM)%YF < MESHES(NOM)%YF .OR. &
             MESHES(NM)%ZS > MESHES(NOM)%ZS .OR. MESHES(NM)%ZF < MESHES(NOM)%ZF) THEN
            MESHES(NM)%EMBEDDED_MESH_FLAG = .TRUE.
            MESHES(NM)%EMBEDDED_RATIO = EMBEDDED_MESH_RATIO
            CYCLE AUTO_EMBED_LOOP
         ENDIF
      ENDIF
   ENDDO
ENDDO AUTO_EMBED_LOOP

! Find parent meshes for embedded meshes

DO NM=1,NMESHES
   IF (.NOT.MESHES(NM)%EMBEDDED_MESH_FLAG) CYCLE
   MESHES(NM)%PARENT_MESH = FIND_PARENT_MESH(NM)
   IF (MESHES(NM)%PARENT_MESH == 0) THEN
      WRITE(MESSAGE,'(A,I3)') 'WARNING: No parent mesh found for embedded MESH ',NM
      CALL SHUTDOWN(MESSAGE)
   ENDIF
ENDDO
...
USE GLOBAL_CONSTANTS, ONLY: DT_EXTERNAL
...
NAMELIST /TIME/ DT,T_BEGIN,T_END,TWFIN,FYI,WALL_INCREMENT,SYNCHRONIZE, &
                ...
                LOCK_TIME_STEP,RESTRICT_TIME_STEP,DT_EXTERNAL
TYPE (MESH_TYPE), POINTER :: M
...
USE GLOBAL_CONSTANTS, ONLY: EXTERNAL_FILENAME
...
INTEGER :: MAX_LEAK_PATHS=0
REAL(EB) :: MINIMUM_ZONE_VOLUME=1._EB
NAMELIST /MISC/ PR,SC,TMPA,GVEC,PRESSURE_RELAX_FACTOR,RELAXATION_FACTOR,FYI, &
                ...
                NOBIAS,VAN_DRIEST,HRRPUVCUT_MAX,EMBEDDED_MESH,EMBEDDED_MESH_RATIO,RELAXED_ALIGNMENT,RUN_AVG_FAC,THERMOPHORETIC_DEPOSITION,TURBULENT_DEPOSITION, &
                ...
                VISIBILITY_FACTOR,MAXIMUM_VISIBILITY,MAX_LEAK_PATHS,MINIMUM_ZONE_VOLUME,EXTERNAL_FILENAME,CCC_DEFAULT, &
...
VISIBILITY_FACTOR  = -1._EB        ! FDS6: задаётся в &MISC; -1 = не задано (см. READ_REAC)
MAXIMUM_VISIBILITY = -1._EB        ! FDS6: задаётся в &MISC; -1 = не задано (см. READ_REAC)
...
CSMAG                = 0.4_EB  ! Smagorinsky constant
...
VN_MAX               = 1.0_EB
VN_MIN               = 0.8_EB
CFL_DP_SCALE         = 1.0_EB       ! масштаб DP-члена (дивергенции) в CFL-норме; <1 — быстрый грубый режим
 DT_GUARD              = .FALSE.    ! R25: страж по умолчанию ВЫКЛ — r24c доказал: кэп 0.01 на unsplit
                                    ! душит зажигание слоя (burn_frac 0.44 против 0.97 у r20); события слоя
...
! FDS6: видимость по умолчанию, если не задана ни в &MISC, ни позже в &REAC
IF (VISIBILITY_FACTOR  < 0._EB) VISIBILITY_FACTOR  = 3._EB
IF (MAXIMUM_VISIBILITY < 0._EB) MAXIMUM_VISIBILITY = 30._EB
...
TMPA  = TMPA + TMPM
TMPA4 = TMPA**4
TEMP_MIN_SMV = REAL(TMPA-TMPM,FB)
...
IF (.NOT. SUPPRESSION .AND. CO_PRODUCTION) THEN
   WRITE(MESSAGE,'(A)')  'WARNING: SUPPRESSION=.FALSE. with CO_PRODUCTION=.TRUE. Results may be unrealistic.'
   WRITE(LU_OUTPUT,'(A)') MESSAGE
ENDIF
! БЫЛО
!IF (.NOT. SUPPRESSION .AND. CO_PRODUCTION) THEN
...
! ============================================================================
! FDS6 COMPATIBILITY STUBS
! ============================================================================

SUBROUTINE READ_WIND

! Read and ignore FDS6 &WIND namelist (stub for compatibility)
! FDS5 uses U0, V0, W0 from &MISC instead

REAL(EB) :: DIRECTION,SPEED,Z_0,L,U_STAR,GROUND_LEVEL_WIND
REAL(EB) :: U0_WIND,V0_WIND,W0_WIND
CHARACTER(100) :: FYI_WIND
NAMELIST /WIND/ DIRECTION,SPEED,Z_0,L,U_STAR,GROUND_LEVEL_WIND,U0_WIND,V0_WIND,W0_WIND,FYI_WIND
...
WIND_LOOP: DO
   ...
   READ(LU_INPUT,NML=WIND,END=29,ERR=30,IOSTAT=IOS)
   ! Read and ignore - FDS5 uses U0, V0, W0 from &MISC
   30 IF (IOS>0) THEN
      ! Just skip errors silently for compatibility
      IOS = 0
   ENDIF
ENDDO WIND_LOOP
29 REWIND(LU_INPUT)
...
! Read and ignore FDS6 &COMB namelist (stub for compatibility)
! FDS5 uses EPUMO2 from &REAC instead

REAL(EB) :: CHI_R,FLAME_SPEED
LOGICAL :: SIMPLE_CHEMISTRY
CHARACTER(100) :: FYI_COMB
NAMELIST /COMB/ CHI_R,FLAME_SPEED,SIMPLE_CHEMISTRY,FYI_COMB
...
COMB_LOOP: DO
   ...
   READ(LU_INPUT,NML=COMB,END=29,ERR=30,IOSTAT=IOS)
   ! Read and ignore - FDS5 uses EPUMO2 from &REAC
   30 IF (IOS>0) THEN
      ! Just skip errors silently for compatibility
      IOS = 0
   ENDIF
...
NAMELIST /DUMP/ RENDER_FILE,SMOKE3D,SMOKE3D_COMPRESSION,SMOKE3D_QUANTITY,FLUSH_FILE_BUFFERS,MASS_FILE,STATE_FILE, &
                ...
                DT_SL3D,DIAGNOSTICS_INTERVAL, &
                CFL_FILE,DT_CFL,STEPS_FILE,DT_STEPS,CPU_FILE,DT_CPU
 
RENDER_FILE          = 'null'
NFRAMES              = -1  ! Will be set to T_END if not specified
SMOKE3D              = .TRUE.
...
COLUMN_DUMP_LIMIT = .FALSE.  ! FDS6 compatibility: single _devc.csv file (no column limit)
...
ENDDO DUMP_LOOP
23 REWIND(LU_INPUT)
! FDS6 compatibility: NFRAMES defaults to T_END (one frame per second)
IF (NFRAMES < 0) NFRAMES = MAX(1, NINT(T_END - T_BEGIN))
...
IF (DT_CFL  < 0._EB) DT_CFL  = (T_END - T_BEGIN)/REAL(NFRAMES,EB)
IF (DT_STEPS< 0._EB) DT_STEPS= (T_END - T_BEGIN)/REAL(NFRAMES,EB)
IF (DT_CPU  < 0._EB) DT_CPU  = (T_END - T_BEGIN)/REAL(NFRAMES,EB)
...
REAL(EB) :: MASS_FRACTION_0,MW,SIGMALJ,EPSILONKLJ,XVAP,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
             SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,H_V,CL
INTEGER  :: N_SPEC_READ,N_MIX,N,I,IPC,MODE
LOGICAL  :: ABSORBING
LOGICAL  :: LUMPED_COMPONENT_ONLY,BACKGROUND,COPY_LUMPED,PRIMITIVE
CHARACTER(60) :: SPEC_ID(100)
REAL(EB) :: VOLUME_FRACTION(100),MASS_FRACTION_FDS6(100)
...
NAMELIST /SPEC/ MASS_FRACTION_0,MW,FYI,ID,SIGMALJ,EPSILONKLJ,CONDUCTIVITY,VISCOSITY,DIFFUSIVITY,ABSORBING, &
                 ...
                 LUMPED_COMPONENT_ONLY,BACKGROUND,COPY_LUMPED,PRIMITIVE, &
                 SPEC_ID,VOLUME_FRACTION,MASS_FRACTION_FDS6,CL
...
ID = 'null'
    ...
    MASS_FRACTION_FDS6 = 0._EB
    MW = 0._EB
    CL = 0._EB
   READ(LU_INPUT,NML=SPEC,END=29,ERR=30,IOSTAT=IOS)
   IF (IOS>0) THEN
      ! Namelist read error — likely FDS6 syntax not recognized, skip this line
      CYCLE COUNT_SPEC_LOOP
   ENDIF
   
    ! Skip FDS6-style species (LUMPED_COMPONENT_ONLY, BACKGROUND, or mixtures with SPEC_ID)
    IF (LUMPED_COMPONENT_ONLY .OR. BACKGROUND .OR. COPY_LUMPED .OR. PRIMITIVE) THEN
       ! R28j: состав люмп-AIR (BACKGROUND-смесь) для O2-стехиометрии SPEC_ID_NU
       IF (BACKGROUND .AND. SPEC_ID(1)/='null' .AND. AIR_LUMP_XMW_SUM<=0._EB) THEN
          DO I=1,100
             IF (SPEC_ID(I)=='null' .OR. SPEC_ID(I)==' ') CYCLE
             AIR_LUMP_XMW_SUM = AIR_LUMP_XMW_SUM + VOLUME_FRACTION(I)*GET_STANDARD_SPEC_MW(TRIM(SPEC_ID(I)))
             IF (TRIM(SPEC_ID(I))=='OXYGEN') AIR_LUMP_X_O2 = VOLUME_FRACTION(I)
          ENDDO
       ENDIF
       ! But if this species has a MW, save it as a potential fuel species
       IF (MW > 0._EB .AND. N_FUEL_SPEC < MAX_FUEL_SPEC) THEN
         N_FUEL_SPEC = N_FUEL_SPEC + 1
         FUEL_SPEC_ID(N_FUEL_SPEC) = ID
         FUEL_SPEC_MW(N_FUEL_SPEC) = MW
      ENDIF
      CYCLE COUNT_SPEC_LOOP
   ENDIF
   IF (SPEC_ID(1) /= 'null') THEN
      ! Extract PRODUCTS mixture components for derived SLCF species output (e.g. HCl)
      ! Convert VOLUME_FRACTION to mass fractions and store in PRODUCTS_COMP_* arrays
      IF (N_PRODUCTS_COMP == 0) THEN
         CALL EXTRACT_PRODUCTS_COMPONENTS(SPEC_ID, VOLUME_FRACTION, MASS_FRACTION_FDS6)
      ENDIF
      CYCLE COUNT_SPEC_LOOP
   ENDIF

! Save FDS5-style species with MW as potential fuel
    IF (MW > 0._EB .AND. N_FUEL_SPEC < MAX_FUEL_SPEC) THEN
       N_FUEL_SPEC = N_FUEL_SPEC + 1
       FUEL_SPEC_ID(N_FUEL_SPEC) = ID
       FUEL_SPEC_MW(N_FUEL_SPEC) = MW
       FUEL_SPEC_CL(N_FUEL_SPEC) = CL  ! Store Cl content for HCl production
    ENDIF
   ...
   30 IF (IOS>0) THEN
         ! Namelist read error — skip FDS6-incompatible SPEC lines silently
         CYCLE COUNT_SPEC_LOOP
      ENDIF
...
SPEC_LOOP: DO N=0,N_SPECIES
   ...
   ELSEIF (N<=N_SPEC_READ) THEN
      ! Read SPEC lines, skipping FDS6-style species
FDS6_SKIP_LOOP: DO
          CALL CHECKREAD('SPEC',LU_INPUT,IOS)
          LUMPED_COMPONENT_ONLY = .FALSE.
          BACKGROUND = .FALSE.
          ...
          MASS_FRACTION_FDS6 = 0._EB
          CL = 0._EB
          READ(LU_INPUT,NML=SPEC,ERR=31,IOSTAT=IOS)
         31 IF (IOS>0) THEN
            ! Namelist read error — likely FDS6 syntax, skip and continue
            IOS = 0
            CYCLE FDS6_SKIP_LOOP
         ENDIF
         IF (LUMPED_COMPONENT_ONLY .OR. BACKGROUND .OR. COPY_LUMPED .OR. PRIMITIVE) CYCLE FDS6_SKIP_LOOP
         IF (SPEC_ID(1) /= 'null') CYCLE FDS6_SKIP_LOOP
         EXIT FDS6_SKIP_LOOP
      ENDDO FDS6_SKIP_LOOP
   ELSE
      SELECT CASE(ID)
...
SUBROUTINE EXTRACT_PRODUCTS_COMPONENTS(SPEC_ID_ARR, VOL_FRAC, MASS_FRAC_FDS6)

! Extract component species from FDS6-style mixture (e.g., PRODUCTS)
! Convert volume fractions to mass fractions for derived SLCF output

CHARACTER(60), INTENT(IN) :: SPEC_ID_ARR(100)
REAL(EB), INTENT(IN) :: VOL_FRAC(100), MASS_FRAC_FDS6(100)
INTEGER :: I, NC
REAL(EB) :: COMP_MW(100), SUM_XMW, TOTAL_VF

NC = 0
DO I = 1, 100
   IF (SPEC_ID_ARR(I) == 'null' .OR. SPEC_ID_ARR(I) == ' ') CYCLE
   NC = NC + 1
   IF (NC > MAX_PRODUCTS_COMP) EXIT
   PRODUCTS_COMP_ID(NC) = SPEC_ID_ARR(I)

   ! Get MW for this component species
   COMP_MW(NC) = GET_STANDARD_SPEC_MW(TRIM(SPEC_ID_ARR(I)))
   PRODUCTS_COMP_MW(NC) = COMP_MW(NC)
   PRODUCTS_COMP_RCON(NC) = R0/COMP_MW(NC)
ENDDO

IF (NC == 0) RETURN

! Determine if we have mass fractions or volume fractions
IF (ANY(MASS_FRAC_FDS6(1:NC) > 0._EB)) THEN
   ! Use mass fractions directly
   TOTAL_VF = SUM(MASS_FRAC_FDS6(1:NC))
   IF (TOTAL_VF > 0._EB) THEN
      DO I = 1, NC
         PRODUCTS_COMP_MASS_FRAC(I) = MASS_FRAC_FDS6(I) / TOTAL_VF
      ENDDO
   ENDIF
ELSE
   ! Convert volume fractions to mass fractions: w_i = (x_i * MW_i) / sum(x_j * MW_j)
   SUM_XMW = 0._EB
   DO I = 1, NC
      SUM_XMW = SUM_XMW + VOL_FRAC(I) * COMP_MW(I)
   ENDDO
   IF (SUM_XMW > 0._EB) THEN
      DO I = 1, NC
         PRODUCTS_COMP_MASS_FRAC(I) = VOL_FRAC(I) * COMP_MW(I) / SUM_XMW
      ENDDO
   ENDIF
ENDIF

! R28j: ненормированная молярная масса люмп-смеси (конвенция Fenix: NU·MW сходится по массе)
PRODUCTS_MW_SUM = 0._EB
DO I = 1, NC
   PRODUCTS_MW_SUM = PRODUCTS_MW_SUM + VOL_FRAC(I)*COMP_MW(I)
ENDDO

N_PRODUCTS_COMP = NC
PRODUCTS_H2O_COMP = 0
DO I = 1, NC
   IF (TRIM(PRODUCTS_COMP_ID(I))=='WATER VAPOR') PRODUCTS_H2O_COMP = I
ENDDO

! Diagnostic output
WRITE(LU_OUTPUT,'(A,I0,A)') 'INFO: Extracted ',NC,' components from PRODUCTS mixture'
DO I = 1, NC
   WRITE(LU_OUTPUT,'(A,A,A,F8.5)') '  ',TRIM(PRODUCTS_COMP_ID(I)),': mass_frac=',PRODUCTS_COMP_MASS_FRAC(I)
ENDDO

END SUBROUTINE EXTRACT_PRODUCTS_COMPONENTS


FUNCTION GET_STANDARD_SPEC_MW(NAME) RESULT(MW_OUT)

! Return molecular weight for standard FDS species names

CHARACTER(*), INTENT(IN) :: NAME
REAL(EB) :: MW_OUT

SELECT CASE(TRIM(NAME))
   CASE('SOOT')                ; MW_OUT = 12.011_EB
   CASE('CARBON DIOXIDE')      ; MW_OUT = 44.010_EB
   CASE('CARBON MONOXIDE')     ; MW_OUT = 28.010_EB
   CASE('HYDROGEN CHLORIDE')   ; MW_OUT = 36.460_EB
   CASE('HYDROGEN FLUORIDE')   ; MW_OUT = 20.006_EB
   CASE('HYDROGEN BROMIDE')    ; MW_OUT = 80.912_EB
   CASE('WATER VAPOR')         ; MW_OUT = 18.015_EB
   CASE('NITROGEN')            ; MW_OUT = 28.014_EB
   CASE('OXYGEN')              ; MW_OUT = 31.998_EB
   CASE('HYDROGEN')            ; MW_OUT = 2.016_EB
   CASE('HYDROGEN CYANIDE')    ; MW_OUT = 27.026_EB
   CASE('METHANE')             ; MW_OUT = 16.043_EB
   CASE('ETHANE')              ; MW_OUT = 30.069_EB
   CASE('PROPANE')             ; MW_OUT = 44.096_EB
   CASE('CARBON')              ; MW_OUT = 12.011_EB
   CASE DEFAULT
      MW_OUT = 28.0_EB  ! Default to N2-like MW
      WRITE(LU_OUTPUT,'(A,A,A)') 'WARNING: Unknown species MW for ',TRIM(NAME),', using 28.0'
END SELECT

END FUNCTION GET_STANDARD_SPEC_MW


SUBROUTINE REAC_PRESCAN_FENIX

! R28j: предварительный скан &SPEC для READ_REAC (выполняется раньше READ_SPEC — порядок FDS5).
! Заполняет PRODUCTS_COMP_*/PRODUCTS_MW_SUM (смесь PRODUCTS), AIR_LUMP_* (люмп-AIR)
! и FUEL_SPEC_* (MW топливной SPEC) до построения state-системы смеси.

REAL(EB) :: MW,MASS_FRACTION_0,SIGMALJ,EPSILONKLJ,XVAP,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY, &
            MASS_EXTINCTION_COEFFICIENT,SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,H_V,CL
REAL(EB) :: VOLUME_FRACTION(100),MASS_FRACTION_FDS6(100)
INTEGER :: I,IOS_PRE
LOGICAL :: LUMPED_COMPONENT_ONLY,BACKGROUND,COPY_LUMPED,PRIMITIVE,ABSORBING
CHARACTER(60) :: SPEC_ID(100),FYI
CHARACTER(30) :: FORMULA,ID
NAMELIST /SPEC/ MASS_FRACTION_0,MW,FYI,ID,SIGMALJ,EPSILONKLJ,CONDUCTIVITY,VISCOSITY,DIFFUSIVITY,ABSORBING, &
                   MASS_EXTINCTION_COEFFICIENT,FORMULA,SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE, &
                   LUMPED_COMPONENT_ONLY,BACKGROUND,COPY_LUMPED,PRIMITIVE, &
                   SPEC_ID,VOLUME_FRACTION,MASS_FRACTION_FDS6,CL
...
REWIND(LU_INPUT)
PRE_LOOP: DO
   CALL CHECKREAD('SPEC',LU_INPUT,IOS_PRE)
   IF (IOS_PRE==1) EXIT PRE_LOOP
   ID = 'null'
   MW = 0._EB
   CL = 0._EB
   LUMPED_COMPONENT_ONLY = .FALSE.
   ...
   VOLUME_FRACTION = 0._EB
   MASS_FRACTION_FDS6 = 0._EB
   READ(LU_INPUT,NML=SPEC,END=111,ERR=112,IOSTAT=IOS_PRE)
   GOTO 113
   111 EXIT PRE_LOOP
   112 CYCLE PRE_LOOP
   113 CONTINUE
   IF (BACKGROUND .AND. SPEC_ID(1)/='null' .AND. AIR_LUMP_XMW_SUM<=0._EB) THEN
      ...
      ENDDO
   ENDIF
   IF (.NOT.(LUMPED_COMPONENT_ONLY .OR. BACKGROUND .OR. COPY_LUMPED .OR. PRIMITIVE)) THEN
      IF (SPEC_ID(1)/='null') THEN
         IF (N_PRODUCTS_COMP==0) CALL EXTRACT_PRODUCTS_COMPONENTS(SPEC_ID,VOLUME_FRACTION,MASS_FRACTION_FDS6)
      ELSEIF (MW>0._EB .AND. N_FUEL_SPEC<MAX_FUEL_SPEC) THEN
         N_FUEL_SPEC = N_FUEL_SPEC + 1
         FUEL_SPEC_ID(N_FUEL_SPEC)  = ID
         FUEL_SPEC_MW(N_FUEL_SPEC)  = MW
         FUEL_SPEC_CL(N_FUEL_SPEC)  = CL
      ENDIF
   ENDIF
ENDDO PRE_LOOP
REWIND(LU_INPUT)

END SUBROUTINE REAC_PRESCAN_FENIX


SUBROUTINE READ_REAC

CHARACTER(30) :: FUEL,OXIDIZER
CHARACTER(60) :: SPEC_ID_NU(MAX_SPECIES)
LOGICAL :: IDEAL
INTEGER :: NN,N_Reac_Read,I
REAL(EB) :: Y_F_INLET, &
            ...
            FUEL_HEAT_OF_FORMATION,AIT,MW_FUEL, &
            COMBUSTION_COMPLETENESS,CHI_R_REAC,RADIATIVE_FRACTION,HOC_COMPLETE, &
            FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,FUEL_N_TO_HCN_FRACTION, &
            LOWER_OXYGEN_LIMIT,HCL_YIELD,HCN_YIELD,CL=0._EB, &
            C_TMP,H_TMP,O_TMP,MW_FUEL_TMP, &
            NU_F_FENIX=0._EB,NU_AIR_FENIX=0._EB,R_PROD_FENIX=0._EB,MW_FUEL_TMP_SAVE=0._EB, &
            HCL_YIELD_FENIX=0._EB,Y_O2_FENIX=0._EB,MASS_IN_FENIX=0._EB,MASS_OUT_FENIX=0._EB
NAMELIST /REAC/ E,BOF,HEAT_OF_COMBUSTION,FYI,FUEL,OXIDIZER,EPUMO2,ID, N_S,&
                ...
                SPEC_ID_NU,COMBUSTION_COMPLETENESS,CHI_R_REAC,RADIATIVE_FRACTION,HOC_COMPLETE, &
                FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,FUEL_N_TO_HCN_FRACTION, &
                LOWER_OXYGEN_LIMIT,HCL_YIELD,HCN_YIELD,CL
 
N_REACTIONS = 0
REWIND(LU_INPUT)

! R28j: &SPEC читается ПОСЛЕ &REAC (порядок FDS5) — извлекаем PRODUCTS/AIR/топливо
! предварительным сканом, чтобы выходы методики были известны при построении state-системы
CALL REAC_PRESCAN_FENIX
...
COUNT_REAC_LOOP: DO
   ...
   ! Check if this is FDS6-style reaction (SPEC_ID_NU specified)
   IF (SPEC_ID_NU(1) /= 'null') THEN
      ! FDS6-style: SPEC_ID_NU(1:3) = 'SPEC_1','AIR','PRODUCTS', NU(1:3) = -1,-18.44,1
      ! Convert to FDS5 Mixture Fraction format
      
      ! Extract fuel name from SPEC_ID_NU(1)
      FUEL = SPEC_ID_NU(1)
      
      ! Find MW_FUEL from FUEL_SPEC_MW array (populated in READ_SPEC)
      MW_FUEL_TMP = 0._EB
      DO I = 1, N_FUEL_SPEC
         IF (TRIM(FUEL_SPEC_ID(I)) == TRIM(FUEL)) THEN
            MW_FUEL_TMP = FUEL_SPEC_MW(I)
            EXIT
         ENDIF
      ENDDO
      
      IF (MW_FUEL_TMP <= 0._EB) THEN
         ! Fallback: search in regular SPECIES array (FDS5-style species with MW)
         DO I = 1, N_SPECIES
            IF (TRIM(SPECIES(I)%ID) == TRIM(FUEL)) THEN
               MW_FUEL_TMP = SPECIES(I)%MW
               EXIT
            ENDIF
         ENDDO
      ENDIF
...
IF (MW_FUEL_TMP <= 0._EB) THEN
          ! Last resort fallback: estimate MW from FUEL name
          WRITE(MESSAGE,'(A,A,A)') 'WARNING: Fuel species ', TRIM(FUEL), ' MW not found in &SPEC, using default C=3,H=8'
          MW_FUEL_TMP = 44._EB  ! Default propane-like
          C_TMP = 3._EB
          H_TMP = 8._EB
          O_TMP = 0._EB
       ELSE
          ! Try to find Cl content from FUEL_SPEC_CL
          DO I = 1, N_FUEL_SPEC
             IF (TRIM(FUEL_SPEC_ID(I)) == TRIM(FUEL)) THEN
                CL = FUEL_SPEC_CL(I)
                EXIT
             ENDIF
          ENDDO
          ! Estimate C, H, O from MW_FUEL (heuristic for hydrocarbons)
          ! For MW=87.15 (furniture): C=3.06, H=8.23, O=2.60
          ! Use empirical formula: C = MW / 28.5
          C_TMP = MW_FUEL_TMP / 28.5_EB
          H_TMP = C_TMP * 2.7_EB  ! H/C ratio ~2.7 for hydrocarbons
          O_TMP = (MW_FUEL_TMP - C_TMP*MW_C - H_TMP*MW_H - CL*MW_HCL) / MW_O
          IF (O_TMP < 0._EB) O_TMP = 0._EB
       ENDIF
       ...
       O = O_TMP
       N = 0._EB
       OTHER = CL  ! Cl content maps to OTHER for HCl production (via MF_MW = MW_HCl)
      
      ! Store COMBUSTION_COMPLETENESS for later use
      IF (COMBUSTION_COMPLETENESS >= 0._EB .AND. COMBUSTION_COMPLETENESS <= 1._EB) THEN
         CCC_FIXED_GLOBAL = COMBUSTION_COMPLETENESS
      ELSEIF (CCC_DEFAULT) THEN
         CCC_FIXED_GLOBAL = 0.93_EB
      ENDIF
      
      ! This is Mixture Fraction reaction
...
! R28j (Fenix+ / SPEC_ID_NU): выходы продуктов из строки &SPEC ID='PRODUCTS'.
! Y_i = x_i·MW_i/(|NU_1|·MW_fuel) [кг/кг топлива]; заменяет дефолты FDS5 (SOOT_YIELD=0.01,
! пропан-формула C=3/H=8), которые не отражают методику владельца.
IF (SPEC_ID_NU(1)/='null' .AND. N_PRODUCTS_COMP>0) THEN
   MW_FUEL_TMP = 0._EB
   DO I = 1, N_FUEL_SPEC
      IF (TRIM(FUEL_SPEC_ID(I))==TRIM(SPEC_ID_NU(1))) MW_FUEL_TMP = FUEL_SPEC_MW(I)
   ENDDO
   IF (MW_FUEL_TMP<=0._EB) THEN
      DO I = 1, N_SPECIES
         IF (TRIM(SPECIES(I)%ID)==TRIM(SPEC_ID_NU(1))) MW_FUEL_TMP = SPECIES(I)%MW
      ENDDO
   ENDIF
   IF (MW_FUEL_TMP>0._EB .AND. PRODUCTS_MW_SUM>0._EB .AND. ABS(NU(1))>0._EB) THEN
      SPEC_ID_NU_ACTIVE = .TRUE.
      MW_FUEL_TMP_SAVE  = MW_FUEL_TMP
      NU_F_FENIX        = ABS(NU(1))
      NU_AIR_FENIX      = ABS(NU(2))
      R_PROD_FENIX      = PRODUCTS_MW_SUM/(NU_F_FENIX*MW_FUEL_TMP)   ! кг продуктов / кг топлива
      DO I = 1, N_PRODUCTS_COMP
         PRODUCTS_COMP_YIELD(I) = PRODUCTS_COMP_MASS_FRAC(I)*R_PROD_FENIX
         IF (TRIM(PRODUCTS_COMP_ID(I))=='SOOT')              SOOT_YIELD = PRODUCTS_COMP_YIELD(I)
         IF (TRIM(PRODUCTS_COMP_ID(I))=='CARBON MONOXIDE')   CO_YIELD   = PRODUCTS_COMP_YIELD(I)
         IF (TRIM(PRODUCTS_COMP_ID(I))=='HYDROGEN CHLORIDE') THEN
            HCL_YIELD_FENIX = PRODUCTS_COMP_YIELD(I)
            OTHER = MAX(OTHER, 1.E-6_EB)   ! резерв состояния HCl; точное значение — после MW_FUEL
         ENDIF
      ENDDO
      SOOT_H_FRACTION = 0._EB              ! сажа методики — углерод
   ENDIF
ENDIF
...
SET_MIXTURE_FRACTION: IF (MIXTURE_FRACTION) THEN
   ...
   ! FDS6: условия подсчёта синхронизированы с условиями заполнения ниже
   ! (SOOT: SOOT_DEPOSITION.OR.SOOT_YIELD; HCl: OTHER.OR.CL)
   IF (SOOT_DEPOSITION .OR. SOOT_YIELD > 0._EB) N_STATE_SPECIES = N_STATE_SPECIES + 1
   IF (H2_YIELD > 0._EB) N_STATE_SPECIES = N_STATE_SPECIES + 1
   IF (OTHER > 0._EB .OR. CL > 0._EB)  N_STATE_SPECIES = N_STATE_SPECIES + 1
   ...
   CALL ChkMemErr('READ','MF_MW',IZERO)
    ...
    ! R28j: точный HCl-выход через OTHER-канал (Y_HCl = OTHER·MW_HCl/MW_FUEL)
    IF (SPEC_ID_NU_ACTIVE .AND. HCL_YIELD_FENIX>0._EB) OTHER = HCL_YIELD_FENIX*MW_FUEL/MW_HCl
    MF_SPEC_ID(1) = 'FUEL'
   MF_SPEC_FORMULA(1) = 'F'
   MF_MW(1) = MW_FUEL
   FUEL_INDEX = 1
...
IF (H2_YIELD > 0._EB)THEN
      ...
      H2_INDEX = NN
ENDIF     
    IF (OTHER > 0._EB .OR. CL > 0._EB) THEN
       NN = NN + 1
       MF_SPEC_ID(NN) = 'HYDROGEN CHLORIDE'
       MF_SPEC_FORMULA(NN) = 'HCl'
       MF_MW(NN) = MW_HCl
       OTHER_INDEX = NN
       IF (OTHER <= 0._EB .AND. CL > 0._EB) OTHER = CL
    ENDIF
...
REACTION%MW_OTHER        = MW_OTHER
    IF (OTHER_INDEX > 0 .AND. OTHER_INDEX <= N_STATE_SPECIES) &
       REACTION%MW_OTHER = MF_MW(OTHER_INDEX)  ! Override with actual MW (MW_HCl for hydrogen chloride)
...
! R28j: O2-стехиометрия из люмп-AIR (Y_O2 = |NU_2|·x_O2·MW_O2/(|NU_1|·MW_fuel));
! замыкание массы продуктов через H2O; нормировка трассера для выводимых компонентов.
IF (SPEC_ID_NU_ACTIVE .AND. AIR_LUMP_XMW_SUM>0._EB .AND. MW_FUEL_TMP_SAVE>0._EB) THEN
   Y_O2_FENIX = NU_AIR_FENIX*AIR_LUMP_X_O2*MW_O2/(NU_F_FENIX*MW_FUEL_TMP_SAVE)
   REACTION(1)%NU(O2_INDEX) = Y_O2_FENIX*REACTION(1)%MW_FUEL/MW_O2
   REACTION(1)%O2_F_RATIO   = Y_O2_FENIX
   IF (CO_PRODUCTION .AND. N_REACTIONS>=2) THEN
      REACTION(2)%NU(O2_INDEX) = REACTION(1)%NU(O2_INDEX)
      REACTION(2)%O2_F_RATIO   = Y_O2_FENIX
   ENDIF
   MASS_IN_FENIX  = REACTION(1)%MW_FUEL + REACTION(1)%NU(O2_INDEX)*MW_O2
   MASS_OUT_FENIX = 0._EB
   DO NN = 4, N_STATE_SPECIES
      IF (H2_INDEX>0 .AND. NN==H2_INDEX) CYCLE
      IF (H2O_INDEX>0 .AND. NN==H2O_INDEX) CYCLE   ! H2O — балансирующий член
      MASS_OUT_FENIX = MASS_OUT_FENIX + REACTION(1)%NU(NN)*MF_MW(NN)
   ENDDO
   IF (H2O_INDEX>0 .AND. MASS_IN_FENIX>MASS_OUT_FENIX) &
      REACTION(1)%NU(H2O_INDEX) = REACTION(1)%NU(H2O_INDEX) + (MASS_IN_FENIX-MASS_OUT_FENIX)/MW_H2O
   PRODUCTS_TRACER_RATIO = MASS_IN_FENIX/REACTION(1)%MW_FUEL
   WRITE(LU_OUTPUT,'(A)') ' R28j: SPEC_ID_NU — выходы продуктов из PRODUCTS (кг/кг топлива):'
   DO NN = 1, N_PRODUCTS_COMP
      WRITE(LU_OUTPUT,'(A,A,A,ES10.3)') '   ',TRIM(PRODUCTS_COMP_ID(NN)),': ',PRODUCTS_COMP_YIELD(NN)
   ENDDO
   WRITE(LU_OUTPUT,'(A,ES10.3,A,ES10.3,A,ES10.3)') ' R28j: O2_F_RATIO = ',Y_O2_FENIX, &
      ' кг/кг; трассер продуктов = ',PRODUCTS_TRACER_RATIO,' кг/кг; SOOT_YIELD = ',REACTION(1)%SOOT_YIELD
ENDIF
...
FLAME_SHEET_QCAP = FLAME_SHEET_QCAP*1000._EB  ! R26: кВт/м3 -> Вт/м3
...
H2_YIELD                    = 0._EB
HEAT_OF_COMBUSTION          = -1._EB
IF (LES .AND. HRRPUA_SHEET<0._EB) HRRPUA_SHEET       = 0._EB
IF (DNS .AND. HRRPUA_SHEET<0._EB) HRRPUA_SHEET       = 200._EB
...
O                           = 0._EB
 N                           = 0._EB
 OTHER                       = 0._EB
 CL                          = 0._EB   ! FDS6: без инициализации мусор из стэка включает HCl-ветку (OOB при N_STATE_SPECIES=6)
 MW_OTHER                    = 28._EB        ! Nitrogen MW
 MW_HCL                      = 36.46_EB        ! MW of HCl, set when HCl produced
 MASS_EXTINCTION_COEFFICIENT = 8700._EB     ! m2/kg
 ! FDS6: значения из &MISC не перезаписываются (сентинел -1 задаётся в READ_MISC)
 IF (MAXIMUM_VISIBILITY < 0._EB) MAXIMUM_VISIBILITY = 30._EB   ! m
 IF (VISIBILITY_FACTOR  < 0._EB) VISIBILITY_FACTOR  = 3._EB
...
IF (MIXTURE_FRACTION) THEN
   ...
   YYMAX(I_PROG_F) = REACTION(1)%Z_F
    ...
    IF (SOOT_DEPOSITION .OR. REACTION(1)%SOOT_YIELD > 0._EB) THEN
       YYMAX(I_PROG_F)    = REACTION(1)%Z_F * (1._EB - REACTION(1)%SOOT_YIELD)
       ! Сажа — свободный вид: стехиометрический кап уничтожает дым в CHECK_MASS_FRACTION (mass.f90:1423)
       YYMAX(I_PROG_SOOT) = 0.1_EB
    ENDIF
...
IF (MIXTURE_FRACTION) THEN
   ...
   ENDIF
    IF (SOOT_DEPOSITION .OR. REACTION(1)%SOOT_YIELD > 0._EB) FUEL_FRAC = 1._EB/(1._EB - REACTION(1)%SOOT_YIELD)
...
IF (OTHER_INDEX > 0 .AND. OTHER_INDEX <=N_STATE_SPECIES) &
          Y2Y(OTHER_INDEX,I_Z_MIN+N) = REACTION(N)%Y_F_INLET / REACTION(N)%MW_FUEL * MF_MW(OTHER_INDEX) * REACTION(N)%NU(OTHER_INDEX)*FUEL_FRAC
   ENDDO    
   IF (CO_PRODUCTION) THEN
...
ELSE
       ...
       IF (SOOT_INDEX > 0 .AND. SOOT_INDEX <=N_STATE_SPECIES) &
          Y2Y(SOOT_INDEX,I_Z_MIN+1) = REACTION(1)%Y_F_INLET / REACTION(1)%MW_FUEL * MW_SOOT * REACTION(1)%NU(SOOT_INDEX)
    ENDIF
    IF (SOOT_DEPOSITION .OR. REACTION(1)%SOOT_YIELD > 0._EB) THEN
...
IF (MIXTURE_FRACTION) THEN
   ...
   DO N=1,N_STATE_SPECIES
      ...
      IF (SOOT_DEPOSITION .OR. REACTION(1)%SOOT_YIELD > 0._EB) &
...
IF(MIXTURE_FRACTION) THEN
   ...
   IF (OTHER_INDEX > 0 .AND. OTHER_INDEX <=N_STATE_SPECIES) &
       STATE_SPECIES(OTHER_INDEX) = 'HYDROGEN CHLORIDE'
ENDIF
...
READ_PROP_LOOP: DO N=0,N_PROP
   ...
   IF (PART_ID /='null' .AND. PDPA_RADIUS == 0.0_EB) THEN
      ...
      IF (DROPLET_VELOCITY==0._EB .AND. ORIFICE_DIAMETER==0. .AND. PRESSURE_RAMP=='null' .AND. SPRAY_PATTERN_TABLE=='null') THEN
         ...
         IF (MYID==0) WRITE(LU_OUTPUT,'(A)') TRIM(MESSAGE)
      ENDIF
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   IF (SPECIFIC_HEAT == 0._EB .AND. SPECIFIC_HEAT_RAMP == 'null' ) THEN
      ...
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (SPECIFIC_HEAT > 10._EB) WRITE(LU_OUTPUT,'(A,A)') 'WARNING: SPECIFIC_HEAT units are kJ/kg/K check MATL ',TRIM(ID)
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   IF (ML%RAMP_C_S/='null') THEN
         IF (RAMPS(-INT(ML%C_S))%DEPENDENT_DATA(1) > 10._EB) &
            WRITE(LU_OUTPUT,'(A,A)') 'WARNING: SPECIFIC_HEAT units are kJ/kg/K check MATL ',TRIM(ID)
   ENDIF   
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
  ...
  ! Determine if the surface is combustible/burning
   ...
   BURNING_IF: IF (BURNING) THEN
      ...
      ! MLR_BASED_HRR: calculate MLRPUA from HRRPUA (FDS6 compatibility, always active)
      IF (SF%HRRPUA>0._EB) THEN
         SF%MLR_BASED_HRR = .TRUE.
         IF (CCC_FIXED_GLOBAL >= 0._EB .AND. CCC_FIXED_GLOBAL <= 1._EB) THEN
            ! Fixed CCC: MLRPUA = HRRPUA / (HoC * CCC)
            SF%MLRPUA = SF%HRRPUA / (RN%HEAT_OF_COMBUSTION * CCC_FIXED_GLOBAL)
            WRITE(LU_OUTPUT,'(A,A,A,ES10.3,A,ES10.3)') ' MLR_BASED_HRR: Surface ', TRIM(SF%ID), &
               ' MLRPUA = ', SF%MLRPUA, ' kg/s/m2, CCC = ', CCC_FIXED_GLOBAL
         ELSE
            ! Procedural CCC: MLRPUA = HRRPUA / HoC (CCC applied dynamically in fire.f90)
            SF%MLRPUA = SF%HRRPUA / RN%HEAT_OF_COMBUSTION
            WRITE(LU_OUTPUT,'(A,A,A,ES10.3,A)') ' MLR_BASED_HRR: Surface ', TRIM(SF%ID), &
               ' MLRPUA = ', SF%MLRPUA, ' kg/s/m2, CCC = procedural(T,O2)'
         ENDIF
         SF%MASS_FLUX(I_FUEL) = SF%MLRPUA
      ENDIF
      ! Adjust burning rate according to the difference of heats of combustion
      ! Skip for MLR_BASED_HRR — MLR is already the final value
      IF (SF%N_LAYERS > 0 .AND. .NOT. SF%MLR_BASED_HRR) THEN
      ...
      ENDIF
       ...
       ! BUG#11: максимальная поверхностная тепловая мощность прописанного очага (W/m2)
       IF (N_REACTIONS>0) HRRPUA_MAX = MAX(HRRPUA_MAX, SF%HRRPUA, SF%MLRPUA*REACTION(1)%HEAT_OF_COMBUSTION)
    ENDIF BURNING_IF
...
! q-
 ...
 ! BUG#11 + R20 (порт FDS6 EDC): при прописанном очаге (HRRPUA/MLRPUA):
 ! (1) лист-модель 200 кВт/м2 (Q_UPPER >= 200000/CELL_SIZE + 2500 кВт/м3 фон;
 !     фон 2500 кВт/м3 — дефолт апстрима FDS5, Issue 1048, см. SET_REAC_DEFAULTS) —
 !     компактное горячее ядро у поверхности, конвективная колонка, динамичный дым;
 ! (2) глобальный губернатор HRR (main.f90): HRR <= 1.15*MLR_глоб*Hc, рост <= 1x/с;
 ! (3) EDC-дроссель unmixedness в fire.f90 (ZETA_STORE) — рост горения в ячейке
 !     ограничен скоростью смешения, детонация/фантомное Q невозможны.
 IF (HRRPUA_MAX>0._EB) THEN
    ! дефолт FDS6 = 200 кВт/м2; пользователь может задать меньше через &MISC
    ! HRRPUA_SHEET (кВт/м2) — смягчает концентрацию тепла над горелкой,
    ! снижает DP/струю в ячейках пламени -> крупнее DT в разгонной фазе
    IF (HRRPUA_SHEET<=0._EB) HRRPUA_SHEET = 200000._EB
    GOVERN_HRR = .TRUE.
    GOV_HOC    = REACTION(1)%HEAT_OF_COMBUSTION
 ENDIF
...
NAMELIST /PRES/ VELOCITY_TOLERANCE,MAX_PRESSURE_ITERATIONS,PRESSIT_RELAX_FACTOR,PRESSIT_ACCELERATOR,PRESSIT_SCALE_FACTOR, &
                PREDICTOR_RELAX_FACTOR,CORRECTOR_RELAX_FACTOR,OLD_PRESSURE_SCHEME,PRES_METHOD, &
                PRESSURE_BLEND_ADAPTIVE,PRESSURE_BLEND_ALPHA_MIN,PRESSURE_BLEND_H_THRESHOLD,PRESSURE_BLEND_FACTOR, &
                PRESSURE_FLOOR_ENABLED,PRESSURE_FLOOR_FACTOR, &
                SOAP_THRESHOLD_SAVE,SOAP_EXTRAP_ALPHA_SAVE,SOAP_DIV_CHECK_TOLERANCE,SOAP_DIV_CHECK, &
...
! Validate PRES_METHOD
SELECT CASE(PRES_METHOD)
   CASE('FFT','SCARC','SOAP')
      ! Valid
   CASE DEFAULT
      CALL SHUTDOWN('ERROR: PRES_METHOD must be FFT, SCARC, or SOAP')
END SELECT

! Validate pressure blending parameters
IF (PRESSURE_BLEND_FACTOR < 0.0_EB .OR. PRESSURE_BLEND_FACTOR > 1.0_EB) THEN
   CALL SHUTDOWN('ERROR: PRESSURE_BLEND_FACTOR must be between 0.0 and 1.0')
ENDIF
IF (PRESSURE_BLEND_ALPHA_MIN < 0.0_EB .OR. PRESSURE_BLEND_ALPHA_MIN > 1.0_EB) THEN
   CALL SHUTDOWN('ERROR: PRESSURE_BLEND_ALPHA_MIN must be between 0.0 and 1.0')
ENDIF
IF (PRESSURE_BLEND_H_THRESHOLD <= 0.0_EB .OR. PRESSURE_BLEND_H_THRESHOLD > 1.0_EB) THEN
   CALL SHUTDOWN('ERROR: PRESSURE_BLEND_H_THRESHOLD must be between 0.0 (exclusive) and 1.0')
ENDIF

! Validate pressure floor parameters
IF (PRESSURE_FLOOR_FACTOR < 0.0_EB .OR. PRESSURE_FLOOR_FACTOR > 1.0_EB) THEN
   CALL SHUTDOWN('ERROR: PRESSURE_FLOOR_FACTOR must be between 0.0 and 1.0')
ENDIF
...
IF (LES) RADIATIVE_FRACTION = 0.45_EB
...
USE GEOMETRY_FUNCTIONS, ONLY: BLOCK_CELL,SEARCH_OTHER_MESHES
USE DEVICE_VARIABLES, ONLY : DEVICE, N_DEVC
USE CONTROL_VARIABLES, ONLY : CONTROL, N_CTRL 
TYPE(OBSTRUCTION_TYPE), POINTER :: OB2,OBT
TYPE(OBSTRUCTION_TYPE), POINTER :: OB_N,OB_M
TYPE(MULTIPLIER_TYPE), POINTER :: MR
TYPE(OBSTRUCTION_TYPE), DIMENSION(:), ALLOCATABLE, TARGET :: TEMP_OBSTRUCTION
INTEGER :: NM,NOM,NOM2,N_OBST_O,NNN,IC,N,NN,NNNN,N_NEW_OBST,RGB(3),N_OBST_NEW,II,JJ,KK, &
           GAP_N,GAP_M,SAVED_I2,SAVED_J2,SAVED_K2,OV_N,OV_M,IIO,JJO,KKO, &
           VF_ILO,VF_IHI,VF_JLO,VF_JHI,VF_KLO,VF_KHI
...
REAL(EB) :: TRANSPARENCY,XB1,XB2,XB3,XB4,XB5,XB6,BULK_DENSITY,VOL_ADJUSTED,VOL_SPECIFIED, &
            I_EXACT_LO,I_EXACT_HI,J_EXACT_LO,J_EXACT_HI,K_EXACT_LO,K_EXACT_HI,SNAP_EXTENT, &
            ORIG_GAP,SNAPPED_GAP,DX_GHOST,DY_GHOST,DZ_GHOST, &
            VF_OBX1,VF_OBX2,VF_OBY1,VF_OBY2,VF_OBZ1,VF_OBZ2, &
            VF_IX1,VF_IX2,VF_IY1,VF_IY2,VF_IZ1,VF_IZ2,VF_OBST,VF_CELL,VF_NEW
LOGICAL :: SAWTOOTH,EMBEDDED,THICKEN,THICKEN_DIR(3),PERMIT_HOLE,ALLOW_VENT,EVACUATION, REMOVABLE,BNDF_FACE(-3:3),BNDF_OBST,OUTLINE,REJECT_OBST,CUT_FACE
NAMELIST /OBST/ XB,ID,PROP_ID,SURF_ID,SURF_IDS,SURF_ID6,FYI,BNDF_FACE,BNDF_OBST, &
                ...
                PERMIT_HOLE,ALLOW_VENT,EVACUATION,MESH_ID,REMOVABLE,MULT_ID,BULK_DENSITY,CUT_FACE
      ...
      K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               ! Mesh-proximity filter (from FDS6): skip OBSTs that are more than one cell away from this mesh.
               ! Prevents coarse meshes from processing OBSTs that belong to neighboring fine meshes.
               IF (XB1 > XF+DX(IBP1) .OR. XB2 < XS-DX(0) .OR. &
                   XB3 > YF+DY(JBP1) .OR. XB4 < YS-DY(0) .OR. &
                   XB5 > ZF+DZ(KBP1) .OR. XB6 < ZS-DZ(0)) THEN
                  N = N-1
                  N_OBST = N_OBST-1
                  CYCLE I_MULT_LOOP
               ENDIF

               ! Determine which directions allow thickening (don't thicken if OBST straddles mesh boundary)
               THICKEN_DIR = THICKEN
               ...
               ! Look for obstructions within half a grid cell of the current mesh.
               ...
               IF (XB2>=XS-0.5_EB*DX(0) .AND. XB2<XS) THEN
                  ...
                  IF (NOM>0 .AND. ASSOCIATED(MESHES(NOM)%DX)) DX_GHOST = MESHES(NOM)%DX(IIO)
               ...
               IF (XB4>=YS-0.5_EB*DY(0) .AND. XB4<YS) THEN
                  ...
                  CALL SEARCH_OTHER_MESHES(0.5_EB*(MAX(XS,XB1)+MIN(XF,XB2)),YS-0.51_EB*DY(0),0.5_EB*(MAX(ZS,XB5)+MIN(ZF,XB6)),&
                                           NOM2,IIO,JJO,KKO)
                  IF (NOM==0 .AND. NOM2>0) REJECT_OBST = .TRUE.
                  IF (NOM>0 .AND. ASSOCIATED(MESHES(NOM)%DY)) DY_GHOST = MESHES(NOM)%DY(JJO)
               ...
               IF (XB6>=ZS-0.5_EB*DZ(0) .AND. XB6<ZS .AND. .NOT.EVACUATION_ONLY(NM)) THEN
                  ...
                  CALL SEARCH_OTHER_MESHES(0.5_EB*(MAX(XS,XB1)+MIN(XF,XB2)),0.5_EB*(MAX(YS,XB3)+MIN(YF,XB4)),ZS-0.51_EB*DZ(0),&
                                           NOM2,IIO,JJO,KKO)
                  IF (NOM==0 .AND. NOM2>0) REJECT_OBST = .TRUE.
                  IF (NOM>0 .AND. ASSOCIATED(MESHES(NOM)%DZ)) DZ_GHOST = MESHES(NOM)%DZ(KKO)
               ...
               ! Clip OBST coordinates to computational domain (MUST be done BEFORE reject check).
               ! This allows zero-thickness walls at mesh boundaries to be processed correctly.
               XB1 = MAX(XB1,XS)
               XB2 = MIN(XB2,XF)
               XB3 = MAX(XB3,YS)
               XB4 = MIN(XB4,YF)
               ...
               ! Reject OBST entirely outside this mesh (after clipping).
               ! An OBST is rejected only if it lies completely outside the mesh domain.
               IF (XB1>XF .OR. XB2<XS .OR. &
                   XB3>YF .OR. XB4<YS .OR. &
                   XB5>ZF .OR. XB6<ZS .OR. REJECT_OBST) THEN
                  N = N-1
                  N_OBST = N_OBST-1
                  CYCLE I_MULT_LOOP
               ENDIF
               ...
               OB%Z1 = XB5
               OB%Z2 = XB6
               OB%CUT_FACE = CUT_FACE
               ! Save clipped XB for cut-face geometry computation
               OB%XB_INPUT = (/XB1,XB2,XB3,XB4,XB5,XB6/)
               ...
               ! Detect sub-cell thin obstructions: thickness < 1 cell in any direction
               ! These will NOT be thickened; handled via cut-face blending (ALPHA_CF_W)
               OB%SUB_CELL_THIN = .FALSE.
               OB%THIN_FRACTION = 1._EB
               IF (OB%I1==OB%I2 .AND. XB2>XB1) THEN
                  OB%SUB_CELL_THIN = .TRUE.
                  OB%THIN_FRACTION(1) = GINV(XB2-XS,1,NM)*RDXI - GINV(XB1-XS,1,NM)*RDXI
                  OB%THIN_FRACTION(1) = MAX(0.01_EB, MIN(1._EB, OB%THIN_FRACTION(1)))
               ENDIF
               IF (OB%J1==OB%J2 .AND. XB4>XB3) THEN
                  OB%SUB_CELL_THIN = .TRUE.
                  OB%THIN_FRACTION(2) = GINV(XB4-YS,2,NM)*RDETA - GINV(XB3-YS,2,NM)*RDETA
                  OB%THIN_FRACTION(2) = MAX(0.01_EB, MIN(1._EB, OB%THIN_FRACTION(2)))
               ENDIF
               IF (OB%K1==OB%K2 .AND. XB6>XB5) THEN
                  OB%SUB_CELL_THIN = .TRUE.
                  OB%THIN_FRACTION(3) = GINV(XB6-ZS,3,NM)*RDZETA - GINV(XB5-ZS,3,NM)*RDZETA
                  OB%THIN_FRACTION(3) = MAX(0.01_EB, MIN(1._EB, OB%THIN_FRACTION(3)))
               ENDIF

               ! If desired, thicken small obstructions (skip sub-cell thin and cut-face ones);
               ! otherwise collapse thin ones (FDS6 logic)

               IF (THICKEN_DIR(1) .AND. .NOT.OB%SUB_CELL_THIN .AND. .NOT.OB%CUT_FACE) THEN
                  IF (OB%I1==OB%I2) THEN
                     ...
                     OB%X2 = X(OB%I2)
                  ENDIF
               ELSEIF (.NOT.THICKEN_DIR(1) .AND. .NOT.OB%CUT_FACE) THEN
               ...
               IF (THICKEN_DIR(2) .AND. .NOT.OB%SUB_CELL_THIN .AND. .NOT.OB%CUT_FACE) THEN
                  IF (OB%J1==OB%J2) THEN
                     ...
                     OB%Y2 = Y(OB%J2)
                  ENDIF
               ELSEIF (.NOT.THICKEN_DIR(2) .AND. .NOT.OB%CUT_FACE) THEN
                  IF (GINV(XB4-YS,2,NM)-GINV(XB3-YS,2,NM)<0.25_EB/RDETA .AND. OB%J1/=OB%J2) THEN
                     IF (GINV(XB3-YS,2,NM)-REAL(OB%J1,EB) < REAL(OB%J2,EB)-GINV(XB4-YS,2,NM)) THEN
                        OB%J2 = OB%J1
                     ELSE
                        OB%J1 = OB%J2
                     ENDIF
               ...
               IF (THICKEN_DIR(3) .AND. .NOT.OB%SUB_CELL_THIN .AND. .NOT.OB%CUT_FACE) THEN
                  IF (OB%K1==OB%K2) THEN
                     ...
                     OB%Z2 = Z(OB%K2)
                  ENDIF
               ELSEIF (.NOT.THICKEN_DIR(3) .AND. .NOT.OB%CUT_FACE) THEN
                  IF (GINV(XB6-ZS,3,NM)-GINV(XB5-ZS,3,NM)<0.25_EB/RDZETA .AND. OB%K1/=OB%K2) THEN
                     IF (GINV(XB5-ZS,3,NM)-REAL(OB%K1,EB) < REAL(OB%K2,EB)-GINV(XB6-ZS,3,NM)) THEN
                        OB%K2 = OB%K1
                     ELSE
                        OB%K1 = OB%K2
                     ENDIF
               ...
               ! Compute fractional face areas for sub-cell OBST accuracy
               OB%FACE_AREA_FRACTION = 1._EB
               IF (OB%I2 > OB%I1 .AND. XB2 > XB1) THEN
                  I_EXACT_LO = GINV(XB1-XS,1,NM)*RDXI
                  I_EXACT_HI = GINV(XB2-XS,1,NM)*RDXI
                  SNAP_EXTENT = REAL(OB%I2-OB%I1,EB)
                  IF (SNAP_EXTENT > 0._EB) OB%FACE_AREA_FRACTION(1) = &
                     MAX(0._EB,MIN(1._EB,(I_EXACT_HI-I_EXACT_LO)/SNAP_EXTENT))
               ENDIF
               IF (OB%J2 > OB%J1 .AND. XB4 > XB3) THEN
                  J_EXACT_LO = GINV(XB3-YS,2,NM)*RDETA
                  J_EXACT_HI = GINV(XB4-YS,2,NM)*RDETA
                  SNAP_EXTENT = REAL(OB%J2-OB%J1,EB)
                  IF (SNAP_EXTENT > 0._EB) OB%FACE_AREA_FRACTION(2) = &
                     MAX(0._EB,MIN(1._EB,(J_EXACT_HI-J_EXACT_LO)/SNAP_EXTENT))
               ENDIF
               IF (OB%K2 > OB%K1 .AND. XB6 > XB5) THEN
                  K_EXACT_LO = GINV(XB5-ZS,3,NM)*RDZETA
                  K_EXACT_HI = GINV(XB6-ZS,3,NM)*RDZETA
                  SNAP_EXTENT = REAL(OB%K2-OB%K1,EB)
                  IF (SNAP_EXTENT > 0._EB) OB%FACE_AREA_FRACTION(3) = &
                     MAX(0._EB,MIN(1._EB,(K_EXACT_HI-K_EXACT_LO)/SNAP_EXTENT))
               ENDIF
...
MESH_LOOP_3: DO NM=1,NMESHES
   ...
   ALLOCATE(M%VOLUME_FACTOR(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('READ','VOLUME_FACTOR',IZERO)
   M%VOLUME_FACTOR = 1._EB
   ...
   ! Compute VOLUME_FACTOR for all gas cells near CUT_FACE OBSTs (Phase 3)
   ! This covers cells partially inside OBSTs, including cells not adjacent to wall faces
   ! and cells intersected by multiple OBSTs
   DO N=1,N_OBST
      OB=>OBSTRUCTION(N)
      IF (.NOT.OB%CUT_FACE) CYCLE

      ! OBST physical coordinates (original, unsnapped for CUT_FACE)
      VF_OBX1 = OB%XB_INPUT(1)
      VF_OBX2 = OB%XB_INPUT(2)
      VF_OBY1 = OB%XB_INPUT(3)
      VF_OBY2 = OB%XB_INPUT(4)
      VF_OBZ1 = OB%XB_INPUT(5)
      VF_OBZ2 = OB%XB_INPUT(6)

      ! Cell range that could overlap with this OBST (snapped indices + 1 buffer)
      VF_ILO = MAX(1, OB%I1)
      VF_IHI = MIN(IBAR, OB%I2 + 1)
      VF_JLO = MAX(1, OB%J1)
      VF_JHI = MIN(JBAR, OB%J2 + 1)
      VF_KLO = MAX(1, OB%K1)
      VF_KHI = MIN(KBAR, OB%K2 + 1)
      ...
      DO K=VF_KLO,VF_KHI
         DO J=VF_JLO,VF_JHI
            DO I=VF_ILO,VF_IHI
               ...
               VF_IX1 = MAX(X(I-1), VF_OBX1)
               VF_IX2 = MIN(X(I),   VF_OBX2)
               VF_IY1 = MAX(Y(J-1), VF_OBY1)
               VF_IY2 = MIN(Y(J),   VF_OBY2)
               VF_IZ1 = MAX(Z(K-1), VF_OBZ1)
               VF_IZ2 = MIN(Z(K),   VF_OBZ2)

               IF (VF_IX2 > VF_IX1 .AND. VF_IY2 > VF_IY1 .AND. VF_IZ2 > VF_IZ1) THEN
                  VF_OBST = (VF_IX2-VF_IX1) * (VF_IY2-VF_IY1) * (VF_IZ2-VF_IZ1)
                  VF_CELL = DX(I) * DY(J) * DZ(K)
                  IF (VF_CELL > 0._EB) THEN
                     VF_NEW = MAX(0.01_EB, 1._EB - VF_OBST / VF_CELL)
                     M%VOLUME_FACTOR(I,J,K) = MIN(M%VOLUME_FACTOR(I,J,K), VF_NEW)
                  ENDIF
               ENDIF
            ENDDO
         ENDDO
...
REAL(EB) :: I_EXACT_LO,I_EXACT_HI,J_EXACT_LO,J_EXACT_HI,K_EXACT_LO,K_EXACT_HI,FRAC_LO,FRAC_HI
...
!  IF (PROCESS(NM)/=MYID) CYCLE MESH_LOOP_1
   ...
   READ_VENT_LOOP: DO NN=1,NVO
      ...
      ! Anti-collapse: expand zero-width VENTs that were collapsed by NINT snap
      IF (.NOT.REJECT_VENT) THEN
         IF (VT%I1==VT%I2 .AND. XB(1)/=XB(2)) THEN
            IF (VT%I1<IBAR) THEN ; VT%I2=VT%I1+1 ; ELSE ; VT%I1=VT%I2-1 ; ENDIF
         ENDIF
         IF (VT%J1==VT%J2 .AND. XB(3)/=XB(4)) THEN
            IF (VT%J1<JBAR) THEN ; VT%J2=VT%J1+1 ; ELSE ; VT%J1=VT%J2-1 ; ENDIF
         ENDIF
         IF (VT%K1==VT%K2 .AND. XB(5)/=XB(6)) THEN
            IF (VT%K1<KBAR) THEN ; VT%K2=VT%K1+1 ; ELSE ; VT%K1=VT%K2-1 ; ENDIF
         ENDIF
      ENDIF

      ! Compute fractional face areas for sub-cell VENT accuracy
      IF (.NOT.REJECT_VENT) THEN
         VT%FACE_AREA_FRACTION = 1._EB
         IF (VT%I1==VT%I2 .AND. XB(1)/=XB(2)) THEN
            VT%FACE_AREA_FRACTION(1) = MIN(1._EB, &
               ABS(GINV(XB(2)-XS,1,NM)-GINV(XB(1)-XS,1,NM))*RDXI)
         ELSE IF (VT%I2 > VT%I1) THEN
            I_EXACT_LO = GINV(XB(1)-XS,1,NM)*RDXI
            I_EXACT_HI = GINV(XB(2)-XS,1,NM)*RDXI
            FRAC_LO = I_EXACT_LO - REAL(VT%I1,EB)
            FRAC_HI = I_EXACT_HI - REAL(VT%I2,EB)
            IF (ABS(FRAC_LO) > 1.E-10_EB) VT%FACE_AREA_FRACTION(1) = MAX(0._EB,MIN(1._EB, &
               (REAL(VT%I2,EB) - I_EXACT_LO) / REAL(VT%I2-VT%I1,EB)))
            IF (VT%I2 > VT%I1+1 .AND. ABS(FRAC_HI) > 1.E-10_EB) VT%FACE_AREA_FRACTION(1) = MAX(0._EB,MIN(1._EB, &
               (I_EXACT_HI - REAL(MAX(VT%I2-1,VT%I1),EB)) / REAL(VT%I2-VT%I1,EB)))
         ENDIF
         IF (VT%J1==VT%J2 .AND. XB(3)/=XB(4)) THEN
            VT%FACE_AREA_FRACTION(2) = MIN(1._EB, &
               ABS(GINV(XB(4)-YS,2,NM)-GINV(XB(3)-YS,2,NM))*RDETA)
         ELSE IF (VT%J2 > VT%J1) THEN
            J_EXACT_LO = GINV(XB(3)-YS,2,NM)*RDETA
            J_EXACT_HI = GINV(XB(4)-YS,2,NM)*RDETA
            FRAC_LO = J_EXACT_LO - REAL(VT%J1,EB)
            FRAC_HI = J_EXACT_HI - REAL(VT%J2,EB)
            IF (ABS(FRAC_LO) > 1.E-10_EB) VT%FACE_AREA_FRACTION(2) = MAX(0._EB,MIN(1._EB, &
               (REAL(VT%J2,EB) - J_EXACT_LO) / REAL(VT%J2-VT%J1,EB)))
            IF (VT%J2 > VT%J1+1 .AND. ABS(FRAC_HI) > 1.E-10_EB) VT%FACE_AREA_FRACTION(2) = MAX(0._EB,MIN(1._EB, &
               (J_EXACT_HI - REAL(MAX(VT%J2-1,VT%J1),EB)) / REAL(VT%J2-VT%J1,EB)))
         ENDIF
         IF (VT%K1==VT%K2 .AND. XB(5)/=XB(6)) THEN
            VT%FACE_AREA_FRACTION(3) = MIN(1._EB, &
               ABS(GINV(XB(6)-ZS,3,NM)-GINV(XB(5)-ZS,3,NM))*RDZETA)
         ELSE IF (VT%K2 > VT%K1) THEN
            K_EXACT_LO = GINV(XB(5)-ZS,3,NM)*RDZETA
            K_EXACT_HI = GINV(XB(6)-ZS,3,NM)*RDZETA
            FRAC_LO = K_EXACT_LO - REAL(VT%K1,EB)
            FRAC_HI = K_EXACT_HI - REAL(VT%K2,EB)
            IF (ABS(FRAC_LO) > 1.E-10_EB) VT%FACE_AREA_FRACTION(3) = MAX(0._EB,MIN(1._EB, &
               (REAL(VT%K2,EB) - K_EXACT_LO) / REAL(VT%K2-VT%K1,EB)))
            IF (VT%K2 > VT%K1+1 .AND. ABS(FRAC_HI) > 1.E-10_EB) VT%FACE_AREA_FRACTION(3) = MAX(0._EB,MIN(1._EB, &
               (K_EXACT_HI - REAL(MAX(VT%K2-1,VT%K1),EB)) / REAL(VT%K2-VT%K1,EB)))
         ENDIF
      ENDIF
...
USE MPI   ! R28b: ALLGATHER/ALLGATHERV глобального списка затравок авто-зон


! R27: после пользовательских &ZONE выполняется авто-детект запертых объёмов
! (карманов) — COUNT_PRESSURE_POCKETS. Синтетические зоны дописываются после
! пользовательских с индексами N_ZONE_USER+1..N_ZONE, нулевыми утечками и
! одноточечным боксом-затравкой; разметку клеток выполняет штатный ZONE_LOOP
! (INITIALIZE_MESH_VARIABLES) тем же флуд-филлом, что и для явных зон, поэтому
! все массивы 0:N_ZONE (PBAR, DSUM/USUM/PSUM, утечки, диагностика) расширяются
! автоматически: N_ZONE финализируется здесь, до всех аллокаций.

INTEGER, PARAMETER :: MAX_LEAK_PATHS=200
REAL(EB) :: LEAK_AREA(0:MAX_LEAK_PATHS)
REAL(EB) :: XYZ(3)
REAL(EB), ALLOCATABLE :: XB_USER(:,:),LEAK_USER(:,:)
INTEGER, ALLOCATABLE :: AUTO_SEED(:,:),CUT_SEED(:,:)
INTEGER, ALLOCATABLE :: FS_CNT(:),FS_DSP(:),FS_BUF(:),FS_ALL(:)
INTEGER :: FS_TOT
REAL(EB), ALLOCATABLE :: CUT_LEAK(:)
INTEGER  :: N,NM,N_ZONE_USER,N_AUTO_Z,N_CUT_Z,NN
LOGICAL :: SEALED,READ_ZONE_LINES
CHARACTER(30) :: ID
NAMELIST /ZONE/ XB,LEAK_AREA,ID,XYZ
...
! R27: сначала читаем строки пользовательских зон в локальный буфер

N_ZONE_USER = N_ZONE
ALLOCATE(XB_USER(6,N_ZONE_USER),STAT=IZERO)
CALL ChkMemErr('READ','XB_USER',IZERO)
ALLOCATE(LEAK_USER(0:MAX_LEAK_PATHS,N_ZONE_USER),STAT=IZERO)
CALL ChkMemErr('READ','LEAK_USER',IZERO)

USER_ZONE_LOOP: DO N=1,N_ZONE_USER
   ...
   IF (READ_ZONE_LINES) THEN
      CALL CHECKREAD('ZONE',LU_INPUT,IOS)
      IF (IOS==1) EXIT USER_ZONE_LOOP
      READ(LU_INPUT,ZONE)
      ! FDS6 compatibility: convert XYZ to small XB around the point
   ...
   XB_USER(:,N)         = XB
   LEAK_USER(0:MAX_LEAK_PATHS,N) = LEAK_AREA(0:MAX_LEAK_PATHS)

ENDDO USER_ZONE_LOOP
REWIND(LU_INPUT)

! Make sure that there are no leak paths to undefined pressure ZONEs
! R27: проверка against пользовательскими индексами — синтетические зоны
! целями LEAK_PATH быть не могут (их нет во входном файле).

DO N=0,N_SURF
   SF => SURFACE(N)
   IF (SF%LEAK_PATH(1)>N_ZONE_USER .OR. SF%LEAK_PATH(2)>N_ZONE_USER) SF%LEAK_PATH = -1
ENDDO

! R27: авто-детект запертых карманов в остатке зоны 0
! R28e: гейт STRICT_POCKETS (.FALSE. = не зонировать карманы; диагностика
! блокировки транспорта: карман-зона запечатывает маршрут дыма через себя,
! комнаты за карманом не получают потока вовсе — кейс fd35e683).
IF (STRICT_POCKETS) CALL COUNT_PRESSURE_POCKETS(N_ZONE_USER,XB_USER,N_AUTO_Z,AUTO_SEED)

! R28b/MPI: карманы детектируются только по мешам своего ранга, а N_ZONE и
! список затравок обязаны быть идентичны на всех рангах: иначе в коллектив
! EXCHANGE_DIVERGENCE_INFO (main.f90, вход при N_ZONE>0) входят не все
! процессы — счёт останавливается на первом шаге. Глобальный список =
! конкатенация перранговых затравок в порядке номеров рангов.
IF (USE_MPI) THEN
   ALLOCATE(FS_CNT(0:NUMPROCS-1),FS_DSP(0:NUMPROCS-1))
   CALL MPI_ALLGATHER(N_AUTO_Z,1,MPI_INTEGER,FS_CNT,1,MPI_INTEGER,MPI_COMM_WORLD,IZERO)
   FS_TOT = SUM(FS_CNT)
   IF (FS_TOT>0) THEN
      FS_DSP(0) = 0
      DO N=0,NUMPROCS-2
         FS_DSP(N+1) = FS_DSP(N) + FS_CNT(N)
      ENDDO
      ALLOCATE(FS_BUF(4*N_AUTO_Z),FS_ALL(4*FS_TOT))
      FS_BUF = RESHAPE(AUTO_SEED,(/4*N_AUTO_Z/))
      CALL MPI_ALLGATHERV(FS_BUF,4*N_AUTO_Z,MPI_INTEGER,FS_ALL,4*FS_CNT,4*FS_DSP,MPI_INTEGER,MPI_COMM_WORLD,IZERO)
      DEALLOCATE(AUTO_SEED)
      ALLOCATE(AUTO_SEED(4,FS_TOT))
      AUTO_SEED = RESHAPE(FS_ALL,(/4,FS_TOT/))
      DEALLOCATE(FS_ALL,FS_BUF)
   ENDIF
   N_AUTO_Z = FS_TOT
   DEALLOCATE(FS_CNT,FS_DSP)
ENDIF

! R27b: авто-зонирование пожарных отсеков по узкому горлу (min-cut от очага,
! синтез тонких OBST + SURF c LEAK_PATH). Вызов строго ПОСЛЕ строгих карманов
! (синтез меняет связность) и ПОСЛЕ LEAK_PATH-валидации (синтетический SURF
! ссылается на индексы синтетических зон).

N_CUT_Z = 0
ALLOCATE(CUT_SEED(4,0),STAT=IZERO)
ALLOCATE(CUT_LEAK(0),STAT=IZERO)
CALL DETECT_AND_SEAL_FIRE_POCKETS(N_ZONE_USER,XB_USER,N_ZONE_USER+N_AUTO_Z,N_CUT_Z,CUT_SEED,CUT_LEAK)

N_ZONE = N_ZONE_USER + N_AUTO_Z + N_CUT_Z
N_AUTO_ZONE = N_AUTO_Z + N_CUT_Z   ! R27a: модульная копия для сводки в INITIALIZE_DIAGNOSTIC_FILE

IF (MYID==0) WRITE(LU_OUTPUT,'(A,I4,A,I4,A,I4,A,I4,A)') ' AUTO-ZONE R27: user zones ',N_ZONE_USER, &
   ', sealed pockets ',N_AUTO_Z,', fire-zone cuts ',N_CUT_Z,', total N_ZONE ',N_ZONE, &
   ' (pockets: sealed ODE; cuts: LEAK_PATH to ambient)'

! Stop the calculation if the user has declared ISOTHERMAL=.TRUE.
! R27: проверка перенесена после авто-детекта — карманы тоже запертые объёмы.
...
! If there are no ZONE lines and no auto-detected pockets, return
! FDS6: фиктивная аллокация (0:0) — main/divg/dump читают ZONE_IS_AUTO
! в условиях с .AND., который не гарантирует короткого замыкания

IF (N_ZONE==0) THEN
   ALLOCATE(ZONE_IS_AUTO(0:0),STAT=IZERO)
   CALL ChkMemErr('READ','ZONE_IS_AUTO',IZERO)
   ZONE_IS_AUTO = .FALSE.
   RETURN
ENDIF

! Allocate ZONE arrays and fill: пользовательские + синтетические R27

ALLOCATE(P_ZONE(N_ZONE),STAT=IZERO)
CALL ChkMemErr('READ','P_ZONE',IZERO)
ALLOCATE(ZONE_IS_AUTO(N_ZONE),STAT=IZERO)          ! R27j: маркер авто-зон
CALL ChkMemErr('READ','ZONE_IS_AUTO',IZERO)
ZONE_IS_AUTO = .FALSE.
IF (N_CUT_Z>0) ZONE_IS_AUTO(N_ZONE_USER+N_AUTO_Z+1:N_ZONE) = .TRUE.

ZONE_FILL_LOOP: DO N=1,N_ZONE

   ALLOCATE(P_ZONE(N)%LEAK_AREA(0:N_ZONE),STAT=IZERO)
   CALL ChkMemErr('READ','LEAK_AREA',IZERO)
   P_ZONE(N)%LEAK_AREA = 0._EB

   IF (N<=N_ZONE_USER) THEN
      WRITE(ID,'(A,I2.2)') 'ZONE_',N
      P_ZONE(N)%ID = ID
      P_ZONE(N)%LEAK_AREA(0:N_ZONE_USER) = LEAK_USER(0:N_ZONE_USER,N)
      P_ZONE(N)%X1 = XB_USER(1,N)
      P_ZONE(N)%X2 = XB_USER(2,N)
      P_ZONE(N)%Y1 = XB_USER(3,N)
      P_ZONE(N)%Y2 = XB_USER(4,N)
      P_ZONE(N)%Z1 = XB_USER(5,N)
      P_ZONE(N)%Z2 = XB_USER(6,N)
    ELSE
       NN = N - N_ZONE_USER
       IF (NN<=N_AUTO_Z) THEN
          ! строгий карман R27: затравка, утечек нет
          M => MESHES(AUTO_SEED(1,NN))
          I  = AUTO_SEED(2,NN)
          J  = AUTO_SEED(3,NN)
          K  = AUTO_SEED(4,NN)
          WRITE(ID,'(A,I2.2)') 'AUTO_',NN
          P_ZONE(N)%ID = ID
       ELSE
          ! R27b: пожарный отсек по min-cut; утечка в атмосферу = площадь среза
          NN = NN - N_AUTO_Z
          M => MESHES(CUT_SEED(1,NN))
          I  = CUT_SEED(2,NN)
          J  = CUT_SEED(3,NN)
          K  = CUT_SEED(4,NN)
          WRITE(ID,'(A,I2.2)') 'CUTZ_',NN
          P_ZONE(N)%ID = ID
          P_ZONE(N)%LEAK_AREA(0) = CUT_LEAK(NN)
       ENDIF
       ! бокс ровно вокруг клетки-затравки: покрывает только её центр
       P_ZONE(N)%X1 = M%XC(I) - 0.5_EB*M%DX(I)
       P_ZONE(N)%X2 = M%XC(I) + 0.5_EB*M%DX(I)
       P_ZONE(N)%Y1 = M%YC(J) - 0.5_EB*M%DY(J)
       P_ZONE(N)%Y2 = M%YC(J) + 0.5_EB*M%DY(J)
       P_ZONE(N)%Z1 = M%ZC(K) - 0.5_EB*M%DZ(K)
       P_ZONE(N)%Z2 = M%ZC(K) + 0.5_EB*M%DZ(K)
    ENDIF

ENDDO ZONE_FILL_LOOP

DEALLOCATE(XB_USER)
DEALLOCATE(LEAK_USER)
DEALLOCATE(AUTO_SEED)
DEALLOCATE(CUT_SEED)
DEALLOCATE(CUT_LEAK)
...
SUBROUTINE COUNT_PRESSURE_POCKETS(N_ZONE_IN,XB_USER,N_AUTO_OUT,AUTO_SEED)

! R27: поиск запертых газовых объёмов (карманов) в остатке зоны 0.
! Карман = компонента связности газовых клеток меша; обход строго как в
! ASSIGN_PRESSURE_ZONE (func.f90): только внутренние клетки 1..IBAR/JBAR/KBAR,
! тонкие OBST на грани непроходимы, SOLID-клетки не пересекаются, ghost-слой
! не traversируется. Компонента является карманом, если она (а) не содержит
! клеток пользовательских &ZONE-боксов и (б) не достигает выхода: OPEN/PERIODIC
! вента на внешней границе меша либо интерполированной границы с газовой
! клеткой соседнего меша напротив. Гидростатика/PBAR кармана задаёт штатная
! инициализация зон; расширение газа уходит в фон via ODE D_PBAR_DT.
! Ограничение R27a: карман на стыке мешей зоной не становится
! (интерполированная граница считается выходом) — консервативно,
! поведение не хуже исходного (зона 0).

INTEGER, INTENT(IN) :: N_ZONE_IN
REAL(EB), INTENT(IN) :: XB_USER(6,N_ZONE_IN)
INTEGER, INTENT(OUT) :: N_AUTO_OUT
INTEGER, INTENT(OUT), ALLOCATABLE :: AUTO_SEED(:,:)

INTEGER :: NM,I,J,K,II,JJ,KK,III,JJJ,KKK,IOR,NN,NOM,IIO,JJO,KKO,ICN,Q_N,NCELL
LOGICAL :: ESCAPE,CLAIMED,ESCAPE_FACE
REAL(EB) :: XVOL,XX,YY,ZZ,XI,YJ,ZK
INTEGER, ALLOCATABLE, DIMENSION(:) :: Q_I,Q_J,Q_K
INTEGER, ALLOCATABLE, DIMENSION(:,:) :: AUTO_TMP
INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: VIS
LOGICAL, ALLOCATABLE, DIMENSION(:,:,:) :: MTX,MTY,MTZ
TYPE (MESH_TYPE), POINTER :: MP,M2
TYPE (OBSTRUCTION_TYPE), POINTER :: OBP
TYPE (VENTS_TYPE), POINTER :: VTP

N_AUTO_OUT = 0
ALLOCATE(AUTO_SEED(4,0))

ZONE_SCAN_MESH: DO NM=1,NMESHES
   IF (EVACUATION_ONLY(NM)) CYCLE ZONE_SCAN_MESH
   ! FDS6/MPI: SOLID/CELL_INDEX аллоцированы только на ранге-владельце меша
   ! (READ_OBST MESH_LOOP) — чужие мешы пропускаем, иначе unassociated pointer
   IF (PROCESS(NM)/=MYID) CYCLE ZONE_SCAN_MESH
   MP => MESHES(NM)
   NCELL = MP%IBAR*MP%JBAR*MP%KBAR
   ALLOCATE(Q_I(NCELL),Q_J(NCELL),Q_K(NCELL))
   ALLOCATE(VIS(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   VIS = 0

   ! R27c: маски тонких OBST (закраска один раз вместо скана всех OBST на грань)

   ALLOCATE(MTX(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1),MTY(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1),MTZ(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   MTX = .FALSE.; MTY = .FALSE.; MTZ = .FALSE.
   DO NN=1,MP%N_OBST
      OBP=>MP%OBSTRUCTION(NN)
      IF (OBP%I1==OBP%I2) THEN
         DO K=MAX(1,OBP%K1+1),MIN(MP%KBAR,OBP%K2)
            DO J=MAX(1,OBP%J1+1),MIN(MP%JBAR,OBP%J2)
               MTX(OBP%I1,J,K) = .TRUE.
            ENDDO
         ENDDO
      ELSEIF (OBP%J1==OBP%J2) THEN
         DO K=MAX(1,OBP%K1+1),MIN(MP%KBAR,OBP%K2)
            DO I=MAX(1,OBP%I1+1),MIN(MP%IBAR,OBP%I2)
               MTY(I,OBP%J1,K) = .TRUE.
            ENDDO
         ENDDO
      ELSEIF (OBP%K1==OBP%K2) THEN
         DO J=MAX(1,OBP%J1+1),MIN(MP%JBAR,OBP%J2)
            DO I=MAX(1,OBP%I1+1),MIN(MP%IBAR,OBP%I2)
               MTZ(I,J,OBP%K1) = .TRUE.
            ENDDO
   ...
   DO K=1,MP%KBAR
      DO J=1,MP%JBAR
         DO I=1,MP%IBAR
            IF (MP%SOLID(MP%CELL_INDEX(I,J,K))) CYCLE
            IF (VIS(I,J,K)/=0) CYCLE

            ! BFS новой компоненты связности
            ...
            ESCAPE  = .FALSE.
            CLAIMED = .FALSE.
            XVOL = MP%DX(I)*MP%RC(I)*MP%DY(J)*MP%DZ(K)
            IF (IN_USER_BOX(MP%XC(I),MP%YC(J),MP%ZC(K))) CLAIMED = .TRUE.
            ...
            SORT_QUEUE: DO
               ...
               SEARCH_LOOP: DO IOR=-3,3
                  ...
                  SELECT CASE(IOR)
                     CASE(-1)
                        II = III-1; JJ = JJJ; KK = KKK
                     CASE( 1)
                        II = III+1; JJ = JJJ; KK = KKK
                     CASE(-2)
                        II = III; JJ = JJJ-1; KK = KKK
                     CASE( 2)
                        II = III; JJ = JJJ+1; KK = KKK
                     CASE(-3)
                        II = III; JJ = JJJ; KK = KKK-1
                     CASE( 3)
                        II = III; JJ = JJJ; KK = KKK+1
                  END SELECT

                  ! Грань на границе меша: проверить, есть ли выход
                  ! (OPEN/PERIODIC вент либо газовая клетка соседнего меша)

                  IF (II<1 .OR. II>MP%IBAR .OR. JJ<1 .OR. JJ>MP%JBAR .OR. KK<1 .OR. KK>MP%KBAR) THEN
                     IF (.NOT.ESCAPE) THEN
                        ESCAPE_FACE = .FALSE.
                        ! (a) вент на внешней границе
                        DO NN=1,MP%N_VENT
                           VTP => MP%VENTS(NN)
                           IF (VTP%BOUNDARY_TYPE/=OPEN_BOUNDARY .AND. VTP%BOUNDARY_TYPE/=PERIODIC_BOUNDARY) CYCLE
                           SELECT CASE(IOR)
                              CASE(-1)
                                 IF (VTP%IOR== 1 .AND. JJ>=VTP%J1+1 .AND. JJ<=VTP%J2 .AND. KK>=VTP%K1+1 .AND. KK<=VTP%K2) ESCAPE_FACE = .TRUE.
                              CASE( 1)
                                 IF (VTP%IOR==-1 .AND. JJ>=VTP%J1+1 .AND. JJ<=VTP%J2 .AND. KK>=VTP%K1+1 .AND. KK<=VTP%K2) ESCAPE_FACE = .TRUE.
                              CASE(-2)
                                 IF (VTP%IOR== 2 .AND. II>=VTP%I1+1 .AND. II<=VTP%I2 .AND. KK>=VTP%K1+1 .AND. KK<=VTP%K2) ESCAPE_FACE = .TRUE.
                              CASE( 2)
                                 IF (VTP%IOR==-2 .AND. II>=VTP%I1+1 .AND. II<=VTP%I2 .AND. KK>=VTP%K1+1 .AND. KK<=VTP%K2) ESCAPE_FACE = .TRUE.
                              CASE(-3)
                                 IF (VTP%IOR== 3 .AND. II>=VTP%I1+1 .AND. II<=VTP%I2 .AND. JJ>=VTP%J1+1 .AND. JJ<=VTP%J2) ESCAPE_FACE = .TRUE.
                              CASE( 3)
                                 IF (VTP%IOR==-3 .AND. II>=VTP%I1+1 .AND. II<=VTP%I2 .AND. JJ>=VTP%J1+1 .AND. JJ<=VTP%J2) ESCAPE_FACE = .TRUE.
                           END SELECT
                           IF (ESCAPE_FACE) EXIT
                        ENDDO
                        ! (b) интерполированная граница: газ в соседнем меше напротив
                        IF (.NOT.ESCAPE_FACE) THEN
                           XX = MP%XC(II)
                           YY = MP%YC(JJ)
                           ZZ = MP%ZC(KK)
                            OTHER_MESH_ESC: DO NOM=1,NMESHES
                               IF (NOM==NM) CYCLE OTHER_MESH_ESC
                               IF (EVACUATION_ONLY(NOM)) CYCLE OTHER_MESH_ESC
                               M2 => MESHES(NOM)
                               IF (XX>=M2%XS .AND. XX<=M2%XF .AND. YY>=M2%YS .AND. YY<=M2%YF .AND. ZZ>=M2%ZS .AND. ZZ<=M2%ZF) THEN
                                  ! FDS6/MPI: SOLID/CELL_INDEX аллоцированы только на ранге-владельце
                                  ! меша (READ_OBST). Состояние клетки чужого меша на этом ранге
                                  ! недоступно — консервативно считаем грань выходом: карман,
                                  ! граничащий с чужим мешем, не герметизируется (анализ per-mesh).
                                 ...
                                 JJO = FLOOR(YJ)
                                 KKO = FLOOR(ZK)
                                 IF (.NOT.M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) THEN
                                    ESCAPE_FACE = .TRUE.
                                    EXIT OTHER_MESH_ESC
                                 ENDIF
                              ENDIF
                           ENDDO OTHER_MESH_ESC
                        ENDIF
                        IF (ESCAPE_FACE) ESCAPE = .TRUE.
                     ENDIF
                     CYCLE SEARCH_LOOP
                  ENDIF

                  ! Тонкие OBST на грани непроходимы (как в ASSIGN_PRESSURE_ZONE)
                  ! R27c: предвычисленные маски MTX/MTY/MTZ

                  SELECT CASE(IOR)
                     CASE(-1)
                        IF (MTX(II,JJ,KK)) CYCLE SEARCH_LOOP
                     CASE( 1)
                        IF (MTX(II-1,JJ,KK)) CYCLE SEARCH_LOOP
                     CASE(-2)
                        IF (MTY(II,JJ,KK)) CYCLE SEARCH_LOOP
                     CASE( 2)
                        IF (MTY(II,JJ-1,KK)) CYCLE SEARCH_LOOP
                     CASE(-3)
                        IF (MTZ(II,JJ,KK)) CYCLE SEARCH_LOOP
                     CASE( 3)
                        IF (MTZ(II,JJ,KK-1)) CYCLE SEARCH_LOOP
                  END SELECT

                  ! SOLID-клетка — не пересекаем; газовую непосещённую — в очередь

                  ICN = MP%CELL_INDEX(II,JJ,KK)
                  IF (MP%SOLID(ICN)) CYCLE SEARCH_LOOP
                  IF (VIS(II,JJ,KK)/=0) CYCLE SEARCH_LOOP
                  VIS(II,JJ,KK) = 1
                  XVOL = XVOL + MP%DX(II)*MP%RC(II)*MP%DY(JJ)*MP%DZ(KK)
                  IF (IN_USER_BOX(MP%XC(II),MP%YC(JJ),MP%ZC(KK))) CLAIMED = .TRUE.
                  Q_N = Q_N + 1
                  Q_I(Q_N) = II
                  Q_J(Q_N) = JJ
                  Q_K(Q_N) = KK
            ...
            ! Компонента без выхода и без пользовательских зон = карман

            IF (CLAIMED .OR. ESCAPE) CYCLE

            N_AUTO_OUT = N_AUTO_OUT + 1
            ALLOCATE(AUTO_TMP(4,N_AUTO_OUT))
            IF (N_AUTO_OUT>1) AUTO_TMP(:,1:N_AUTO_OUT-1) = AUTO_SEED(:,1:N_AUTO_OUT-1)
            AUTO_TMP(1,N_AUTO_OUT) = NM
            AUTO_TMP(2,N_AUTO_OUT) = I
            AUTO_TMP(3,N_AUTO_OUT) = J
            AUTO_TMP(4,N_AUTO_OUT) = K
            IF (ALLOCATED(AUTO_SEED)) DEALLOCATE(AUTO_SEED)
            CALL MOVE_ALLOC(AUTO_TMP,AUTO_SEED)
            IF (MYID==0) WRITE(LU_OUTPUT,'(A,I4,A,I3,A,3F9.3,A,F10.2,A)') ' AUTO-ZONE R27: pocket ',N_AUTO_OUT, &
               ' mesh ',NM,' seed xyz ',MP%XC(I),MP%YC(J),MP%ZC(K),' volume ',XVOL,' m3'

         ENDDO
      ENDDO
   ENDDO

   DEALLOCATE(Q_I,Q_J,Q_K,VIS,MTX,MTY,MTZ)

ENDDO ZONE_SCAN_MESH

CONTAINS

LOGICAL FUNCTION IN_USER_BOX(XX,YY,ZZ)
REAL(EB), INTENT(IN) :: XX,YY,ZZ
INTEGER :: NN
IN_USER_BOX = .FALSE.
DO NN=1,N_ZONE_IN
   IF (XX>XB_USER(1,NN) .AND. XX<XB_USER(2,NN) .AND. &
       YY>XB_USER(3,NN) .AND. YY<XB_USER(4,NN) .AND. &
       ZZ>XB_USER(5,NN) .AND. ZZ<XB_USER(6,NN)) THEN
      IN_USER_BOX = .TRUE.
      RETURN
   ENDIF
ENDDO
END FUNCTION IN_USER_BOX

END SUBROUTINE COUNT_PRESSURE_POCKETS


SUBROUTINE DETECT_AND_SEAL_FIRE_POCKETS(N_ZONE_IN,XB_USER,NEXT_ZONE,N_CUT_Z,CUT_SEED,CUT_LEAK)

! R27b: авто-зонирование пожарных отсеков по узкому горлу. Для каждой компоненты
! с очагом (VENT на SURF%HRRPUA>0), не занятой пользовательской &ZONE и не
! являющейся строгим карманом, ищется минимальный разрез (max-flow Эдмондса-
! Карпа, неориентированные рёбра-грани с единичной пропускной способностью,
! сток = газовые клетки с выходом на OPEN/PERIODIC вент либо интерполированную
! границу с газом соседнего меша; рёбра к стоку бесконечны — разрез внутренний).
! Если площадь разреза <= AUTO_ZONE_MAX_GAP, разрез герметизируется синтезом
! тонких OBST (по одному на грань), создаётся синтетический SURF с
! LEAK_PATH=(зона,0) и зона с LEAK_AREA(0)=площадь разреза: расширение уходит
! в PBAR через ODE D_PBAR_DT, обмен с атмосферой — через штатную машину
! утечек (R26f-бюджет). Ограничение: анализ per-mesh; S не содержит стоковых
! клеток (следствие max-flow), поэтому зона строго внутри меша.

INTEGER, INTENT(IN) :: N_ZONE_IN,NEXT_ZONE
REAL(EB), INTENT(IN) :: XB_USER(6,N_ZONE_IN)
INTEGER, INTENT(OUT) :: N_CUT_Z
INTEGER, INTENT(OUT), ALLOCATABLE :: CUT_SEED(:,:)
REAL(EB), INTENT(OUT), ALLOCATABLE :: CUT_LEAK(:)

INTEGER :: NM,NV,NN,NOM,IIO,JJO,KKO,ICN,Q_N,NFS,FLOW,FLOW_LIMIT,NCUTF,IBC_SYN,IZN,NNM,NSEL
INTEGER :: I,J,K,II,JJ,KK,III,JJJ,KKK,IOR,TARGET,R_SEED,NNEW,ISEED,JSEED,KSEED
INTEGER, ALLOCATABLE, DIMENSION(:) :: Q_I,Q_J,Q_K,FI,FJ,FK
INTEGER, ALLOCATABLE, DIMENSION(:,:) :: CUT_TMP
INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: CVIS
REAL(EB), ALLOCATABLE, DIMENSION(:) :: LEAK_TMP
INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: FLX,FLY,FLZ,PAR
LOGICAL, ALLOCATABLE, DIMENSION(:,:,:) :: SINKM,IN_S,BALL,TBX,TBY,TBZ
LOGICAL :: ESCAPE,CLAIMED,FOUND
REAL(EB) :: CUT_AREA
TYPE (MESH_TYPE), POINTER :: MP,M2
TYPE (OBSTRUCTION_TYPE), POINTER :: OBP
TYPE (VENTS_TYPE), POINTER :: VTP
TYPE (OBSTRUCTION_TYPE), POINTER, DIMENSION(:) :: OB_DUMMY
TYPE (SURFACE_TYPE), ALLOCATABLE, DIMENSION(:) :: SURF_TMP

N_CUT_Z = 0
ALLOCATE(CUT_SEED(4,0))
ALLOCATE(CUT_LEAK(0))
IF (.NOT.AUTO_ZONE .OR. AUTO_ZONE_MAX_GAP<=0._EB) RETURN

MESH_SCAN: DO NM=1,NMESHES
   IF (EVACUATION_ONLY(NM)) CYCLE MESH_SCAN
   ! FDS6/MPI: как в COUNT_PRESSURE_POCKETS — только мешы своего ранга
   IF (PROCESS(NM)/=MYID) CYCLE MESH_SCAN
   MP => MESHES(NM)

   ! (1) Затравки: газовые клетки вентов с HRRPUA>0

   NFS = 0
   ALLOCATE(FI(MP%IBAR*MP%JBAR*MP%KBAR),FJ(MP%IBAR*MP%JBAR*MP%KBAR),FK(MP%IBAR*MP%JBAR*MP%KBAR))
   DO NV=1,MP%N_VENT
      VTP => MP%VENTS(NV)
      IF (VTP%IBC<=0) CYCLE
      IF (SURFACE(VTP%IBC)%HRRPUA<=0._EB .AND. SURFACE(VTP%IBC)%MLRPUA<=0._EB) CYCLE
      DO KK=VTP%K1+1,MAX(VTP%K1+1,VTP%K2)
         DO JJ=VTP%J1+1,MAX(VTP%J1+1,VTP%J2)
            DO II=VTP%I1+1,MAX(VTP%I1+1,VTP%I2)
               ICN = MP%CELL_INDEX(II,JJ,KK)
               IF (MP%SOLID(ICN)) CYCLE
               NFS = NFS+1
               FI(NFS) = II
               FJ(NFS) = JJ
               FK(NFS) = KK
   ...
   ALLOCATE(Q_I(MP%IBAR*MP%JBAR*MP%KBAR),Q_J(MP%IBAR*MP%JBAR*MP%KBAR),Q_K(MP%IBAR*MP%JBAR*MP%KBAR))
   ALLOCATE(CVIS(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   ALLOCATE(FLX(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1),FLY(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1),FLZ(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   ALLOCATE(PAR(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   ALLOCATE(SINKM(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   ALLOCATE(IN_S(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   ALLOCATE(BALL(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   ALLOCATE(TBX(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1),TBY(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1),TBZ(0:MP%IBP1,0:MP%JBP1,0:MP%KBP1))
   CVIS = 0
   FLOW_LIMIT = INT(AUTO_ZONE_MAX_GAP/(MP%CELL_SIZE*MP%CELL_SIZE)) + 4

   ! Маски тонких OBST по граням: закраска прямоугольников один раз (O(N_OBST x
   ! площадь), а не скан всех OBST на каждую грань BFS — иначе детект на кейсе
   ! 64x112x24 с 213 OBST крутится десятки минут).

   TBX = .FALSE.; TBY = .FALSE.; TBZ = .FALSE.
   DO NN=1,MP%N_OBST
      OBP=>MP%OBSTRUCTION(NN)
      IF (OBP%I1==OBP%I2) THEN
         DO K=MAX(1,OBP%K1+1),MIN(MP%KBAR,OBP%K2)
   ...
   ! Расширение затравок шаром радиуса R_SEED: периметр шара всегда больше
   ! FLOW_LIMIT, поэтому тривиальный разрез «вокруг очага» не может выиграть —
   ! минимальный разрез остаётся горлом отсека (двери/проёмы).

   R_SEED = INT(SQRT(REAL(FLOW_LIMIT,EB))*0.5_EB) + 2
   BALL = .FALSE.
   Q_N = 0
   DO NNM=1,NFS
      BALL(FI(NNM),FJ(NNM),FK(NNM)) = .TRUE.
      Q_N = Q_N + 1
      Q_I(Q_N) = FI(NNM); Q_J(Q_N) = FJ(NNM); Q_K(Q_N) = FK(NNM)
   ENDDO
   DO NN=1,R_SEED
      NNEW = 0
      DO NNM=1,Q_N
         I = Q_I(NNM); J = Q_J(NNM); K = Q_K(NNM)
         DO IOR=-3,3
            IF (IOR==0) CYCLE
            SELECT CASE(IOR)
               ...
               CASE( 3); II = I; JJ = J; KK = K+1
            END SELECT
            IF (II<1 .OR. II>MP%IBAR .OR. JJ<1 .OR. JJ>MP%JBAR .OR. KK<1 .OR. KK>MP%KBAR) CYCLE
            IF (THIN_BLOCKED2(IOR,II,JJ,KK)) CYCLE
            IF (MP%SOLID(MP%CELL_INDEX(II,JJ,KK))) CYCLE
            IF (BALL(II,JJ,KK)) CYCLE
            BALL(II,JJ,KK) = .TRUE.
            NNEW = NNEW + 1
            FI(NNEW) = II; FJ(NNEW) = JJ; FK(NNEW) = KK
         ENDDO
      ENDDO
      IF (NNEW==0) EXIT
      DO NNM=1,NNEW
         Q_I(NNM) = FI(NNM); Q_J(NNM) = FJ(NNM); Q_K(NNM) = FK(NNM)
      ENDDO
      Q_N = NNEW
   ENDDO
   NFS = 0
   DO K=1,MP%KBAR
      DO J=1,MP%JBAR
         DO I=1,MP%IBAR
            IF (.NOT.BALL(I,J,K)) CYCLE
            NFS = NFS + 1
            FI(NFS) = I; FJ(NFS) = J; FK(NFS) = K
         ENDDO
      ENDDO
   ENDDO
   DEALLOCATE(BALL)
   IF (MYID==0) THEN; WRITE(LU_ERR,'(A,I8)') 'AUTOZONE R27: seed ball cells ',NFS; FLUSH(LU_ERR); ENDIF

   SEED_LOOP: DO NN=1,NFS

      I = FI(NN); J = FJ(NN); K = FK(NN)
      IF (CVIS(I,J,K)/=0) CYCLE SEED_LOOP
      ! R27d: затравка зоны = клетка-источник текущей компоненты (I,J,K после
      ! циклов сбора разреза становятся мусором IBAR+1 и зона выходит ПУСТОЙ)
      ISEED = I; JSEED = J; KSEED = K

      ! (2) Компонента связности от затравки: пропуск занятых пользователем и
      ! строгих карманов (без выхода), сбор затравок этой компоненты

      ESCAPE  = .FALSE.
      CLAIMED = .FALSE.
      Q_N = 1
      Q_I(1) = I; Q_J(1) = J; Q_K(1) = K
      CVIS(I,J,K) = 1
      IF (IN_USER_BOX2(MP%XC(I),MP%YC(J),MP%ZC(K))) CLAIMED = .TRUE.
      SORT_QUEUE2: DO
         IF (Q_N<1) EXIT SORT_QUEUE2
         III = Q_I(Q_N); JJJ = Q_J(Q_N); KKK = Q_K(Q_N)
         Q_N = Q_N - 1
         SEARCH2: DO IOR=-3,3
            IF (IOR==0) CYCLE SEARCH2
            SELECT CASE(IOR)
               CASE(-1); II = III-1; JJ = JJJ; KK = KKK
               CASE( 1); II = III+1; JJ = JJJ; KK = KKK
               CASE(-2); II = III; JJ = JJJ-1; KK = KKK
               CASE( 2); II = III; JJ = JJJ+1; KK = KKK
               CASE(-3); II = III; JJ = JJJ; KK = KKK-1
               CASE( 3); II = III; JJ = JJJ; KK = KKK+1
            END SELECT
            IF (II<1 .OR. II>MP%IBAR .OR. JJ<1 .OR. JJ>MP%JBAR .OR. KK<1 .OR. KK>MP%KBAR) THEN
               IF (.NOT.ESCAPE) THEN
                  IF (FACE_ESCAPE2(IOR,III,JJJ,KKK)) ESCAPE = .TRUE.
               ENDIF
               CYCLE SEARCH2
            ENDIF
            IF (THIN_BLOCKED2(IOR,II,JJ,KK)) CYCLE SEARCH2
            ICN = MP%CELL_INDEX(II,JJ,KK)
            IF (MP%SOLID(ICN)) CYCLE SEARCH2
            IF (CVIS(II,JJ,KK)/=0) CYCLE SEARCH2
            CVIS(II,JJ,KK) = 1
            IF (IN_USER_BOX2(MP%XC(II),MP%YC(JJ),MP%ZC(KK))) CLAIMED = .TRUE.
            Q_N = Q_N + 1
            Q_I(Q_N) = II; Q_J(Q_N) = JJ; Q_K(Q_N) = KK
         ENDDO SEARCH2
      ENDDO SORT_QUEUE2

      IF (CLAIMED) CYCLE SEED_LOOP       ! пользователь разметил &ZONE — не трогаем
      IF (.NOT.ESCAPE) CYCLE SEED_LOOP   ! строгий карман — уже сделан R27

      IF (MYID==0) THEN; WRITE(LU_ERR,'(A,2L2)') 'AUTOZONE R27: component check escape/claimed ',ESCAPE,CLAIMED; FLUSH(LU_ERR); ENDIF
      ! (3) Инициализация max-flow: FL=0 (ребро-грань свободно), 9 (ребра нет);
      ! сток = клетка с выходной грани
      ...
      DO K=1,MP%KBAR
         DO J=1,MP%JBAR
            DO I=1,MP%IBAR
               IF (MP%SOLID(MP%CELL_INDEX(I,J,K))) CYCLE
               IF (I<MP%IBAR) THEN
                  IF (.NOT.MP%SOLID(MP%CELL_INDEX(I+1,J,K)) .AND. .NOT.THIN_BLOCKED2(-1,I,J,K)) FLX(I,J,K) = 0
               ENDIF
               IF (J<MP%JBAR) THEN
                  IF (.NOT.MP%SOLID(MP%CELL_INDEX(I,J+1,K)) .AND. .NOT.THIN_BLOCKED2(-2,I,J,K)) FLY(I,J,K) = 0
               ENDIF
               IF (K<MP%KBAR) THEN
                  IF (.NOT.MP%SOLID(MP%CELL_INDEX(I,J,K+1)) .AND. .NOT.THIN_BLOCKED2(-3,I,J,K)) FLZ(I,J,K) = 0
               ENDIF
            ENDDO
         ENDDO
      ENDDO
      ...
      DO K=1,MP%KBAR
         DO J=1,MP%JBAR
            DO I=1,MP%IBAR
               IF (MP%SOLID(MP%CELL_INDEX(I,J,K))) CYCLE
               IF (I==1)        THEN; IF (FACE_ESCAPE2(-1,1,J,K)) SINKM(I,J,K) = .TRUE.; ENDIF
               IF (I==MP%IBAR)  THEN; IF (FACE_ESCAPE2( 1,I,J,K)) SINKM(I,J,K) = .TRUE.; ENDIF
               IF (J==1)        THEN; IF (FACE_ESCAPE2(-2,I,1,K)) SINKM(I,J,K) = .TRUE.; ENDIF
               IF (J==MP%JBAR)  THEN; IF (FACE_ESCAPE2( 2,I,J,K)) SINKM(I,J,K) = .TRUE.; ENDIF
               IF (K==1)        THEN; IF (FACE_ESCAPE2(-3,I,J,1)) SINKM(I,J,K) = .TRUE.; ENDIF
               IF (K==MP%KBAR)  THEN; IF (FACE_ESCAPE2( 3,I,J,K)) SINKM(I,J,K) = .TRUE.; ENDIF
            ENDDO
         ENDDO
      ENDDO

      ! Затравка компоненты у самого выхода — отсек не заперт, не зонируем

      FOUND = .FALSE.
      DO NNM=1,NFS
         IF (CVIS(FI(NNM),FJ(NNM),FK(NNM))==1 .AND. SINKM(FI(NNM),FJ(NNM),FK(NNM))) FOUND = .TRUE.
      ENDDO
      IF (FOUND) CYCLE SEED_LOOP

      ! Затравки этой компоненты: CVIS==1 (фильтр в цикле ниже)

      ! (4) Эдмондс-Карп: multiline-source BFS до стоковой клетки
            ...
            AUGMENT_LOOP: DO
               ...
               Q_N = 0
               DO NNM=1,NFS
                  II = FI(NNM); JJ = FJ(NNM); KK = FK(NNM)
                  IF (CVIS(II,JJ,KK)/=1) CYCLE
                  IF (PAR(II,JJ,KK)==-1) THEN
                     PAR(II,JJ,KK) = 0
                     Q_N = Q_N + 1
                     Q_I(Q_N) = II; Q_J(Q_N) = JJ; Q_K(Q_N) = KK
                  ENDIF
               ...
               TARGET = 0
               BFS_LOOP: DO
                  IF (Q_N<1) EXIT BFS_LOOP
                  III = Q_I(Q_N); JJJ = Q_J(Q_N); KKK = Q_K(Q_N)
                  Q_N = Q_N - 1
                  IF (SINKM(III,JJJ,KKK)) THEN
                     TARGET = 1
                     EXIT BFS_LOOP
                  ENDIF
                  DO IOR=-3,3
                     ...
                     END SELECT
                     IF (II<1 .OR. II>MP%IBAR .OR. JJ<1 .OR. JJ>MP%JBAR .OR. KK<1 .OR. KK>MP%KBAR) CYCLE
                     IF (PAR(II,JJ,KK)/=-1) CYCLE
                     ! R27c: коды направлений 1..6 (НЕ ±1..3!) — значение -1 занято
                     ! маркером «непосещено»; коллизия давала переоткрытия и вечный BACKTRACK
                     NSEL = 0
                     SELECT CASE(IOR)
                        CASE(-1)
                           IF (FLX(II,JJ,KK)/=9 .AND. FLX(II,JJ,KK)>=0) NSEL = 1
                        CASE( 1)
                           IF (FLX(III,JJJ,KKK)/=9 .AND. FLX(III,JJJ,KKK)<=0) NSEL = 2
                        CASE(-2)
                           IF (FLY(II,JJ,KK)/=9 .AND. FLY(II,JJ,KK)>=0) NSEL = 3
                        CASE( 2)
                           IF (FLY(III,JJJ,KKK)/=9 .AND. FLY(III,JJJ,KKK)<=0) NSEL = 4
                        CASE(-3)
                           IF (FLZ(II,JJ,KK)/=9 .AND. FLZ(II,JJ,KK)>=0) NSEL = 5
                        CASE( 3)
                           IF (FLZ(III,JJJ,KKK)/=9 .AND. FLZ(III,JJJ,KKK)<=0) NSEL = 6
                     END SELECT
                     IF (NSEL==0) CYCLE
                     PAR(II,JJ,KK) = NSEL
                     Q_N = Q_N + 1
               ...
               IF (TARGET==0) EXIT AUGMENT_LOOP

               ! Увеличение вдоль пути: от стоковой клетки назад к затравке

               II = III; JJ = JJJ; KK = KKK
               BACKTRACK: DO WHILE (PAR(II,JJ,KK)/=0)
                  NSEL = PAR(II,JJ,KK)
                  SELECT CASE(NSEL)
                     CASE(1)
                        FLX(II,JJ,KK) = FLX(II,JJ,KK) - 1
                        II = II + 1
                     CASE(2)
                        FLX(II-1,JJ,KK) = FLX(II-1,JJ,KK) + 1
                        II = II - 1
                     CASE(3)
                        FLY(II,JJ,KK) = FLY(II,JJ,KK) - 1
                        JJ = JJ + 1
                     CASE(4)
                        FLY(II,JJ-1,KK) = FLY(II,JJ-1,KK) + 1
                        JJ = JJ - 1
                     CASE(5)
                        FLZ(II,JJ,KK) = FLZ(II,JJ,KK) - 1
                        KK = KK + 1
                     CASE(6)
                        FLZ(II,JJ,KK-1) = FLZ(II,JJ,KK-1) + 1
                        KK = KK - 1
                  END SELECT
               ENDDO BACKTRACK

               FLOW = FLOW + 1
               IF (FLOW>FLOW_LIMIT) EXIT AUGMENT_LOOP

            ENDDO AUGMENT_LOOP

      IF (FLOW>FLOW_LIMIT) THEN
         IF (MYID==0) WRITE(LU_OUTPUT,'(A,I3,A,I5,A,F7.2,A)') ' AUTO-ZONE R27b: mesh ',NM, &
            ': bottleneck > ',FLOW,' paths — отсек НЕ зонируется (порог ',AUTO_ZONE_MAX_GAP,' m2)'
         CYCLE SEED_LOOP
      ENDIF

      IF (MYID==0) THEN; WRITE(LU_ERR,'(A,I6,A,I6)') 'AUTOZONE R27: max-flow done, FLOW=',FLOW,' limit=',FLOW_LIMIT; FLUSH(LU_ERR); ENDIF
      ! (5) S = достижимость в остаточной сети; разрез = насыщенные грани S|не-S

      IN_S = .FALSE.
      Q_N = 0
      DO NNM=1,NFS
         II = FI(NNM); JJ = FJ(NNM); KK = FK(NNM)
         IF (CVIS(II,JJ,KK)==1 .AND. .NOT.IN_S(II,JJ,KK)) THEN
            IN_S(II,JJ,KK) = .TRUE.
            Q_N = Q_N + 1
            Q_I(Q_N) = II; Q_J(Q_N) = JJ; Q_K(Q_N) = KK
         ENDIF
      ...
      RESIDUAL_BFS: DO
         ...
         DO IOR=-3,3
            ...
            END SELECT
            IF (II<1 .OR. II>MP%IBAR .OR. JJ<1 .OR. JJ>MP%JBAR .OR. KK<1 .OR. KK>MP%KBAR) CYCLE
            IF (IN_S(II,JJ,KK)) CYCLE
            SELECT CASE(IOR)
               CASE(-1)
                  IF (FLX(II,JJ,KK)/=9 .AND. FLX(II,JJ,KK)>=0) IN_S(II,JJ,KK) = .TRUE.
               CASE( 1)
                  IF (FLX(III,JJJ,KKK)/=9 .AND. FLX(III,JJJ,KKK)<=0) IN_S(II,JJ,KK) = .TRUE.
               CASE(-2)
                  IF (FLY(II,JJ,KK)/=9 .AND. FLY(II,JJ,KK)>=0) IN_S(II,JJ,KK) = .TRUE.
               CASE( 2)
                  IF (FLY(III,JJJ,KKK)/=9 .AND. FLY(III,JJJ,KKK)<=0) IN_S(II,JJ,KK) = .TRUE.
               CASE(-3)
                  IF (FLZ(II,JJ,KK)/=9 .AND. FLZ(II,JJ,KK)>=0) IN_S(II,JJ,KK) = .TRUE.
               CASE( 3)
                  IF (FLZ(III,JJJ,KKK)/=9 .AND. FLZ(III,JJJ,KKK)<=0) IN_S(II,JJ,KK) = .TRUE.
            END SELECT
            IF (IN_S(II,JJ,KK)) THEN
               Q_N = Q_N + 1
               Q_I(Q_N) = II; Q_J(Q_N) = JJ; Q_K(Q_N) = KK
            ENDIF
         ENDDO
      ...
      ! Сбор граней разреза (обе ориентации: S-клетка может быть любой стороной грани)
      ...
      DO K=1,MP%KBAR
         DO J=1,MP%JBAR
            DO I=1,MP%IBAR
               IF (.NOT.IN_S(I,J,K)) CYCLE
               IF (I<MP%IBAR) THEN
                  IF (FLX(I,J,K)/=9 .AND. .NOT.IN_S(I+1,J,K)) THEN
                     NCUTF = NCUTF + 1
                     CUT_AREA = CUT_AREA + MP%DY(J)*MP%DZ(K)
                  ENDIF
               ENDIF
               IF (I>1) THEN
                  IF (FLX(I-1,J,K)/=9 .AND. .NOT.IN_S(I-1,J,K)) THEN
                     NCUTF = NCUTF + 1
                     CUT_AREA = CUT_AREA + MP%DY(J)*MP%DZ(K)
                  ENDIF
               ENDIF
               IF (J<MP%JBAR) THEN
                  IF (FLY(I,J,K)/=9 .AND. .NOT.IN_S(I,J+1,K)) THEN
                     NCUTF = NCUTF + 1
                     CUT_AREA = CUT_AREA + MP%DX(I)*MP%DZ(K)
                  ENDIF
               ENDIF
               IF (J>1) THEN
                  IF (FLY(I,J-1,K)/=9 .AND. .NOT.IN_S(I,J-1,K)) THEN
                     NCUTF = NCUTF + 1
                     CUT_AREA = CUT_AREA + MP%DX(I)*MP%DZ(K)
                  ENDIF
               ENDIF
               IF (K<MP%KBAR) THEN
                  IF (FLZ(I,J,K)/=9 .AND. .NOT.IN_S(I,J,K+1)) THEN
                     NCUTF = NCUTF + 1
                     CUT_AREA = CUT_AREA + MP%DX(I)*MP%DY(J)
                  ENDIF
               ENDIF
               IF (K>1) THEN
                  IF (FLZ(I,J,K-1)/=9 .AND. .NOT.IN_S(I,J,K-1)) THEN
                     NCUTF = NCUTF + 1
                     CUT_AREA = CUT_AREA + MP%DX(I)*MP%DY(J)
                  ENDIF
               ENDIF
      ...
      IF (MYID==0) THEN; WRITE(LU_ERR,'(A,I6,A,F8.3)') 'AUTOZONE R27: cut faces=',NCUTF,' area m2=',CUT_AREA; FLUSH(LU_ERR); ENDIF
      IF (NCUTF==0 .OR. CUT_AREA>AUTO_ZONE_MAX_GAP) THEN
         IF (MYID==0) WRITE(LU_OUTPUT,'(A,I3,A,F7.3,A)') ' AUTO-ZONE R27b: mesh ',NM, &
            ': cut area ',CUT_AREA,' m2 > порог — НЕ зонируется'
         CYCLE SEED_LOOP
      ENDIF

      ! (6) Синтетический SURF с LEAK_PATH=(зона,0); копия INERT (SURFACE(0))

      IZN = NEXT_ZONE + N_CUT_Z + 1
      IBC_SYN = N_SURF + 1
      ALLOCATE(SURF_TMP(0:IBC_SYN))
      SURF_TMP(0:N_SURF) = SURFACE(0:N_SURF)
      IF (ALLOCATED(SURFACE)) DEALLOCATE(SURFACE)
      CALL MOVE_ALLOC(SURF_TMP,SURFACE)
      SF => SURFACE(IBC_SYN)
      SURFACE(IBC_SYN) = SURFACE(0)
      NULLIFY(SF%DX,SF%RDX,SF%RDXN,SF%X_S,SF%DX_WGT,SF%MF_FRAC,SF%PARTICLE_INSERT_CLOCK)
      NULLIFY(SF%N_LAYER_CELLS,SF%LAYER_INDEX,SF%MATL_INDEX,SF%RESIDUE_INDEX,SF%MIN_DIFFUSIVITY,SF%MATL_NAME)
      WRITE(SF%ID,'(A,I2.2)') 'AUTO_LEAK_',IZN
      SF%USER_DEFINED = .FALSE.
      SF%LEAK_PATH(1) = IZN
      SF%LEAK_PATH(2) = 0
      N_SURF = IBC_SYN

      ! (7) Синтез тонких OBST по граням разреза (по одному на грань)

      ALLOCATE(OB_DUMMY(0:MP%N_OBST+NCUTF))
      OB_DUMMY(0:MP%N_OBST) = MP%OBSTRUCTION(0:MP%N_OBST)
      NCUTF = 0
      DO K=1,MP%KBAR
         DO J=1,MP%JBAR
            DO I=1,MP%IBAR
               ...
               IF (I<MP%IBAR) THEN
                  IF (FLX(I,J,K)/=9 .AND. .NOT.IN_S(I+1,J,K)) THEN
                     NCUTF = NCUTF + 1
                     CALL SET_CUT_OBST(OB_DUMMY(MP%N_OBST+NCUTF),MP,1,I,J,K,IBC_SYN)
                  ENDIF
               ENDIF
               IF (I>1) THEN
                  IF (FLX(I-1,J,K)/=9 .AND. .NOT.IN_S(I-1,J,K)) THEN
                     NCUTF = NCUTF + 1
                     CALL SET_CUT_OBST(OB_DUMMY(MP%N_OBST+NCUTF),MP,1,I-1,J,K,IBC_SYN)
                  ENDIF
               ENDIF
               IF (J<MP%JBAR) THEN
                  IF (FLY(I,J,K)/=9 .AND. .NOT.IN_S(I,J+1,K)) THEN
                     NCUTF = NCUTF + 1
                     CALL SET_CUT_OBST(OB_DUMMY(MP%N_OBST+NCUTF),MP,2,I,J,K,IBC_SYN)
                  ENDIF
               ENDIF
               IF (J>1) THEN
                  IF (FLY(I,J-1,K)/=9 .AND. .NOT.IN_S(I,J-1,K)) THEN
                     NCUTF = NCUTF + 1
                     CALL SET_CUT_OBST(OB_DUMMY(MP%N_OBST+NCUTF),MP,2,I,J-1,K,IBC_SYN)
                  ENDIF
               ENDIF
               IF (K<MP%KBAR) THEN
                  IF (FLZ(I,J,K)/=9 .AND. .NOT.IN_S(I,J,K+1)) THEN
                     NCUTF = NCUTF + 1
                     CALL SET_CUT_OBST(OB_DUMMY(MP%N_OBST+NCUTF),MP,3,I,J,K,IBC_SYN)
                  ENDIF
               ENDIF
               IF (K>1) THEN
                  IF (FLZ(I,J,K-1)/=9 .AND. .NOT.IN_S(I,J,K-1)) THEN
                     NCUTF = NCUTF + 1
                     CALL SET_CUT_OBST(OB_DUMMY(MP%N_OBST+NCUTF),MP,3,I,J,K-1,IBC_SYN)
                  ENDIF
               ENDIF
            ENDDO
         ENDDO
      ENDDO
      DEALLOCATE(MP%OBSTRUCTION)
      MP%OBSTRUCTION => OB_DUMMY
      MP%N_OBST = MP%N_OBST + NCUTF

      IF (MYID==0) THEN; WRITE(LU_ERR,'(A)') 'AUTOZONE R27: walls+surf synthesized'; FLUSH(LU_ERR); ENDIF
      ! (8) Результат: затравка зоны, утечка = площадь разреза

      N_CUT_Z = N_CUT_Z + 1
      ALLOCATE(CUT_TMP(4,N_CUT_Z))
      IF (N_CUT_Z>1) CUT_TMP(:,1:N_CUT_Z-1) = CUT_SEED(:,1:N_CUT_Z-1)
      CUT_TMP(1,N_CUT_Z) = NM
      CUT_TMP(2,N_CUT_Z) = ISEED
      CUT_TMP(3,N_CUT_Z) = JSEED
      CUT_TMP(4,N_CUT_Z) = KSEED
      IF (ALLOCATED(CUT_SEED)) DEALLOCATE(CUT_SEED)
      CALL MOVE_ALLOC(CUT_TMP,CUT_SEED)
      ALLOCATE(LEAK_TMP(N_CUT_Z))
      IF (N_CUT_Z>1) LEAK_TMP(1:N_CUT_Z-1) = CUT_LEAK(1:N_CUT_Z-1)
      LEAK_TMP(N_CUT_Z) = CUT_AREA
      IF (ALLOCATED(CUT_LEAK)) DEALLOCATE(CUT_LEAK)
      CALL MOVE_ALLOC(LEAK_TMP,CUT_LEAK)
      IF (MYID==0) WRITE(LU_OUTPUT,'(A,I4,A,I3,A,F7.3,A,I5,A)') ' AUTO-ZONE R27b: zone ',IZN, &
         ' mesh ',NM,' sealed cut ',CUT_AREA,' m2 with ',NCUTF,' thin OBST; leak to ambient via LEAK_PATH'

   ENDDO SEED_LOOP

   DEALLOCATE(Q_I,Q_J,Q_K,CVIS,FLX,FLY,FLZ,PAR,SINKM,IN_S,TBX,TBY,TBZ,FI,FJ,FK)

ENDDO MESH_SCAN

CONTAINS

LOGICAL FUNCTION IN_USER_BOX2(XX,YY,ZZ)
REAL(EB), INTENT(IN) :: XX,YY,ZZ
INTEGER :: N2
IN_USER_BOX2 = .FALSE.
DO N2=1,N_ZONE_IN
   IF (XX>XB_USER(1,N2) .AND. XX<XB_USER(2,N2) .AND. &
       YY>XB_USER(3,N2) .AND. YY<XB_USER(4,N2) .AND. &
       ZZ>XB_USER(5,N2) .AND. ZZ<XB_USER(6,N2)) THEN
      IN_USER_BOX2 = .TRUE.
      RETURN
   ENDIF
ENDDO
END FUNCTION IN_USER_BOX2

LOGICAL FUNCTION THIN_BLOCKED2(IOR,II,JJ,KK)
INTEGER, INTENT(IN) :: IOR,II,JJ,KK
! R27b: предвычисленные маски TBX/TBY/TBZ (закраска прямоугольников тонких OBST)
SELECT CASE(IOR)
   CASE(-1); THIN_BLOCKED2 = TBX(II,JJ,KK)
   CASE( 1); THIN_BLOCKED2 = TBX(II-1,JJ,KK)
   CASE(-2); THIN_BLOCKED2 = TBY(II,JJ,KK)
   CASE( 2); THIN_BLOCKED2 = TBY(II,JJ-1,KK)
   CASE(-3); THIN_BLOCKED2 = TBZ(II,JJ,KK)
   CASE( 3); THIN_BLOCKED2 = TBZ(II,JJ,KK-1)
   CASE DEFAULT; THIN_BLOCKED2 = .FALSE.
END SELECT
END FUNCTION THIN_BLOCKED2

LOGICAL FUNCTION FACE_ESCAPE2(IOR,III,JJJ,KKK)
INTEGER, INTENT(IN) :: IOR,III,JJJ,KKK
INTEGER :: N2,NO,IIO,JJO,KKO
REAL(EB) :: XX2,YY2,ZZ2,XI2,YJ2,ZK2
LOGICAL :: ESC
TYPE (MESH_TYPE), POINTER :: M3
TYPE (VENTS_TYPE), POINTER :: VT3
SELECT CASE(IOR)
   CASE(-1); XX2 = MP%XC(III-1); YY2 = MP%YC(JJJ);   ZZ2 = MP%ZC(KKK)
   CASE( 1); XX2 = MP%XC(III+1); YY2 = MP%YC(JJJ);   ZZ2 = MP%ZC(KKK)
   CASE(-2); XX2 = MP%XC(III);   YY2 = MP%YC(JJJ-1); ZZ2 = MP%ZC(KKK)
   CASE( 2); XX2 = MP%XC(III);   YY2 = MP%YC(JJJ+1); ZZ2 = MP%ZC(KKK)
   CASE(-3); XX2 = MP%XC(III);   YY2 = MP%YC(JJJ);   ZZ2 = MP%ZC(KKK-1)
   CASE( 3); XX2 = MP%XC(III);   YY2 = MP%YC(JJJ);   ZZ2 = MP%ZC(KKK+1)
END SELECT
ESC = .FALSE.
DO N2=1,MP%N_VENT
   VT3 => MP%VENTS(N2)
   IF (VT3%BOUNDARY_TYPE/=OPEN_BOUNDARY .AND. VT3%BOUNDARY_TYPE/=PERIODIC_BOUNDARY) CYCLE
   SELECT CASE(IOR)
      CASE(-1)
         IF (VT3%IOR== 1 .AND. JJJ>=VT3%J1+1 .AND. JJJ<=VT3%J2 .AND. KKK>=VT3%K1+1 .AND. KKK<=VT3%K2) ESC = .TRUE.
      CASE( 1)
         IF (VT3%IOR==-1 .AND. JJJ>=VT3%J1+1 .AND. JJJ<=VT3%J2 .AND. KKK>=VT3%K1+1 .AND. KKK<=VT3%K2) ESC = .TRUE.
      CASE(-2)
         IF (VT3%IOR== 2 .AND. III>=VT3%I1+1 .AND. III<=VT3%I2 .AND. KKK>=VT3%K1+1 .AND. KKK<=VT3%K2) ESC = .TRUE.
      CASE( 2)
         IF (VT3%IOR==-2 .AND. III>=VT3%I1+1 .AND. III<=VT3%I2 .AND. KKK>=VT3%K1+1 .AND. KKK<=VT3%K2) ESC = .TRUE.
      CASE(-3)
         IF (VT3%IOR== 3 .AND. III>=VT3%I1+1 .AND. III<=VT3%I2 .AND. JJJ>=VT3%J1+1 .AND. JJJ<=VT3%J2) ESC = .TRUE.
      CASE( 3)
         IF (VT3%IOR==-3 .AND. III>=VT3%I1+1 .AND. III<=VT3%I2 .AND. JJJ>=VT3%J1+1 .AND. JJJ<=VT3%J2) ESC = .TRUE.
   END SELECT
   IF (ESC) EXIT
ENDDO
IF (.NOT.ESC) THEN
   OTHER_MESH2: DO NO=1,NMESHES
      IF (NO==NM) CYCLE OTHER_MESH2
      IF (EVACUATION_ONLY(NO)) CYCLE OTHER_MESH2
      M3 => MESHES(NO)
       IF (XX2>=M3%XS .AND. XX2<=M3%XF .AND. YY2>=M3%YS .AND. YY2<=M3%YF .AND. ZZ2>=M3%ZS .AND. ZZ2<=M3%ZF) THEN
          ! FDS6/MPI: SOLID/CELL_INDEX чужого меша недоступны на этом ранге —
          ! консервативно считаем грань выходом (см. COUNT_PRESSURE_POCKETS)
          IF (PROCESS(NO)/=MYID) THEN
             ESC = .TRUE.
             EXIT OTHER_MESH2
          ENDIF
          XI2 = MAX( 1._EB , MIN( REAL(M3%IBAR,EB)+ALMOST_ONE , M3%CELLSI(FLOOR((XX2-M3%XS)*M3%RDXINT)) + 1._EB ) )
         YJ2 = MAX( 1._EB , MIN( REAL(M3%JBAR,EB)+ALMOST_ONE , M3%CELLSJ(FLOOR((YY2-M3%YS)*M3%RDYINT)) + 1._EB ) )
         ZK2 = MAX( 1._EB , MIN( REAL(M3%KBAR,EB)+ALMOST_ONE , M3%CELLSK(FLOOR((ZZ2-M3%ZS)*M3%RDZINT)) + 1._EB ) )
         IIO = FLOOR(XI2); JJO = FLOOR(YJ2); KKO = FLOOR(ZK2)
         IF (.NOT.M3%SOLID(M3%CELL_INDEX(IIO,JJO,KKO))) THEN
            ESC = .TRUE.
            EXIT OTHER_MESH2
         ENDIF
      ENDIF
...
ENDIF
FACE_ESCAPE2 = ESC
END FUNCTION FACE_ESCAPE2

SUBROUTINE SET_CUT_OBST(OB,MPX,AXIS,II,JJ,KK,IBC_IN)
TYPE (OBSTRUCTION_TYPE), INTENT(OUT) :: OB
TYPE (MESH_TYPE), INTENT(IN) :: MPX
INTEGER, INTENT(IN) :: AXIS,II,JJ,KK,IBC_IN
SELECT CASE(AXIS)
   CASE(1)
      OB%I1 = II;   OB%I2 = II
      OB%J1 = JJ-1; OB%J2 = JJ
      OB%K1 = KK-1; OB%K2 = KK
      OB%X1 = MPX%X(OB%I1); OB%X2 = MPX%X(OB%I2)
      OB%Y1 = MPX%Y(OB%J1); OB%Y2 = MPX%Y(OB%J2)
      OB%Z1 = MPX%Z(OB%K1); OB%Z2 = MPX%Z(OB%K2)
   CASE(2)
      OB%J1 = JJ;   OB%J2 = JJ
      OB%I1 = II-1; OB%I2 = II
      OB%K1 = KK-1; OB%K2 = KK
...
OB%THIN       = .TRUE.
OB%CUT_FACE   = .FALSE.
OB%PERMIT_HOLE = .FALSE.
OB%HIDDEN     = .FALSE.
OB%REMOVABLE  = .FALSE.
OB%RGB        = (/255,0,255/)
OB%ORDINAL    = 900000 + IZN
END SUBROUTINE SET_CUT_OBST

END SUBROUTINE DETECT_AND_SEAL_FIRE_POCKETS
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      ! Skip QUANTITY='TIME' from CSV output (it duplicates the time column)

      IF (POINTS==1 .AND. QUANTITY/='TIME') N_DEVC_TIME = N_DEVC_TIME + 1
      ...
      ! Enable DEVC/CTRL event log if device has activation setpoint (FDS6 compatibility)
...
READ_CTRL_LOOP: DO NC=1,N_CTRL
   ...
   SELECT CASE(FUNCTION_TYPE)
      ...
      CASE('EXTERNAL')
         CF%CONTROL_INDEX = EXTERNAL_GATE
         CF%LATCH = .FALSE.
      CASE DEFAULT
   ...
   ! Enable DEVC/CTRL event log for all CTRL (FDS6 compatibility)
   IF (CF%CONTROL_INDEX < 100) WRITE_DEVC_CTRL = .TRUE.
...
PROC_CTRL_LOOP: DO NC = 1, N_CTRL
   ...
   CF => CONTROL(NC)
    ...
    ! Перебор ограничен размером массива INPUT_ID (40): при полностью
    ! заполненном списке (INPUT_ID(1:40) в экспорте Fenix+) поиск маркера 'null'
    ! выходил за границы массива (ошибка "cannot locate item for input")
    CF%N_INPUTS = 0
    INPUT_COUNT: DO NN=1,SIZE(CF%INPUT_ID)
       IF (CF%INPUT_ID(NN)=='null') EXIT INPUT_COUNT
       CF%N_INPUTS = CF%N_INPUTS + 1
    END DO INPUT_COUNT
   IF (CF%N_INPUTS==0 .AND. CF%CONTROL_INDEX /= EXTERNAL_GATE) THEN
      WRITE(MESSAGE,'(A,I5,A)')  'ERROR: CTRL ',NC,' must have at least one input'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (CF%N_INPUTS==0 .AND. CF%CONTROL_INDEX == EXTERNAL_GATE) THEN
      ! FDS6 EXTERNAL gate: no input needed, controlled by external CSV file
      ALLOCATE (CF%INPUT(1),STAT=IZERO)
      CALL ChkMemErr('READ','CF%INPUT',IZERO)
      ALLOCATE (CF%INPUT_TYPE(1),STAT=IZERO)
   ...
   BUILD_INPUT: DO NN = 1, CF%N_INPUTS
      ...
      ! FDS6 compatibility: handle built-in 'CLOCK' input for CUSTOM CTRL
      IF (TRIM(CF%INPUT_ID(NN))=='CLOCK') THEN
         CF%INPUT(NN) = 0
         CF%INPUT_TYPE(NN) = CLOCK_INPUT
         CYCLE BUILD_INPUT
      ENDIF
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   M=>MESHES(NM)
   IF (.NOT.ASSOCIATED(M%OBSTRUCTION)) CYCLE MESH_LOOP  ! Skip meshes where OBSTRUCTION not allocated
   IF (M%N_OBST==0) CYCLE MESH_LOOP  ! Skip meshes with no obstructions
   DO N=1,M%N_OBST
      OB=>M%OBSTRUCTION(N)
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
  ...
  ! Assign properties to the DEVICE array
   ...
   IF (DV%STATISTICS/='null') CALL CHANGE_UNITS(DV%QUANTITY,DV%UNITS,DV%STATISTICS,MYID,LU_OUTPUT)
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      VECTORLOOP: DO ITER=1,NITER
         ...
         ! FDS6 compatibility: skip SLCF if SPEC_INDEX=0 (unknown species)
         IF (SL%SPEC_INDEX==0 .AND. SPEC_ID/='null') THEN
            WRITE(MESSAGE,'(A,A,A)') 'WARNING: Skipping SLCF for unknown SPEC_ID=',TRIM(SPEC_ID)
            WRITE(LU_OUTPUT,'(A)') TRIM(MESSAGE)
            SL%SPEC_INDEX = -999  ! Mark as invalid
            CYCLE SLCF_LOOP
         ENDIF
...
IF (SPEC_ID=='hydrogen chloride') SPEC_ID='HYDROGEN CHLORIDE'
IF (SPEC_ID=='hydrogen fluoride') SPEC_ID='HYDROGEN FLUORIDE'
IF (SPEC_ID=='hydrogen bromide')  SPEC_ID='HYDROGEN BROMIDE'
IF (SPEC_ID=='hydrogen cyanide')  SPEC_ID='HYDROGEN CYANIDE'
...
! Search PRODUCTS mixture components (derived species from FDS6 lumped mixtures)
IF (SPEC_ID/='null' .AND. SPEC_INDEX==0 .AND. N_PRODUCTS_COMP>0) THEN
   DO NS=1,N_PRODUCTS_COMP
      IF (TRIM(SPEC_ID)==TRIM(PRODUCTS_COMP_ID(NS))) THEN
         SPEC_INDEX = DERIVED_SPEC_OFFSET + NS
         EXIT
      ENDIF
   ENDDO
ENDIF
...
IF (SPEC_ID/='null' .AND. SPEC_INDEX==0) THEN
   ! FDS6 compatibility: map standard FDS6 species names to FDS5 mixture fraction indices
   SELECT CASE(TRIM(SPEC_ID))
      CASE('OXYGEN')
         SPEC_INDEX = -2  ! O2 is always species 2 in mixture fraction
      CASE('NITROGEN')
         SPEC_INDEX = -3  ! N2 is always species 3
      CASE('CARBON DIOXIDE')
         SPEC_INDEX = -4  ! CO2 is always species 4
      CASE('WATER VAPOR')
         IF (N_STATE_SPECIES >= 5) SPEC_INDEX = -5
      CASE('CARBON MONOXIDE')
         IF (N_STATE_SPECIES >= 6) SPEC_INDEX = -6
      CASE('SOOT')
         IF (N_STATE_SPECIES >= 7) SPEC_INDEX = -7
      CASE('HYDROGEN')
         IF (N_STATE_SPECIES >= 8) SPEC_INDEX = -8
      CASE('OTHER')
         IF (N_STATE_SPECIES >= 9) SPEC_INDEX = -9
      CASE DEFAULT
         ! Unknown species - write warning and skip (don't crash)
         WRITE(MESSAGE,'(A,A,A)') 'WARNING: SPEC_ID ',TRIM(SPEC_ID),' not found in FDS5 species list, skipping output'
         WRITE(LU_OUTPUT,'(A)') TRIM(MESSAGE)
         SPEC_INDEX = 0  ! Will be handled by calling code
   END SELECT
ENDIF 
...
QUANTITY_INDEX_LOOP: DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
   QUANTITY_IF: IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
      ...
      IF (SPEC_INDEX >= DERIVED_SPEC_OFFSET) THEN
         ! Derived species from PRODUCTS mixture
         COMP_IDX = SPEC_INDEX - DERIVED_SPEC_OFFSET
         IF (COMP_IDX >= 1 .AND. COMP_IDX <= N_PRODUCTS_COMP) THEN
            SMOKEVIEW_LABEL = TRIM(PRODUCTS_COMP_ID(COMP_IDX))//' '//TRIM(QUANTITY)
            SMOKEVIEW_BAR_LABEL = TRIM(OUTPUT_QUANTITY(ND)%SHORT_NAME)//'_'//TRIM(PRODUCTS_COMP_ID(COMP_IDX))
         ELSE
...
FUNCTION FIND_PARENT_MESH(NM_CHILD) RESULT(NM_PARENT)
! Find the coarse mesh that completely contains the embedded mesh NM_CHILD.
! Returns 0 if no parent is found.
USE PRECISION_PARAMETERS
USE MESH_VARIABLES
USE GLOBAL_CONSTANTS
IMPLICIT NONE
INTEGER, INTENT(IN) :: NM_CHILD
INTEGER :: NM_PARENT, NOM
TYPE (MESH_TYPE), POINTER :: MC, MP

MC => MESHES(NM_CHILD)
NM_PARENT = 0

DO NOM = 1, NMESHES
   IF (NOM == NM_CHILD) CYCLE
   IF (MESHES(NOM)%EMBEDDED_MESH_FLAG) CYCLE  ! parent must be non-embedded
   MP => MESHES(NOM)
   ! Check if child is completely inside parent
   IF (MC%XS >= MP%XS .AND. MC%XF <= MP%XF .AND. &
       MC%YS >= MP%YS .AND. MC%YF <= MP%YF .AND. &
       MC%ZS >= MP%ZS .AND. MC%ZF <= MP%ZF) THEN
      ! Pick the smallest enclosing mesh (highest resolution parent)
      IF (NM_PARENT == 0) THEN
         NM_PARENT = NOM
      ELSE
         ! Prefer parent with smaller cell size (larger IJK)
         IF (MP%IBAR > MESHES(NM_PARENT)%IBAR) NM_PARENT = NOM
      ENDIF
   ENDIF
ENDDO

END FUNCTION FIND_PARENT_MESH
