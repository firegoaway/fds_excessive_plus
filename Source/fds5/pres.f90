...
PUBLIC PRESSURE_SOLVER,PRESSURE_SOLVER_SOAP,COMPUTE_VELOCITY_ERROR,GET_REV_PRES,COMPUTE_A_B,COMPUTE_CORRECTION_PRESSURE
...
USE MISC_FUNCTIONS, ONLY: INTERP_OVERLAP_WEIGHT
USE GLOBAL_CONSTANTS

INTEGER, INTENT(IN) :: NM
REAL(EB), INTENT(IN) :: T
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU=>NULL(),VV=>NULL(),WW=>NULL(),HP=>NULL(),RHOP=>NULL()
...
REAL(EB) :: TRM1,TRM2,TRM3,TRM4,RES,LHSS,RHSS,H_OTHER,DWWDT,DVVDT,DUUDT,RFODT,TNOW,DUMMY=0._EB, &
            ...
            H_NORM,H_DIFF_NORM,H_CHANGE,ALPHA_BLEND,H_MIN,WGT, &
            DX_OFF,DY_OFF,DZ_OFF,H_CORR  ! Phase 4: gradient correction
...
!$OMP PRIVATE(N_INT_CELLS,DX_OTHER,DY_OTHER,DZ_OTHER,VT,TSI,TIME_RAMP_FACTOR,P_EXTERNAL,WGT)
         ...
         DO KKO=IJKW(12,IW),IJKW(15,IW)
            DO JJO=IJKW(11,IW),IJKW(14,IW)
               DO IIO=IJKW(10,IW),IJKW(13,IW)
                  WGT = INTERP_OVERLAP_WEIGHT(IOR,ABS(IOR),I,J,K,IIO,JJO,KKO, &
                       Y,MESHES(NOM)%Y, &
                       Z,MESHES(NOM)%Z, &
                       X,MESHES(NOM)%X)
                  ! Phase 4: gradient correction for 2nd-order interpolation
                  DX_OFF = XC(I) - MESHES(NOM)%XC(IIO)
                  DY_OFF = YC(J) - MESHES(NOM)%YC(JJO)
                  DZ_OFF = ZC(K) - MESHES(NOM)%ZC(KKO)
                  IF (PREDICTOR) THEN
                     H_CORR = OMESH(NOM)%H(IIO,JJO,KKO) &
                            + OMESH(NOM)%DHDX(IIO,JJO,KKO) * DX_OFF &
                            + OMESH(NOM)%DHDY(IIO,JJO,KKO) * DY_OFF &
                            + OMESH(NOM)%DHDZ(IIO,JJO,KKO) * DZ_OFF
                     H_OTHER = H_OTHER + H_CORR * WGT
                  ENDIF
                  IF (CORRECTOR) THEN
                     H_CORR = OMESH(NOM)%HS(IIO,JJO,KKO) &
                            + OMESH(NOM)%DHDX(IIO,JJO,KKO) * DX_OFF &
                            + OMESH(NOM)%DHDY(IIO,JJO,KKO) * DY_OFF &
                            + OMESH(NOM)%DHDZ(IIO,JJO,KKO) * DZ_OFF
                     H_OTHER = H_OTHER + H_CORR * WGT
...
NO_SCARC_IF: IF (PRES_METHOD /= 'SCARC') THEN
   ...
   ! Adaptive pressure blending: damp sharp pressure jumps to prevent perforation
   ! Compute H_change = ||H_new - H_prev||_inf / ||H_new||_inf
   ! If H_change < threshold: alpha = 1.0 (no damping)
   ! If H_change > threshold: alpha = alpha_min + (1-alpha_min) * threshold/H_change
   
   IF (PRESSURE_BLEND_ADAPTIVE) THEN
      ! Compute relative change in pressure
      H_NORM = 0._EB
      H_DIFF_NORM = 0._EB
      !$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I) REDUCTION(MAX:H_NORM,H_DIFF_NORM)
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               H_NORM = MAX(H_NORM, ABS(HP(I,J,K)))
               H_DIFF_NORM = MAX(H_DIFF_NORM, ABS(HP(I,J,K) - H_PREV(I,J,K)))
            ENDDO
         ENDDO
      ENDDO
      !$OMP END PARALLEL DO
      ...
      IF (H_NORM > 1.0E-10_EB) THEN
         H_CHANGE = H_DIFF_NORM / H_NORM
      ELSE
         H_CHANGE = 0._EB
      ENDIF
      
      IF (H_CHANGE < PRESSURE_BLEND_H_THRESHOLD) THEN
         ALPHA_BLEND = 1.0_EB  ! No damping needed
      ELSE
         ALPHA_BLEND = PRESSURE_BLEND_ALPHA_MIN + (1.0_EB - PRESSURE_BLEND_ALPHA_MIN) * &
                       PRESSURE_BLEND_H_THRESHOLD / H_CHANGE
      ENDIF
      
      ! Store for diagnostics
      PRESSURE_BLEND_ALPHA_CURRENT = ALPHA_BLEND
      
      ! Apply blending
      IF (ALPHA_BLEND < 1.0_EB) THEN
         !$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I)
         DO K=1,KBAR
            DO J=1,JBAR
               DO I=1,IBAR
                  HP(I,J,K) = ALPHA_BLEND * HP(I,J,K) + (1.0_EB - ALPHA_BLEND) * H_PREV(I,J,K)
               ENDDO
            ENDDO
         ENDDO
         !$OMP END PARALLEL DO
   ...
   ELSE IF (PRESSURE_BLEND_FACTOR < 1.0_EB) THEN
      ! Fixed blending (legacy mode)
      !$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I)
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               HP(I,J,K) = PRESSURE_BLEND_FACTOR * HP(I,J,K) + (1.0_EB - PRESSURE_BLEND_FACTOR) * H_PREV(I,J,K)
            ENDDO
         ENDDO
      ENDDO
      !$OMP END PARALLEL DO
      PRESSURE_BLEND_ALPHA_CURRENT = PRESSURE_BLEND_FACTOR
   ENDIF

   ! Save current pressure for next step (for blending)
   !$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            H_PREV(I,J,K) = HP(I,J,K)
         ENDDO
      ENDDO
   ENDDO
   !$OMP END PARALLEL DO

   ! Pressure floor: prevent negative total pressure
   ! Total pressure: p_full = PBAR(K,PZ) + RHOP * HP
   ! Requirement: p_full >= PRESSURE_FLOOR_FACTOR * P_0(K)
   IF (PRESSURE_FLOOR_ENABLED) THEN
      !$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I,H_MIN)
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               IF (RHOP(I,J,K) > 1.0E-6_EB) THEN
                  H_MIN = (PRESSURE_FLOOR_FACTOR * P_0(K) - PBAR(K,PRESSURE_ZONE(I,J,K))) / RHOP(I,J,K)
                  IF (HP(I,J,K) < H_MIN) THEN
                     HP(I,J,K) = H_MIN
                     !$OMP ATOMIC
                     PRESSURE_FLOOR_CLIP_COUNT = PRESSURE_FLOOR_CLIP_COUNT + 1
                  ENDIF
               ENDIF
            ENDDO
         ENDDO
...
! ============================================================================
! Adaptive wrapper over FFT pressure solver.
! If the RHS of Poisson's equation changes smoothly between time steps,
! the solution is extrapolated from previous steps (O(N), faster than FFT).
! If the change is abrupt, falls back to the full FFT solve.
! After extrapolation, ||div(v)|| is checked; if it exceeds tolerance,
! the extrapolation is discarded and FFT is used instead.
! After 3 consecutive extrapolation failures, extrapolation is permanently
! disabled for the remainder of the simulation.
!
! Storage: SOAP_PRHS_PREV = PRHS^n  (saved for next step)
!          SOAP_HP_PREV   = H^n     (saved for next step, used in extrapolation)
! These are dedicated arrays allocated in init.f90 when PRES_METHOD=='SOAP'.
! ============================================================================

SUBROUTINE PRESSURE_SOLVER_SOAP(T,NM)
...
REAL(EB), POINTER, DIMENSION(:) :: UWP=>NULL()
INTEGER :: I,J,K,IW,IOR,NOM,N_INT_CELLS,IIO,JJO,KKO
REAL(EB) :: TRM1,TRM2,TRM3,TRM4,RFODT,TNOW,DUMMY=0._EB,TSI,TIME_RAMP_FACTOR, &
            DX_OTHER,DY_OTHER,DZ_OTHER,P_EXTERNAL,H_OTHER,DUUDT,DVVDT,DWWDT,WGT, &
            DX_OFF,DY_OFF,DZ_OFF,H_CORR  ! Phase 4: gradient correction
REAL(EB) :: PRHS_NORM,PRHS_DIFF_NORM,SOAP_INDICATOR,EXTRAP_FACTOR,DDDT_VAL,DIV_NORM
REAL(EB) :: H_MIN
LOGICAL :: SKIP_SOLVE,DID_EXTRAP
TYPE (VENTS_TYPE), POINTER :: VT=>NULL()
REAL(EB), PARAMETER :: TWO_EPS = 2.0_EB*EPSILON(1.0_EB)
...
! === STEP 1: Apply pressure boundary conditions (identical to PRESSURE_SOLVER) ===

!$OMP PARALLEL
!$OMP DO PRIVATE(IW,I,J,K,IOR,DUUDT,DVVDT,DWWDT,NOM,H_OTHER,KKO,JJO,IIO,N_INT_CELLS,DX_OTHER,DY_OTHER,DZ_OTHER,VT,TSI,TIME_RAMP_FACTOR,P_EXTERNAL,WGT)
WALL_CELL_LOOP: DO IW=1,NEWC
   I   = IJKW(1,IW)
   J   = IJKW(2,IW)
   K   = IJKW(3,IW)
   ...
   IF_DIRICHLET: IF (PRESSURE_BC_INDEX(IW)==DIRICHLET) THEN
      NOT_OPEN: IF (BOUNDARY_TYPE(IW)/=OPEN_BOUNDARY .AND. BOUNDARY_TYPE(IW)/=INTERPOLATED_BOUNDARY) THEN
         SELECT CASE(IOR)
            CASE( 1)
               IF (PREDICTOR) THEN; DUUDT =       RFODT*(-UWP(IW)-        U(0,J,K)           )
               ELSE;                DUUDT = 2._EB*RFODT*(-UWP(IW)-0.5_EB*(U(0,J,K)+US(0,J,K))); ENDIF
               BXS(J,K) = HP(1,J,K)     + 0.5_EB*DX(0)   *(DUUDT+FVX(0,J,K))
            CASE(-1)
               IF (PREDICTOR) THEN; DUUDT =       RFODT*( UWP(IW)-        U(IBAR,J,K)              )
               ELSE;                DUUDT = 2._EB*RFODT*( UWP(IW)-0.5_EB*(U(IBAR,J,K)+US(IBAR,J,K))); ENDIF
               BXF(J,K) = HP(IBAR,J,K) - 0.5_EB*DX(IBP1)*(DUUDT+FVX(IBAR,J,K))
            CASE( 2)
               IF (PREDICTOR) THEN; DVVDT =       RFODT*(-UWP(IW)-        V(I,0,K)           )
               ELSE;                DVVDT = 2._EB*RFODT*(-UWP(IW)-0.5_EB*(V(I,0,K)+VS(I,0,K))); ENDIF
               BYS(I,K) = HP(I,1,K)    + 0.5_EB*DY(0)   *(DVVDT+FVY(I,0,K))
            CASE(-2)
               IF (PREDICTOR) THEN; DVVDT =       RFODT*( UWP(IW)-        V(I,JBAR,K)              )
               ELSE;                DVVDT = 2._EB*RFODT*( UWP(IW)-0.5_EB*(V(I,JBAR,K)+VS(I,JBAR,K))); ENDIF
               BYF(I,K) = HP(I,JBAR,K) - 0.5_EB*DY(JBP1)*(DVVDT+FVY(I,JBAR,K))
            CASE( 3)
               IF (PREDICTOR) THEN; DWWDT =       RFODT*(-UWP(IW)-        W(I,J,0)           )
               ELSE;                DWWDT = 2._EB*RFODT*(-UWP(IW)-0.5_EB*(W(I,J,0)+WS(I,J,0))); ENDIF
               BZS(I,J) = HP(I,J,1)    + 0.5_EB*DZ(0)   *(DWWDT+FVZ(I,J,0))
            CASE(-3)
               IF (PREDICTOR) THEN; DWWDT =       RFODT*( UWP(IW)-        W(I,J,KBAR)              )
               ELSE;                DWWDT = 2._EB*RFODT*( UWP(IW)-0.5_EB*(W(I,J,KBAR)+WS(I,J,KBAR))); ENDIF
               BZF(I,J) = HP(I,J,KBAR) - 0.5_EB*DZ(KBP1)*(DWWDT+FVZ(I,J,KBAR))
         END SELECT
      ENDIF NOT_OPEN
      ...
      INTERPOLATED_ONLY: IF (BOUNDARY_TYPE(IW)==INTERPOLATED_BOUNDARY) THEN
         ...
         ENDDO
         SELECT CASE(IOR)
            CASE( 1); DX_OTHER = MESHES(NOM)%DX(IJKW(10,IW)); BXS(J,K) = (DX_OTHER*HP(1,J,K)     + DX(1)   *H_OTHER)/(DX(1)   +DX_OTHER) + WALL_WORK1(IW)
            CASE(-1); DX_OTHER = MESHES(NOM)%DX(IJKW(10,IW)); BXF(J,K) = (DX_OTHER*HP(IBAR,J,K)  + DX(IBAR)*H_OTHER)/(DX(IBAR)+DX_OTHER) + WALL_WORK1(IW)
            CASE( 2); DY_OTHER = MESHES(NOM)%DY(IJKW(11,IW)); BYS(I,K) = (DY_OTHER*HP(I,1,K)     + DY(1)   *H_OTHER)/(DY(1)   +DY_OTHER) + WALL_WORK1(IW)
            CASE(-2); DY_OTHER = MESHES(NOM)%DY(IJKW(11,IW)); BYF(I,K) = (DY_OTHER*HP(I,JBAR,K)  + DY(JBAR)*H_OTHER)/(DY(JBAR)+DY_OTHER) + WALL_WORK1(IW)
            CASE( 3); DZ_OTHER = MESHES(NOM)%DZ(IJKW(12,IW)); BZS(I,J) = (DZ_OTHER*HP(I,J,1)     + DZ(1)   *H_OTHER)/(DZ(1)   +DZ_OTHER) + WALL_WORK1(IW)
            CASE(-3); DZ_OTHER = MESHES(NOM)%DZ(IJKW(12,IW)); BZF(I,J) = (DZ_OTHER*HP(I,J,KBAR)  + DZ(KBAR)*H_OTHER)/(DZ(KBAR)+DZ_OTHER) + WALL_WORK1(IW)
         END SELECT
      ENDIF INTERPOLATED_ONLY
      ...
      OPEN_IF: IF (BOUNDARY_TYPE(IW)==OPEN_BOUNDARY) THEN
         IF (VENT_INDEX(IW)>0) THEN
            VT => VENTS(VENT_INDEX(IW))
            IF (TW(IW) == T_BEGIN .AND. VT%PRESSURE_RAMP_INDEX >=1) THEN; TSI = T
            ELSE;                                                         TSI = T - T_BEGIN; ENDIF
            TIME_RAMP_FACTOR = EVALUATE_RAMP(TSI,DUMMY,VT%PRESSURE_RAMP_INDEX)
            P_EXTERNAL = TIME_RAMP_FACTOR*VT%DYNAMIC_PRESSURE
         ENDIF
         SELECT CASE(IOR)
            CASE( 1); IF (UU(0,J,K)<0._EB)    THEN; BXS(J,K) = P_EXTERNAL/RHO_F(IW) + KRES(1,J,K)
                   ELSE;                                BXS(J,K) = P_EXTERNAL/RHO_F(IW) + H0; ENDIF
            CASE(-1); IF (UU(IBAR,J,K)>0._EB) THEN; BXF(J,K) = P_EXTERNAL/RHO_F(IW) + KRES(IBAR,J,K)
                   ELSE;                                BXF(J,K) = P_EXTERNAL/RHO_F(IW) + H0; ENDIF
            CASE( 2); IF (VV(I,0,K)<0._EB)    THEN; BYS(I,K) = P_EXTERNAL/RHO_F(IW) + KRES(I,1,K)
                   ELSE;                                BYS(I,K) = P_EXTERNAL/RHO_F(IW) + H0; ENDIF
            CASE(-2); IF (VV(I,JBAR,K)>0._EB) THEN; BYF(I,K) = P_EXTERNAL/RHO_F(IW) + KRES(I,JBAR,K)
                   ELSE;                                BYF(I,K) = P_EXTERNAL/RHO_F(IW) + H0; ENDIF
            CASE( 3); IF (WW(I,J,0)<0._EB)    THEN; BZS(I,J) = P_EXTERNAL/RHO_F(IW) + KRES(I,J,1)
                   ELSE;                                BZS(I,J) = P_EXTERNAL/RHO_F(IW) + H0; ENDIF
            CASE(-3); IF (WW(I,J,KBAR)>0._EB) THEN; BZF(I,J) = P_EXTERNAL/RHO_F(IW) + KRES(I,J,KBAR)
                   ELSE;                                BZF(I,J) = P_EXTERNAL/RHO_F(IW) + H0; ENDIF
         END SELECT
      ENDIF OPEN_IF
   ENDIF IF_DIRICHLET
ENDDO WALL_CELL_LOOP
...
! === STEP 2: Compute RHS of Poisson equation (identical to PRESSURE_SOLVER) ===

SELECT CASE(IPS)
   CASE(:1,4,7)
      IF (CYLINDRICAL) THEN
         !$OMP DO COLLAPSE(2) PRIVATE(K,I,TRM1,TRM3,TRM4)
         DO K=1,KBAR; DO I=1,IBAR
            TRM1 = (R(I-1)*FVX(I-1,1,K)-R(I)*FVX(I,1,K))*RDX(I)*RRN(I)
            TRM3 = (FVZ(I,1,K-1)-FVZ(I,1,K))*RDZ(K)
            TRM4 = -DDDT(I,1,K)
            PRHS(I,1,K) = TRM1 + TRM3 + TRM4
...
! === STEP 3: Compute SOAP smoothness indicator ===
! SOAP_INDICATOR = ||PRHS^n - PRHS^(n-1)||_inf / ||PRHS^n||_inf
! PRHS^(n-1) is stored in WORK7 from the previous call.

PRHS_NORM      = 0._EB
PRHS_DIFF_NORM = 0._EB
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         ...
         IF (.NOT. SOAP_FIRST_CALL) PRHS_DIFF_NORM = MAX(PRHS_DIFF_NORM, ABS(PRHS(I,J,K) - SOAP_PRHS_PREV(I,J,K)))
      ENDDO
   ENDDO
ENDDO
!$OMP END PARALLEL DO

IF (.NOT. SOAP_FIRST_CALL .AND. PRHS_NORM > TWO_EPS) THEN
   SOAP_INDICATOR = PRHS_DIFF_NORM / PRHS_NORM
ELSE
   SOAP_INDICATOR = 2._EB  ! Force FFT on first step and when RHS is negligible
ENDIF

! === STEP 4: Decide whether to extrapolate or solve ===
SKIP_SOLVE = (.NOT. SOAP_EXTRAP_DISABLED) .AND. (SOAP_INDICATOR < SOAP_THRESHOLD_SAVE)
DID_EXTRAP = .FALSE.

IF (SKIP_SOLVE) THEN
   ! --- Extrapolation path ---
   DID_EXTRAP = .TRUE.
   EXTRAP_FACTOR = SOAP_EXTRAP_ALPHA_SAVE * (1._EB - SOAP_INDICATOR / SOAP_THRESHOLD_SAVE)
   EXTRAP_FACTOR = MAX(0.1_EB, MIN(0.9_EB, EXTRAP_FACTOR))

   ! H^(n+1) ≈ H^n + factor × (H^n - H^(n-1))
   ! SOAP_HP_PREV holds HP from the previous call.
   !$OMP PARALLEL DO PRIVATE(I,J,K)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (.NOT. SOAP_FIRST_CALL) THEN
               HP(I,J,K) = HP(I,J,K) + EXTRAP_FACTOR * (HP(I,J,K) - SOAP_HP_PREV(I,J,K))
            ENDIF
         ENDDO
      ENDDO
   ENDDO
   ...
   IF (SOAP_DIV_CHECK) THEN
      DIV_NORM = 0._EB
      !$OMP PARALLEL DO PRIVATE(I,J,K,DDDT_VAL) REDUCTION(MAX:DIV_NORM)
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
      ...
      IF (DIV_NORM > SOAP_DIV_CHECK_TOLERANCE) THEN
         ! Extrapolation was bad — discard it and redo with FFT
         DID_EXTRAP = .FALSE.
         SOAP_FAIL_COUNT = SOAP_FAIL_COUNT + 1
         SOAP_DIV_EXCEEDED = .TRUE.
         IF (SOAP_FAIL_COUNT >= 3) SOAP_EXTRAP_DISABLED = .TRUE.
         SKIP_SOLVE = .FALSE.
      ELSE
         SOAP_FAIL_COUNT = 0
         SOAP_DIV_EXCEEDED = .FALSE.
      ENDIF
   ENDIF
ENDIF

! === STEP 5: Full FFT solve (executed when extrapolation is skipped or failed) ===
...
IF (.NOT. SKIP_SOLVE) THEN
   ...
   ! Copy FFT output (stored in PRHS) to HP
   SELECT CASE(IPS)
      CASE(:1,4,7)
         !$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I)
         DO K=1,KBAR; DO J=1,JBAR; DO I=1,IBAR
   ...
   SOAP_FFT_COUNT = SOAP_FFT_COUNT + 1
ELSE
   SOAP_EXTRAP_COUNT = SOAP_EXTRAP_COUNT + 1
ENDIF

! === STEP 6: Save PRHS^n and HP^n for next time step ===

!$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I)
DO K=1,KBAR; DO J=1,JBAR; DO I=1,IBAR
   SOAP_PRHS_PREV(I,J,K) = PRHS(I,J,K)
   SOAP_HP_PREV(I,J,K)   = HP(I,J,K)
ENDDO; ENDDO; ENDDO
!$OMP END PARALLEL DO
SOAP_FIRST_CALL = .FALSE.

! === STEP 6.5: Pressure floor - prevent negative total pressure ===
! Total pressure: p_full = PBAR(K,PZ) + RHOP * HP
! Requirement: p_full >= PRESSURE_FLOOR_FACTOR * P_0(K)
IF (PRESSURE_FLOOR_ENABLED) THEN
   !$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I,H_MIN)
...
! === STEP 7: Apply boundary conditions to H (identical to PRESSURE_SOLVER) ===
