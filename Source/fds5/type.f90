...
TYPE OMESH_TYPE
   ...
   REAL(EB), POINTER, DIMENSION(:,:,:) :: DHDX,DHDY,DHDZ  ! Phase 4: pressure gradients for 2nd-order interpolation
...
TYPE OBSTRUCTION_TYPE
   ...
   REAL(EB), DIMENSION(3) :: FDS_AREA=-1._EB,INPUT_AREA=-1._EB,FACE_AREA_FRACTION=1._EB
   REAL(EB), DIMENSION(6) :: XB_ORIG=0._EB
   ...
   LOGICAL :: SAWTOOTH=.TRUE.,HIDDEN=.FALSE.,PERMIT_HOLE=.TRUE.,ALLOW_VENT=.TRUE.,CONSUMABLE=.FALSE.,REMOVABLE=.FALSE., &
              THIN=.FALSE.,HOLE_FILLER=.FALSE.,SUB_CELL_THIN=.FALSE.,CUT_FACE=.TRUE.
   REAL(EB), DIMENSION(3) :: THIN_FRACTION=1._EB
   REAL(EB), DIMENSION(6) :: XB_INPUT=0._EB  ! Original XB before any snap/clip (for cut-face)
...
TYPE VENTS_TYPE
   ...
   REAL(EB), DIMENSION(3) :: FACE_AREA_FRACTION=1._EB
...
! R26 FLAME_SHEET (режим A): prescribed-очаги (VENT, SURF%HRRPUA>0) для размаза Q.
! R26e: вертикальные грани (IOR=±1,±2) регистрируются горизонтальной проекцией.
TYPE FS_BURNER_TYPE
   REAL(EB) :: X1,X2,Y1,Y2        ! прямоугольник footprint, глобальные координаты (м)
   REAL(EB) :: ZV=0._EB           ! z плоскости старта конуса (м)
   REAL(EB) :: QPP=0._EB          ! пиковая поверхностная мощность (Вт/м2)
   REAL(EB) :: AREA_EQ=-1._EB     ! R26e: истинная площадь верт. грани (м2); <0 = считать по footprint
   INTEGER  :: RAMP_INDEX=0       ! SF%RAMP_INDEX(TIME_HEAT)
   REAL(EB) :: TAU_Q=0._EB        ! SF%TAU(TIME_HEAT)
   REAL(EB) :: T_IGN=0._EB        ! время розжига (с)
END TYPE FS_BURNER_TYPE
TYPE(FS_BURNER_TYPE), DIMENSION(:), ALLOCATABLE :: FS_BURNERS
INTEGER :: N_FS_BURNERS=0
INTEGER :: N_FS_BURNERS_GLOBAL=0   ! R28b: сумма по рангам (корректный баннер при MPI)
