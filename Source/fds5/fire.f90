...
USE TYPES, ONLY: FS_BURNERS,N_FS_BURNERS,N_FS_BURNERS_GLOBAL
...
REAL(EB) :: Q_UPPER
REAL(EB) :: F_GOVERN  ! BUG#11: глобальный фактор губернатора HRR (1 = не активен)
REAL(EB) :: DZ_CAP=1._EB  ! BUG#11: локальный потолок доли выгорания ячейки за шаг
LOGICAL, SAVE :: FS_BANNER_DONE=.FALSE.    ! R26: признак режима A уже объявлен в .out
LOGICAL, SAVE :: FS_DEGRADE_WARNED=.FALSE. ! R26: warning о деградации колонны уже выдан

! R20 (порт FDS6 EDC): хранилище unmixedness-доли ZETA по ячейкам каждого меша.
! ZETA=1 — содержимое полностью несмешанное (горение невозможно, пока смешение
! не переведёт долю в смешанную зону: ZETA*=exp(-DT/TAU_MIX)). Ленивая аллокация.
TYPE ZETA_MESH_TYPE
   REAL(EB), DIMENSION(:,:,:), ALLOCATABLE :: Z
END TYPE ZETA_MESH_TYPE
TYPE(ZETA_MESH_TYPE), ALLOCATABLE, SAVE, DIMENSION(:) :: ZETA_STORE
...
! BUG#11 (ghost HRR): глобальный губернатор для прописанных очагов —
 ! суммарный HRR не может превышать 1.15 x (подача топлива x Hc).
 ! Фактор считается по HRR/MLR предыдущего шага (main.f90) и применяется
 ! к локальному Q до обновления концентраций (сохраняет консервативность).
F_GOVERN = 1._EB
IF (GOVERN_HRR) F_GOVERN = MIN(1._EB, HRR_CAP_GLOBAL/MAX(HRR_GLOBAL_PREV,1.E-10_EB))

! BUG#11: локальный анты-детонационный лимит (только для прописанных очагов):
! за один шаг сгорает <= DZ_CAP доступного топлива ячейки. При малом MIX_TIME
! (сильный сдвиг на фронте богатая-смесь/воздух) доля (1-exp(-DT/tau)) -> 1,
! ячейка сжигает весь запас за шаг -> скачок давления -> акустический обвал DT
! (Numerical Instability @329.9 c в 06558cad). Аналог пола unmixedness FDS6.
DZ_CAP = 1._EB
IF (GOVERN_HRR) DZ_CAP = 0.25_EB

! R26 FLAME_SHEET (режим A): prescribed Q вместо EDC — см. FDS5_FLAME_SHEET_DESIGN.md
IF (FLAME_SHEET .AND. .NOT.FS_BANNER_DONE) THEN
   IF (MYID==0) WRITE(LU_OUTPUT,'(A,I5,A,ES9.2,A,ES9.2,A)') &
...
IF (FLAME_SHEET) THEN
   CALL COMBUSTION_FLAME_SHEET
ELSE IF (MIXTURE_FRACTION) THEN
   CALL COMBUSTION_MF
ELSE
   IF (COMBUSTION2) THEN
      CALL COMBUSTION_FR2
...
REAL(EB) :: Y_FU_0,A,ETRM,Y_O2_0,Y_CO_0,DYF,DX_FDT,HFAC_F,DTT,DELTA,DELTA2,ACCEL, &
             ...
             Y_FU_MAX,TMP_F_MIN,Y_F_CORR,Z_2_MIN,Z_2_MIN_FAC,WGT,OMWGT,Q_BOUND_1,Q_BOUND_2,Q_BOUND_3,YY_GET(1:N_SPECIES), &
              ...
              ZMIX,BURN_F, & ! R20: unmixedness EDC
...
! R20: ленивая аллокация ZETA-хранилища (1.0 = всё несмешанное)
IF (.NOT.ALLOCATED(ZETA_STORE)) ALLOCATE(ZETA_STORE(NMESHES))
IF (.NOT.ALLOCATED(ZETA_STORE(NM)%Z)) &
   ALLOCATE(ZETA_STORE(NM)%Z(0:IBP1,0:JBP1,0:KBP1),SOURCE=1._EB)
...
!$OMP PRIVATE(TAU_D,TAU_U,TAU_G,EPSK,KSGS,ZMIX,BURN_F)
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         IC = CELL_INDEX(I,J,K)
         ...
         ! Two-mode combustion system:
         ! Mode 1 (CCC_FIXED_GLOBAL valid): MLR-based HRR with fixed CCC, bypass traditional suppression
         ! Mode 2 (CCC_FIXED_GLOBAL = -1): Original FDS5 combustion with ZETA, MIX_TIME, extinction
         IF_SUPPRESSION: IF (SUPPRESSION .AND. CCC_FIXED_GLOBAL < 0._EB) THEN  ! Original FDS5 suppression
...
ENDIF IF_SUPPRESSION
          ...
          ! Calculate DELTA (characteristic length) unconditionally for MIX_TIME
...
LES_IF: IF (LES) THEN
              EXPERIMENTAL_IF: IF (NEW_MIX_TIME) THEN
                 ! LES with NEW_MIX_TIME: use strain-based MIX_TIME
                 ...
                 EPSK = MU(I,J,K)/RHO(I,J,K)*SS2
                 KSGS = 2.25_EB*(EPSK*DELTA/PI)**TWTH
                 TAU_U = DELTA/SQRT(2._EB*KSGS+1.E-10_EB)
                 TAU_G = SQRT(2._EB*DELTA/(GRAV+1.E-10_EB))
                 MIX_TIME(I,J,K)=MIN(TAU_D,TAU_U,TAU_G)
              ELSE EXPERIMENTAL_IF
                 ! FDS 5 default MIX_TIME
                 MIX_TIME(I,J,K)=C_EDC*SC*RHO(I,J,K)*DELTA**2/MU(I,J,K)
              ENDIF EXPERIMENTAL_IF
ELSE
                ! Non-LES (transient): use FDS5 default MIX_TIME formula
                MIX_TIME(I,J,K)=C_EDC*SC*RHO(I,J,K)*DELTA**2/MU(I,J,K)
                ! Cap MIX_TIME at 1.0s to prevent unrealistically slow combustion on coarse meshes
                ! Without this, coarse meshes (Δ≈1m) give MIX_TIME ≈ 5000s → DT/MIX_TIME too small
             ENDIF LES_IF

              ! Ограничение MIX_TIME физическим временем плавучести (аналог FDS6: TAU_G=sqrt(2Δ/g))
              ! Устраняет «замирание» горения в ламинарных зонах, где μ молекулярная мала,
              ! а произвольный кап 10 с позволяет топливу накапливаться до взрывного выгорания.
              ! TAU_G обеспечивает физический масштаб перемешивания даже без турбулентности.
              LES_MIX_TIME_CAP: IF (LES .AND. .NOT.NEW_MIX_TIME) THEN
                 TAU_G = SQRT(2._EB*DELTA/(GRAV+1.E-10_EB))
                 MIX_TIME(I,J,K) = MIN(MIX_TIME(I,J,K), TAU_G)
              ENDIF LES_MIX_TIME_CAP

              NONLES_MIX_TIME_CAP: IF (.NOT.LES) THEN
                 TAU_G = SQRT(2._EB*DELTA/(GRAV+1.E-10_EB))
                 MIX_TIME(I,J,K) = MIN(MIX_TIME(I,J,K), TAU_G)
              ENDIF NONLES_MIX_TIME_CAP
...
S_L = LAMINAR_FLAME_SPEED(TMPA,RN%O2_F_RATIO/(Y_O2_0/Y_FU_0))
            ...
            R20_EDC_IF: IF (GOVERN_HRR) THEN
               ! R20 (порт FDS6 EDC): unmixedness-дроссель. Горит только смешанная
               ! доля содержимого ячейки: BURN_F = 1 - ZETA*exp(-DT/TAU_MIX).
               ! Смешение экспоненциально с физическим TAU_MIX -> рост HRR в каждой
               ! ячейке ограничен скоростью перемешивания: детонация невозможна,
               ! Q консервативно (строго из сгорающей массы). В стационарном плюме
               ! ZETA->0 и топливо горит у поверхности (компактное горячее ядро).
               ZMIX   = ZETA_STORE(NM)%Z(I,J,K)*EXP(-DT/MIX_TIME(I,J,K))
               BURN_F = 1._EB - ZMIX
               IF (Y_FU_0 < Y_O2_0/RN%O2_F_RATIO) THEN
                  DYF = Y_FU_0*BURN_F
               ELSE
                  DYF = Y_O2_0/RN%O2_F_RATIO*BURN_F
               ENDIF
               Q_BOUND_1 = DYF*RHO(I,J,K)*HFAC_F
               ZETA_STORE(NM)%Z(I,J,K) = ZMIX
            ELSE R20_EDC_IF
            NEW_DYF_IF: IF (NEW_MIX_TIME) THEN
               IF (Y_FU_0 < Y_O2_0/RN%O2_F_RATIO) THEN
                  DYF = Y_FU_0 * MIN(DZ_CAP, 1._EB -EXP(-DT/MIX_TIME(I,J,K)))
               ELSE
                  DYF = Y_O2_0/RN%O2_F_RATIO * MIN(DZ_CAP, 1._EB -EXP(-DT/MIX_TIME(I,J,K)))
               ENDIF
               Q_BOUND_1 = DYF*RHO(I,J,K)*HFAC_F
            ELSE NEW_DYF_IF
               ! FDS 5 default
               DYF = MIN(Y_FU_0,Y_O2_0/RN%O2_F_RATIO)
               Q_BOUND_1 = DYF*RHO(I,J,K)*HFAC_F*MIN(DZ_CAP,MIN(1._EB,DT/MIX_TIME(I,J,K)))
ENDIF NEW_DYF_IF
            ENDIF R20_EDC_IF

             Q_BOUND_2 = Q_UPPER
             Q_NEW = MIN(Q_BOUND_1,Q_BOUND_2)*F_GOVERN
            ! MLR-based HRR mode: HRR grows strictly from MLR via MIX_TIME limiting
            ! No additional procedural CCC needed - MIX_TIME limiting already handles combustion rate
            ! (Original FDS5: Q_NEW = Q_NEW * MIN(1, DT/MIX_TIME) in Q_BOUND_1 calculation)
            ...
            IF (CO_PRODUCTION) THEN
               ...
               IF (SOOT_INDEX > 0 .AND. I_PROG_SOOT > 0) THEN
                  YY(I,J,K,I_PROG_SOOT) = YY(I,J,K,I_PROG_SOOT) + DYF * RN%SOOT_YIELD
               ELSE
                  YY(I,J,K,I_PROG_CO) = YY(I,J,K,I_PROG_CO) + DYF * RN%SOOT_YIELD
               ENDIF
         ...
         DO I=1,IBAR
            ...
            ! Compute slow reaction
             ...
             IF (Q(I,J,K)/=0._EB) THEN
                DYCO = MIN(Y_CO_0,Y_O2_0/CO_TO_O2)*MIN(DZ_CAP,MIN(1._EB,DT/MIX_TIME(I,J,K)))
            ELSE
            ...
            ENDIF
             Q_OLD = Q(I,J,K)
             Q_NEW = MIN(Q_UPPER-Q_OLD,DYCO*RHO(I,J,K)*HFAC_CO)*F_GOVERN
...
SUBROUTINE COMBUSTION_FLAME_SHEET

! R26 FLAME_SHEET (режим A): prescribed Q = q''*RAMP*A, размаз по КОНУСУ над
! footprint (слой n расширяет footprint на n-1 колец чебышев — физичное
! расплывание плюма; заполнение снизу вверх с капом FLAME_SHEET_QCAP, объём
! конуса растёт до достаточного). Топливо конвертируется в products+soot в тех
! же ячейках БЕЗ тепла (Q уже prescribed): интеграл сжигания = интеграл
! впрыска -> ghost-накопление невозможно.

USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE PHYSICAL_FUNCTIONS, ONLY: GET_SPECIFIC_GAS_CONSTANT
INTEGER :: NB,I,J,K,N2,KSTART,NL,IR,IMIN,IMAX,JMIN,JMAX,KLAST
INTEGER :: QH,QT,QI,QJ,QK,NI,NJ,NK
INTEGER, SAVE, DIMENSION(:,:), ALLOCATABLE :: FS_QUE
LOGICAL, SAVE, DIMENSION(:,:,:), ALLOCATABLE :: FS_REACH
LOGICAL, SAVE, DIMENSION(:,:,:), ALLOCATABLE :: FS_TX,FS_TY,FS_TZ
INTEGER, SAVE, DIMENSION(3) :: FS_RDIM=0
REAL(EB), SAVE, DIMENSION(:,:,:), ALLOCATABLE :: FS_QFILT  ! R28b: состояние τ-фильтра конуса
INTEGER, SAVE, DIMENSION(4) :: FS_RDIMQ=0                  ! R28b: габариты+меш фильтра (сброс при смене меша)
LOGICAL, SAVE :: FS_BLOCK_WARNED=.FALSE.
INTEGER :: II2,JJ2,KK2
REAL(EB) :: TSI,RAMPV,OX,OY,AOV,A_EFF,TGT,TOT_AOV,TGT_M,MFUEL,BFRAC,DYF,VC,VOL,REM,QC,VTOT,DD
REAL(EB) :: QAPL_SUM,ALPH   ! R28b: приложенная мощность конуса, коэффициент фильтра
REAL(EB), ALLOCATABLE :: VL(:),VSUF(:)
INTEGER,  ALLOCATABLE :: KL(:)
REAL(EB) :: YY_GET(1:N_SPECIES)

Q = 0._EB
IF (N_FS_BURNERS==0) RETURN

RN => REACTION(1)
ALLOCATE(VL(0:KBAR+2),VSUF(0:KBAR+2),KL(0:KBAR+1))

BURNER_LOOP: DO NB=1,N_FS_BURNERS

   ! Конус стартует с плоскости вентиля: только меш, содержащий её (V1 — без
   ! кросс-меш продолжения). Вент на полу меша (ZS==ZV) принадлежит ЭТОМУ мешу
   IF (MESHES(NM)%ZS>FS_BURNERS(NB)%ZV+1.E-6_EB .OR. MESHES(NM)%ZF<=FS_BURNERS(NB)%ZV+1.E-6_EB) &
      CYCLE BURNER_LOOP

   TSI = T - FS_BURNERS(NB)%T_IGN
   IF (TSI<0._EB) CYCLE BURNER_LOOP
   RAMPV = 1._EB
   IF (FS_BURNERS(NB)%RAMP_INDEX>=1) RAMPV = EVALUATE_RAMP(TSI,FS_BURNERS(NB)%TAU_Q,FS_BURNERS(NB)%RAMP_INDEX)
   IF (RAMPV<=0._EB) CYCLE BURNER_LOOP

   ! --- Границы footprint в этом меше + точная площадь перекрытия ---
   IMIN=IBAR+1; IMAX=0; JMIN=JBAR+1; JMAX=0; TOT_AOV=0._EB
   DO J=1,JBAR
      OY = MIN(FS_BURNERS(NB)%Y2,YC(J)+0.5_EB*DY(J)) - MAX(FS_BURNERS(NB)%Y1,YC(J)-0.5_EB*DY(J))
      IF (OY<=0._EB) CYCLE
      DO I=1,IBAR
         OX = MIN(FS_BURNERS(NB)%X2,XC(I)+0.5_EB*DX(I)) - MAX(FS_BURNERS(NB)%X1,XC(I)-0.5_EB*DX(I))
         IF (OX<=0._EB) CYCLE
         TOT_AOV = TOT_AOV + OX*OY
         IMIN=MIN(IMIN,I); IMAX=MAX(IMAX,I); JMIN=MIN(JMIN,J); JMAX=MAX(JMAX,J)
      ENDDO
   ENDDO
   IF (TOT_AOV<=0._EB) CYCLE BURNER_LOOP

   KSTART = 0
   DO K=1,KBAR
      IF (Z(K-1)>=FS_BURNERS(NB)%ZV-1.E-6_EB) THEN
         KSTART = K
         EXIT
      ENDIF
   ENDDO
   IF (KSTART==0) CYCLE BURNER_LOOP

   ! R26h: маска связности (BFS по газовым клеткам от footprint). Геометрические
   ! кольца Чебышева расставляли тепло и за стенами — в соседних помещениях
   ! (кейс 46c08148, видно в SmokeView как прямоугольник HRRPUV сквозь стены).
   ! Теперь конус греет только газ, достижимый из footprint. Пересчёт на каждом
   ! шаге дёшев (BURN_AWAY меняет SOLID); на суммарную мощность не влияет (TGT
   ! тот же, меняется только геометрия размещения).
   DO  ! подъём по собственной колонне до первого газового слоя (крышка над вентом)
      VOL = 0._EB
      DO J=JMIN,JMAX
         DO I=IMIN,IMAX
            IF (.NOT.SOLID(CELL_INDEX(I,J,KSTART))) VOL = VOL + 1._EB
         ENDDO
      ENDDO
      IF (VOL>0._EB .OR. KSTART>=KBAR) EXIT
      KSTART = KSTART + 1
   ENDDO
     IF (ANY(FS_RDIM/=(/IBP1,JBP1,KBP1/))) THEN
        IF (ALLOCATED(FS_REACH)) DEALLOCATE(FS_REACH)
        IF (ALLOCATED(FS_TX)) DEALLOCATE(FS_TX)
        IF (ALLOCATED(FS_TY)) DEALLOCATE(FS_TY)
        IF (ALLOCATED(FS_TZ)) DEALLOCATE(FS_TZ)
        IF (ALLOCATED(FS_QUE))  DEALLOCATE(FS_QUE)
        ALLOCATE(FS_REACH(0:IBP1,0:JBP1,0:KBP1))
        ALLOCATE(FS_TX(0:IBP1,0:JBP1,0:KBP1),FS_TY(0:IBP1,0:JBP1,0:KBP1),FS_TZ(0:IBP1,0:JBP1,0:KBP1))
        ALLOCATE(FS_QUE(3,MAX(1,IBP1*JBP1*KBP1)))
        FS_RDIM = (/IBP1,JBP1,KBP1/)
     ENDIF
     ! R28b: буфер под состояние τ-фильтра (пересоздание при смене габаритов или меша)
     IF (ANY(FS_RDIMQ/=(/IBP1,JBP1,KBP1,NM/))) THEN
        IF (ALLOCATED(FS_QFILT)) DEALLOCATE(FS_QFILT)
        ALLOCATE(FS_QFILT(0:IBP1,0:JBP1,0:KBP1))
        FS_QFILT = 0._EB
        FS_RDIMQ = (/IBP1,JBP1,KBP1,NM/)
     ENDIF
    FS_REACH = .FALSE.

    ! R27h: маски тонких OBST по граням (закраска прямоугольников). Без них BFS
    ! конуса проходит сквозь тонкие стены (в т.ч. синтетические R27b у среза
    ! авто-зоны) и греет фантомным Q за запечатанной границей зоны.

    FS_TX = .FALSE.; FS_TY = .FALSE.; FS_TZ = .FALSE.
    DO N2=1,N_OBST
       IF (OBSTRUCTION(N2)%I1==OBSTRUCTION(N2)%I2) THEN
          DO KK2=MAX(1,OBSTRUCTION(N2)%K1+1),MIN(KBAR,OBSTRUCTION(N2)%K2)
             DO JJ2=MAX(1,OBSTRUCTION(N2)%J1+1),MIN(JBAR,OBSTRUCTION(N2)%J2)
                FS_TX(OBSTRUCTION(N2)%I1,JJ2,KK2) = .TRUE.
             ENDDO
          ENDDO
       ELSEIF (OBSTRUCTION(N2)%J1==OBSTRUCTION(N2)%J2) THEN
          DO KK2=MAX(1,OBSTRUCTION(N2)%K1+1),MIN(KBAR,OBSTRUCTION(N2)%K2)
             DO II2=MAX(1,OBSTRUCTION(N2)%I1+1),MIN(IBAR,OBSTRUCTION(N2)%I2)
                FS_TY(II2,OBSTRUCTION(N2)%J1,KK2) = .TRUE.
             ENDDO
          ENDDO
       ELSEIF (OBSTRUCTION(N2)%K1==OBSTRUCTION(N2)%K2) THEN
          DO JJ2=MAX(1,OBSTRUCTION(N2)%J1+1),MIN(JBAR,OBSTRUCTION(N2)%J2)
             DO II2=MAX(1,OBSTRUCTION(N2)%I1+1),MIN(IBAR,OBSTRUCTION(N2)%I2)
                FS_TZ(II2,JJ2,OBSTRUCTION(N2)%K1) = .TRUE.
             ENDDO
          ENDDO
       ENDIF
    ENDDO
   ...
   DO J=JMIN,JMAX
      DO I=IMIN,IMAX
         IF (SOLID(CELL_INDEX(I,J,KSTART))) CYCLE
         FS_REACH(I,J,KSTART) = .TRUE.
         QT = QT + 1
         FS_QUE(1,QT) = I; FS_QUE(2,QT) = J; FS_QUE(3,QT) = KSTART
      ENDDO
   ENDDO
    DO WHILE (QH<QT)
       QH = QH + 1
       QI = FS_QUE(1,QH); QJ = FS_QUE(2,QH); QK = FS_QUE(3,QH)
       NI = QI+1; NJ = QJ;   NK = QK
       IF (NI<=IBAR .AND. .NOT.FS_TX(QI,QJ,QK) .AND. .NOT.SOLID(CELL_INDEX(NI,NJ,NK)) .AND. .NOT.FS_REACH(NI,NJ,NK)) THEN
          FS_REACH(NI,NJ,NK) = .TRUE.; QT = QT+1; FS_QUE(1,QT)=NI; FS_QUE(2,QT)=NJ; FS_QUE(3,QT)=NK
       ENDIF
       NI = QI-1
       IF (NI>=1 .AND. .NOT.FS_TX(NI,QJ,QK) .AND. .NOT.SOLID(CELL_INDEX(NI,NJ,NK)) .AND. .NOT.FS_REACH(NI,NJ,NK)) THEN
          FS_REACH(NI,NJ,NK) = .TRUE.; QT = QT+1; FS_QUE(1,QT)=NI; FS_QUE(2,QT)=NJ; FS_QUE(3,QT)=NK
       ENDIF
       NI = QI; NJ = QJ+1
       IF (NJ<=JBAR .AND. .NOT.FS_TY(QI,QJ,QK) .AND. .NOT.SOLID(CELL_INDEX(NI,NJ,NK)) .AND. .NOT.FS_REACH(NI,NJ,NK)) THEN
          FS_REACH(NI,NJ,NK) = .TRUE.; QT = QT+1; FS_QUE(1,QT)=NI; FS_QUE(2,QT)=NJ; FS_QUE(3,QT)=NK
       ENDIF
       NJ = QJ-1
       IF (NJ>=1 .AND. .NOT.FS_TY(QI,NJ,QK) .AND. .NOT.SOLID(CELL_INDEX(NI,NJ,NK)) .AND. .NOT.FS_REACH(NI,NJ,NK)) THEN
          FS_REACH(NI,NJ,NK) = .TRUE.; QT = QT+1; FS_QUE(1,QT)=NI; FS_QUE(2,QT)=NJ; FS_QUE(3,QT)=NK
       ENDIF
       NJ = QJ; NK = QK+1
       IF (NK<=KBAR .AND. .NOT.FS_TZ(QI,QJ,QK) .AND. .NOT.SOLID(CELL_INDEX(NI,NJ,NK)) .AND. .NOT.FS_REACH(NI,NJ,NK)) THEN
          FS_REACH(NI,NJ,NK) = .TRUE.; QT = QT+1; FS_QUE(1,QT)=NI; FS_QUE(2,QT)=NJ; FS_QUE(3,QT)=NK
       ENDIF
       NK = QK-1
       IF (NK>=KSTART .AND. .NOT.FS_TZ(QI,QJ,NK) .AND. .NOT.SOLID(CELL_INDEX(NI,NJ,NK)) .AND. .NOT.FS_REACH(NI,NJ,NK)) THEN
          FS_REACH(NI,NJ,NK) = .TRUE.; QT = QT+1; FS_QUE(1,QT)=NI; FS_QUE(2,QT)=NJ; FS_QUE(3,QT)=NK
       ENDIF
    ENDDO
   IF (QT==0) THEN
      ! очаг полностью замурован: тепло не размещается (предупреждение однократно)
      IF (MYID==0 .AND. .NOT.FS_BLOCK_WARNED) THEN
         WRITE(LU_OUTPUT,'(A)') ' WARNING: FLAME_SHEET burner fully blocked by solids - heat skipped'
         FS_BLOCK_WARNED = .TRUE.
      ENDIF
      CYCLE BURNER_LOOP
   ENDIF

   ! R26e: для вертикальных граней мощность берётся по истинной площади AREA_EQ,
   ! а не по площади горизонтальной проекции footprint (сохранение суммарной мощности)
   A_EFF = TOT_AOV
   IF (FS_BURNERS(NB)%AREA_EQ>0._EB) A_EFF = FS_BURNERS(NB)%AREA_EQ
   TGT = FS_BURNERS(NB)%QPP*RAMPV*A_EFF

   ! --- Перечисление слоёв конуса (снизу вверх) до достаточного объёма ---
   ! R26g: полностью сплошной слой трактуется как крышка — конус НЕ проникает
   ! сквозь неё в изолированные полости (кейс 46c08148: перечисление сквозь
   ! плиту подшивного потолка грело однослойную полость над ней, тепловой
   ! разгон замкнутой щели давал струи ~60 м/с и клин DT@~309 с). Кольца
   ! продолжают расползаться в последнем газовом слое ПОД крышкой.
   NL = 0
   VTOT = 0._EB
   KLAST = 0
   LAYER_ENUM: DO
      K = KSTART + NL
      IF (K>KBAR) EXIT LAYER_ENUM
      IF (FLAME_SHEET_HMAX>0._EB .AND. Z(K-1)>FS_BURNERS(NB)%ZV+FLAME_SHEET_HMAX) EXIT LAYER_ENUM
      IR = NL
      VOL = 0._EB
       DO J=MAX(1,JMIN-IR),MIN(JBAR,JMAX+IR)
          DO I=MAX(1,IMIN-IR),MIN(IBAR,IMAX+IR)
             DD = MAX(IMIN-I,I-IMAX,JMIN-J,J-JMAX,0)
             IF (DD/=IR) CYCLE
             IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
             IF (.NOT.FS_REACH(I,J,K)) CYCLE   ! R26h: только связанный с очагом газ
             VOL = VOL + DX(I)*RC(I)*DY(J)*DZ(K)
          ENDDO
       ENDDO
       IF (VOL<=0._EB .AND. KLAST>0) THEN
          ! крышка: повторить то же кольцо в последнем газовом слое под ней
          K = KLAST
          DO J=MAX(1,JMIN-IR),MIN(JBAR,JMAX+IR)
             DO I=MAX(1,IMIN-IR),MIN(IBAR,IMAX+IR)
                DD = MAX(IMIN-I,I-IMAX,JMIN-J,J-JMAX,0)
                IF (DD/=IR) CYCLE
                IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
                IF (.NOT.FS_REACH(I,J,K)) CYCLE   ! R26h
                VOL = VOL + DX(I)*RC(I)*DY(J)*DZ(K)
             ENDDO
          ENDDO
         IF (VOL<=0._EB) EXIT LAYER_ENUM
      ELSEIF (VOL>0._EB) THEN
         KLAST = K
      ENDIF
      NL = NL + 1
      KL(NL) = K
      VL(NL) = VOL
      VTOT = VTOT + VOL
      IF (FLAME_SHEET_QCAP*VTOT>=TGT .AND. VOL>0._EB) EXIT LAYER_ENUM
   ENDDO LAYER_ENUM
   IF (NL==0 .OR. VTOT<=0._EB) CYCLE BURNER_LOOP

   ! --- Заполнение Q послойно (равномерно внутри слоя) + сбор топлива ---
   MFUEL = 0._EB
   IF (TGT<=FLAME_SHEET_QCAP*VTOT) THEN
      VSUF(NL+1) = 0._EB
      DO N2=NL,1,-1
         VSUF(N2) = VL(N2) + VSUF(N2+1)
      ENDDO
      REM = TGT
      DO N2=1,NL
         QC = MIN(FLAME_SHEET_QCAP,REM/MAX(VSUF(N2),1.E-12_EB))
         K = KL(N2)
         IR = N2 - 1
          DO J=MAX(1,JMIN-IR),MIN(JBAR,JMAX+IR)
             DO I=MAX(1,IMIN-IR),MIN(IBAR,IMAX+IR)
                ...
                Q(I,J,K) = Q(I,J,K) + QC
                MFUEL = MFUEL + RHO(I,J,K)*YY(I,J,K,I_FUEL)*RN%Y_F_INLET*VC
             ENDDO
          ENDDO
          REM = REM - QC*VL(N2)
       ENDDO
   ELSE
      ! деградация: весь объём конуса в меше исчерпан (низкое помещение)
      QC = TGT/VTOT
      DO N2=1,NL
         K = KL(N2)
         IR = N2 - 1
         ...
         ENDDO
      ENDDO
      IF (MYID==0 .AND. .NOT.FS_DEGRADE_WARNED) THEN
         WRITE(LU_OUTPUT,'(A,ES9.2,A)') ' WARNING: FLAME_SHEET cone volume exhausted, uniform Q=',QC, &
            ' W/m3 exceeds QCAP (room height limited by mesh top / solids)'
         FS_DEGRADE_WARNED = .TRUE.
      ENDIF
   ENDIF

    ! --- R28b: тепловая буферизация конуса (τ-фильтр первого порядка) ---
    ! Газ получает мощность через экспоненциальный фильтр FLAME_SHEET_TAU:
    ! Q_appl(n) = Q_appl(n-1) + (1-exp(-DT/tau))*(Q_tgt - Q_appl(n-1)).
    ! Ограничивает локальный dT/dt на крутых участках RAMP и при выходе на плато
    ! (142-с проблема: скачок лидера div на (34,55,13) при t~140.2 с). Массовая
    ! конверсия ниже считается от ФАКТИЧЕСКИ приложенной мощности (QAPL_SUM),
    ! поэтому интеграл сжигания соответствует теплу, реально переданному газу.
    ! При TAU=0 поведение тождественно R28a (QAPL_SUM = TGT).
    IF (FLAME_SHEET_TAU>0._EB) THEN
       ALPH = 1._EB - EXP(-DT/FLAME_SHEET_TAU)
       QAPL_SUM = 0._EB
       DO N2=1,NL
          K = KL(N2)
          IR = N2 - 1
          DO J=MAX(1,JMIN-IR),MIN(JBAR,JMAX+IR)
             DO I=MAX(1,IMIN-IR),MIN(IBAR,IMAX+IR)
                ...
                IF (DD/=IR) CYCLE
                IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
                IF (.NOT.FS_REACH(I,J,K)) CYCLE
                VC = DX(I)*RC(I)*DY(J)*DZ(K)
                FS_QFILT(I,J,K) = FS_QFILT(I,J,K) + ALPH*(Q(I,J,K)-FS_QFILT(I,J,K))
                Q(I,J,K) = FS_QFILT(I,J,K)
                QAPL_SUM = QAPL_SUM + Q(I,J,K)*VC
             ENDDO
          ENDDO
       ENDDO
    ELSE
    ...
    ! --- Пассивная конверсия топлива по тем же слоям (доля BFRAC локального) ---
    TGT_M = QAPL_SUM*RN%Y_F_INLET*DT/RN%HEAT_OF_COMBUSTION
   BFRAC = MIN(1._EB, TGT_M/MAX(MFUEL,1.E-12_EB))
   IF (BFRAC>0._EB) THEN
      DO N2=1,NL
         K = KL(N2)
         IR = N2 - 1
          DO J=MAX(1,JMIN-IR),MIN(JBAR,JMAX+IR)
             DO I=MAX(1,IMIN-IR),MIN(IBAR,IMAX+IR)
                ...
                IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
                IF (.NOT.FS_REACH(I,J,K)) CYCLE   ! R26h
                IF (YY(I,J,K,I_FUEL)<=1.E-10_EB) CYCLE
               DYF = BFRAC*YY(I,J,K,I_FUEL)*RN%Y_F_INLET
               YY(I,J,K,I_FUEL) = YY(I,J,K,I_FUEL) - DYF
               IF (SOOT_INDEX>0 .AND. I_PROG_SOOT>0) THEN
                  YY(I,J,K,I_PROG_F)    = YY(I,J,K,I_PROG_F)    + DYF*(1._EB-RN%SOOT_YIELD)
                  YY(I,J,K,I_PROG_SOOT) = YY(I,J,K,I_PROG_SOOT) + DYF*RN%SOOT_YIELD
...
! Пересчёт молекулярной массы смеси после конверсии (зеркально COMBUSTION_MF)
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         YY_GET(:) = YY(I,J,K,:)
...
DEALLOCATE(VL,VSUF,KL)

END SUBROUTINE COMBUSTION_FLAME_SHEET
