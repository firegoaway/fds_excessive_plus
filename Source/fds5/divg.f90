...
REAL(EB) :: DELKDELT,VC,DTDX,DTDY,DTDZ,TNOW, YSUM,YY_GET(1:N_SPECIES),ZZ_GET(1:N_MIX_SPECIES), &
            HDIFF,DYDX,DYDY,DYDZ,T,RDT,RHO_D_DYDN,TSI,VDOT_LEAK,TIME_RAMP_FACTOR,ZONE_VOLUME,CP_MF,DELTA_P,CB,XX,WW,PRES_RAMP_FACTOR,&
            H_G,TMP_G,TMP_WGT
TYPE(SURFACE_TYPE), POINTER :: SF
INTEGER :: IW,N,IOR,II,JJ,KK,IIG,JJG,KKG,ITMP,IBC,I,J,K,IPZ,IOPZ,N_LPZ
...
PRESSURE_ZONE_LOOP: DO IPZ=1,N_ZONE
   ...
   ! R26f: явная схема утечек устойчива при ОДНОМ пути на зону. У зоны может быть
   ! несколько путей (LEAK_AREA ко всем соседям + атмосфера): каждый путь
   ! ограничивался полным «нулевым» расходом |dP|*V/(gamma*P) за шаг, суммарный
   ! отток превышал бюджет в N_путей раз, переворачивал знак dP и раскачивал
   ! давления зон с ростом ~N за шаг (кейс 46c08148: +-0.016 атм на пожаре 10 кВт,
   ! струи утечки 10^2..10^3 м/с у дверного полотна LEAK_PATH, DT-спираль, NI).
   ! Фикс: бюджет зоны делится на число активных путей N_LPZ — суммарный отток
   ! не гасит больше 1/N доли перепада за шаг, знак dP не переворачивается.
   U_LEAK = 0._EB
   N_LPZ = 0
   DO IOPZ=0,N_ZONE
      IF (IOPZ/=IPZ .AND. FDS_LEAK_AREA(IPZ,IOPZ,NM)>0._EB) N_LPZ = N_LPZ + 1
   ENDDO
    OTHER_ZONE_LOOP: DO IOPZ=0,N_ZONE
       ...
       IF (FDS_LEAK_AREA(IPZ,IOPZ,NM) > 0._EB) THEN
          IF (N_LPZ==1) THEN
             ! R27g: полунеявный (backward Euler) орис для однопутевой зоны:
             ! dP/dt = (S - A*u(P))/PSUM, u = sign*sqrt(2|P|/rho), S = DSUM-USUM
             ! (утечка ещё не в USUM — WALL_LOOP4 ниже). Явная схема при PSUM<<DT
             ! звенит (релаксация ~1 мс против шага ~30 мс, кейс c45daa57:
             ! D_PBAR_DT +-10..77 1/с). Неявный корень w^2 + B*w - X = 0:
             ! w = sqrt(B^2/4 + X) - B/2, X = dP + (DT/PSUM)*S, B = (DT/PSUM)*A*sqrt(2/rho)
             CB = DT*ACTUAL_LEAK_AREA(IPZ,IOPZ)*SQRT(2._EB/RHOA)/PSUM(IPZ,NM)
             XX = DELTA_P + DT*(DSUM(IPZ,NM)-USUM(IPZ,NM))/PSUM(IPZ,NM)
             IF (XX>=0._EB) THEN
                WW = SQRT(0.25_EB*CB*CB + XX) - 0.5_EB*CB
                VDOT_LEAK = ACTUAL_LEAK_AREA(IPZ,IOPZ)*SQRT(2._EB/RHOA)*WW
             ELSE
                WW = SQRT(0.25_EB*CB*CB - XX) - 0.5_EB*CB
                VDOT_LEAK = -ACTUAL_LEAK_AREA(IPZ,IOPZ)*SQRT(2._EB/RHOA)*WW
             ENDIF
          ELSE
             VDOT_LEAK = SIGN(1._EB,DELTA_P)*MIN(ACTUAL_LEAK_AREA(IPZ,IOPZ)*SQRT(2._EB*ABS(DELTA_P)/RHOA), &
                                                 ABS(DELTA_P)*RDT*ZONE_VOLUME/(GAMMA*PBAR(1,IPZ)*REAL(MAX(N_LPZ,1),EB)))
          ENDIF
   ...
   WALL_LOOP4: DO IW=1,NWC
      ...
      IBC = IJKW(5,IW)
      SF=>SURFACE(IBC)
      ! R27j: утечка авто-зоны в атмосферу — ОДНОСТОРОННЯЯ (только со стороны
      ! зоны IPZ>0). Объёмная (не массовая) утечка FDS5 при горячей зоне
      ! создаёт фантомную массу (rho_amb-rho_zone)*VDOT в зоне 0 (кейс
      ! c45daa57: разгон струи 12 м/с у двери, H +-[100..200], NI @25.6).
      ! Атмосферная сторона молчит — объём абсорбируется OPEN-границей.
       IF (SF%LEAK_PATH(1)==IPZ) THEN
          IOPZ = SF%LEAK_PATH(2)
       ELSE
          IOPZ = SF%LEAK_PATH(1)
       ENDIF
       ! R27j-фикс: вложенные IF вместо .AND.-цепочки — Fortran не гарантирует
       ! короткого замыкания: при IOPZ<1 (невалидный LEAK_PATH=-1) чтение
       ! ZONE_IS_AUTO(MIN(IOPZ,N_ZONE)) и U_LEAK(IOPZ) — выход за границы.
       ! Поверхности с невалидным путём утечки пропускаются.
       IF (SF%LEAK_PATH(1)==IPZ .OR. SF%LEAK_PATH(2)==IPZ) THEN
          IF (PREDICTOR .AND. IOPZ>=0 .AND. IOPZ<=N_ZONE) THEN
             ! односторонняя утечка авто-зоны: со стороны IPZ>0 применяется всегда
             IF (IPZ>0) THEN
             ...
             ELSEIF (.NOT.ZONE_IS_AUTO(IOPZ)) THEN
                UWS(IW) = UWS(IW) + U_LEAK(IOPZ)
                IF (IW<=NEWC) DUWDT(IW) = RDT*(UWS(IW)-UW(IW))   ! DUWDT is needed by Poisson solver only at external boundary
             ENDIF
          ENDIF
...
! Apply volume factor correction for partial-volume cells (cut-face OBSTs)
! VOLUME_FACTOR < 1 means cell is partially blocked; divergence is amplified
!$OMP PARALLEL DO COLLAPSE(3) PRIVATE(K,J,I)
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
         IF (VOLUME_FACTOR(I,J,K) < 0.99_EB) THEN
            DP(I,J,K) = DP(I,J,K) / VOLUME_FACTOR(I,J,K)
         ENDIF
      ENDDO
   ENDDO
ENDDO
