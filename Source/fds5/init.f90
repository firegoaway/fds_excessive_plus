...
! SOAP solver arrays (allocated only when SOAP is active)
IF (PRES_METHOD == 'SOAP') THEN
   ALLOCATE(M%SOAP_PRHS_PREV(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','SOAP_PRHS_PREV',IZERO)
   M%SOAP_PRHS_PREV = 0._EB
   ALLOCATE(M%SOAP_HP_PREV(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','SOAP_HP_PREV',IZERO)
   M%SOAP_HP_PREV = 0._EB
   M%SOAP_FIRST_CALL = .TRUE.
ELSE
   NULLIFY(M%SOAP_PRHS_PREV)
   NULLIFY(M%SOAP_HP_PREV)
   M%SOAP_FIRST_CALL = .TRUE.
ENDIF
...
NOT_EVAC_IF_2: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   M%AREA_ADJUST = 1._EB
   ALLOCATE(M%ALPHA_CF_W(NDWC),STAT=IZERO)
   CALL ChkMemErr('INIT','ALPHA_CF_W',IZERO)
   M%ALPHA_CF_W = 1._EB
   ALLOCATE(M%CUT_FACE_FLAG(NDWC),STAT=IZERO)
   CALL ChkMemErr('INIT','CUT_FACE_FLAG',IZERO)
   M%CUT_FACE_FLAG = .FALSE.
   ALLOCATE(M%CFACE_CEN(3,NDWC),STAT=IZERO)
   CALL ChkMemErr('INIT','CFACE_CEN',IZERO)
   M%CFACE_CEN = 0._EB
...
OBST_LOOP_2: DO N=1,M%N_OBST
   ...
   DO K=OB%K1+1,OB%K2
      DO J=OB%J1+1,OB%J2
         ...
         IF (M%SOLID(IC)) CYCLE   ! Ghost cell inside another OBST — skip wall cell creation
         IOR = -1
         IBC = OB%IBC(IOR)
   ...
   DO K=OB%K1+1,OB%K2
      DO J=OB%J1+1,OB%J2
         I = OB%I2
         IF (OB%SUB_CELL_THIN .AND. OB%I1==OB%I2) CYCLE  ! Skip +X face for sub-cell thin OBST (handled by -X face)
   ...
   DO K=OB%K1+1,OB%K2
      DO I=OB%I1+1,OB%I2
         J = OB%J2
         IF (OB%SUB_CELL_THIN .AND. OB%J1==OB%J2) CYCLE  ! Skip +Y face for sub-cell thin OBST
   ...
   DO J=OB%J1+1,OB%J2
      DO I=OB%I1+1,OB%I2
         K = OB%K2
         IF (OB%SUB_CELL_THIN .AND. OB%K1==OB%K2) CYCLE  ! Skip +Z face for sub-cell thin OBST
      ...
      DO K=1,M%KBAR
         DO J=1,M%JBAR
            DO I=1,M%IBAR
               ...
               IF (N_SPECIES==0) THEN
                  ITMP = MAX(0, MIN(4999, INT(M%TMP(I,J,K))))
...
ALLOCATE(M%H_PREV(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
CALL ChkMemErr('INIT','H_PREV',IZERO)
M%H_PREV = 0._EB
IF (KBAR>1) THEN
...
IF (.NOT.CFL_FILE)  CFL_CLOCK  = 1.E10_EB
IF (.NOT.CPU_FILE)  CPU_CLOCK  = 1.E10_EB
...
ALLOCATE(H_ZONE_MEAN(0:N_ZONE,NMESHES),STAT=IZERO)   ! R26j
CALL ChkMemErr('INIT','H_ZONE_MEAN',IZERO)
H_ZONE_MEAN = 0._EB
...
USE GEOMETRY_FUNCTIONS, ONLY : SEARCH_OTHER_MESHES, IS_CELL_INSIDE_OBST
...
INTEGER  :: N,I_OBST,I,J,K,IBCX,IIG,JJG,KKG,IW,IIO,JJO,KKO,IC,ICG,ICO,NOM_CHECK(0:1)
INTEGER :: NSLICE
LOGICAL :: VENT_FOUND,ALIGNED
TYPE (MESH_TYPE), POINTER :: MM
TYPE (OBSTRUCTION_TYPE), POINTER :: OBX
TYPE (VENTS_TYPE), POINTER :: VT
TYPE (CUT_FACE_DATA) :: CF_LOCAL
...
CHECK_MESHES: IF (IW<=M%NEWC .AND.  .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   IF (NOM_CHECK(0)/=NOM_CHECK(1)) THEN
      IF (RELAXED_ALIGNMENT) THEN
         WRITE(LU_ERR,'(A,I3,A,I3)') 'WARNING: MESH ',NM,' wall cell spans multiple meshes (',MAXVAL(NOM_CHECK), &
                                     '). Using relaxed interpolation.'
      ELSE
   ...
   FOUND_OTHER_MESH: IF (NOM>0) THEN
      ...
      IF (.NOT.ALIGNED) THEN
         IF (RELAXED_ALIGNMENT) THEN
            WRITE(LU_ERR,'(A,I3,A,I3)') 'WARNING: MESH ',NM,' cell size not integer multiple of MESH ',NOM, &
      ...
      IF (.NOT.ALIGNED) THEN
         IF (RELAXED_ALIGNMENT) THEN
            WRITE(LU_ERR,'(A,I3,A,I3)') 'WARNING: MESH ',NM,' is finer in one direction and coarser in the other than MESH ',NOM, &
                                        '. Using area-weighted interpolation.'
         ELSE
...
! Compute cut-face fraction ALPHA_CF_W: fraction of wall cell face covered by geometry
! Used in NO_FLUX blending for stair-step and partial-cell boundaries

! FDS6: тонкие OBST точно на границе меша (z=0 и т.п.) дают фантомный wall-cell
! с индексом газовой клетки K=0 (или I/J=0, KBAR+1) — вычисление cut-face для него
! невозможно (Z(K-1)=Z(-1)); пропускаем, у фантомной грани нет газовой клетки
IF (IOR/=0 .AND. .NOT.EVACUATION_ONLY(NM) .AND. I_OBST>0 .AND. &
    I>=1 .AND. I<=M%IBAR .AND. J>=1 .AND. J<=M%JBAR .AND. K>=1 .AND. K<=M%KBAR) THEN

   OBX => M%OBSTRUCTION(I_OBST)

   IF (OBX%CUT_FACE) THEN
      ! Cut-face OBST: use exact XB_INPUT coordinates for precise geometry
      CALL COMPUTE_CUT_FACE(OBX%XB_INPUT, .TRUE., I, J, K, IOR, CF_LOCAL)
      M%ALPHA_CF_W(IW) = MAX(0._EB, MIN(1._EB, CF_LOCAL%AREA_FRAC))
      M%CUT_FACE_FLAG(IW) = .TRUE.
      M%CFACE_CEN(:,IW) = CF_LOCAL%XYZCEN
      ! Store volume factor for the gas-side cell adjacent to this wall face
      IF (IOR > 0) THEN
         M%VOLUME_FACTOR(I,J,K) = MIN(M%VOLUME_FACTOR(I,J,K), CF_LOCAL%VOLUME_FRAC)
      ELSE
         M%VOLUME_FACTOR(I,J,K) = MIN(M%VOLUME_FACTOR(I,J,K), CF_LOCAL%VOLUME_FRAC)
      ENDIF
   ELSEIF (OBX%SUB_CELL_THIN) THEN
      ! Sub-cell thin OBST: use volume fraction along thin direction
      M%ALPHA_CF_W(IW) = OBX%THIN_FRACTION(ABS(IOR))
   ELSE
      ! Normal OBST: use face intersection fraction
      SELECT CASE(ABS(IOR))
      CASE(1)
         Y1 = MAX(M%Y(J-1), OBX%Y1)
         Y2 = MIN(M%Y(J),   OBX%Y2)
         Z1 = MAX(M%Z(K-1), OBX%Z1)
         Z2 = MIN(M%Z(K),   OBX%Z2)
         IF (Y2>Y1 .AND. Z2>Z1) M%ALPHA_CF_W(IW) = (Y2-Y1)*(Z2-Z1)/(M%DY(J)*M%DZ(K))
      CASE(2)
         X1 = MAX(M%X(I-1), OBX%X1)
         X2 = MIN(M%X(I),   OBX%X2)
         Z1 = MAX(M%Z(K-1), OBX%Z1)
         Z2 = MIN(M%Z(K),   OBX%Z2)
         IF (X2>X1 .AND. Z2>Z1) M%ALPHA_CF_W(IW) = (X2-X1)*(Z2-Z1)/(M%DX(I)*M%DZ(K))
      CASE(3)
         X1 = MAX(M%X(I-1), OBX%X1)
         X2 = MIN(M%X(I),   OBX%X2)
         Y1 = MAX(M%Y(J-1), OBX%Y1)
         Y2 = MIN(M%Y(J),   OBX%Y2)
         IF (X2>X1 .AND. Y2>Y1) M%ALPHA_CF_W(IW) = (X2-X1)*(Y2-Y1)/(M%DX(I)*M%DY(J))
   END SELECT
   ENDIF
   M%ALPHA_CF_W(IW) = MAX(0._EB, MIN(1._EB, M%ALPHA_CF_W(IW)))
...
USE GEOMETRY_FUNCTIONS, ONLY : BLOCK_CELL, IS_CELL_INSIDE_OBST
...
! But preserve SOLID if cell is inside an OBST (multi-mesh overlap case)

IF (IW<=NEWC .AND. (BOUNDARY_TYPE(IW)==OPEN_BOUNDARY .OR. BOUNDARY_TYPE(IW)==INTERPOLATED_BOUNDARY)) THEN
   IF (.NOT.IS_CELL_INSIDE_OBST(NM,IC)) SOLID(IC) = .FALSE.
ENDIF
