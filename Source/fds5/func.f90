...
WRITE(LU_OUTPUT,'(/A,A,A,A)') TRIM(MESSAGE),' (CHID: ',TRIM(CHID),')'
...
! Read external control file for CTRL values (FDS6/Fenix+ compatibility)
! File format: CTRL, "CTRL_ID", value
! Example: CTRL, "CTRL_2", 1.0  (sets CTRL_2 to .TRUE.)

USE GLOBAL_CONSTANTS, ONLY: EXTERNAL_CTRL,EXTERNAL_FILENAME
USE CONTROL_VARIABLES, ONLY: CONTROL,N_CTRL
INTEGER :: NC,IOS,COMMA1,COMMA2,QUOTE1,QUOTE2,IU
CHARACTER(256) :: LINE
CHARACTER(60) :: INPUT_LABEL,INPUT_TYPE
REAL(EB) :: INPUT_REAL
...
! Initialize with current state if not allocated
IF (.NOT.ALLOCATED(EXTERNAL_CTRL)) THEN
   ALLOCATE(EXTERNAL_CTRL(N_CTRL))
   DO NC=1,N_CTRL
      EXTERNAL_CTRL(NC) = CONTROL(NC)%INITIAL_STATE
   ENDDO
ENDIF

! Check if file exists
INQUIRE(FILE=EXTERNAL_FILENAME,EXIST=FILE_EXISTS)
IF (.NOT.FILE_EXISTS) RETURN

! Open and read file
IU = 97
OPEN(IU,FILE=EXTERNAL_FILENAME,STATUS='OLD',IOSTAT=IOS)
IF (IOS /= 0) THEN
   FAILED = .TRUE.
   RETURN
ENDIF
...
DO
   READ(IU,FMT='(A)',END=200,ERR=110) LINE
   
   ! Skip empty lines
   IF (LEN_TRIM(LINE) == 0) CYCLE
   
   ! Parse line: CTRL, "CTRL_2", -1 (Fenix+ format)
   COMMA1 = INDEX(LINE,',')
   IF (COMMA1 > 0) THEN
      ! Extract type (before first comma)
      INPUT_TYPE = ADJUSTL(LINE(1:COMMA1-1))
      
      ! Find second comma
      COMMA2 = INDEX(LINE(COMMA1+1:),',')
      IF (COMMA2 > 0) THEN
         COMMA2 = COMMA1 + COMMA2
         ! Extract label (between commas, remove quotes)
         INPUT_LABEL = ADJUSTL(LINE(COMMA1+1:COMMA2-1))
         QUOTE1 = INDEX(INPUT_LABEL,'"')
         IF (QUOTE1 > 0) THEN
            QUOTE2 = INDEX(INPUT_LABEL(QUOTE1+1:),'"')
            IF (QUOTE2 > 0) THEN
               INPUT_LABEL = INPUT_LABEL(QUOTE1+1:QUOTE1+QUOTE2-1)
            ENDIF
         ENDIF
         
         ! Extract value (after second comma)
         READ(LINE(COMMA2+1:),*,IOSTAT=IOS) INPUT_REAL
         IF (IOS /= 0) CYCLE
      ELSE
         CYCLE
      ENDIF
   ...
   ! Process CTRL type
   IF (TRIM(INPUT_TYPE)=='CTRL') THEN
      DO NC=1,N_CTRL
         IF (TRIM(CONTROL(NC)%ID) == TRIM(INPUT_LABEL)) THEN
            IF (INPUT_REAL < 0._EB) THEN
...
CHARACTER(200), ALLOCATABLE, DIMENSION(:) :: DUMMY
...
LOGICAL FUNCTION IS_CELL_INSIDE_OBST(NM,IC)
! Check if cell IC is inside any OBST in mesh NM
INTEGER, INTENT(IN) :: NM,IC
TYPE(MESH_TYPE), POINTER :: M

M => MESHES(NM)
IS_CELL_INSIDE_OBST = (M%OBST_INDEX_C(IC) > 0)

END FUNCTION IS_CELL_INSIDE_OBST
...
FUNCTION INTERP_OVERLAP_WEIGHT(IOR,ABS_IOR,I,J,K,IIO,JJO,KKO, &
                               YF,YF2,ZF,ZF2, &
                               XF,XF2) RESULT(WGT)
! Compute area-weighted overlap fraction for INTERPOLATED_BOUNDARY cells.
! Returns: overlap_area / receiving_cell_face_area for the two transverse directions.
! For aligned meshes this returns 1.0 (same as old MIN-based formula).
USE GLOBAL_CONSTANTS, ONLY: EB
INTEGER, INTENT(IN) :: IOR,ABS_IOR,I,J,K,IIO,JJO,KKO
REAL(EB), INTENT(IN) :: YF(:),YF2(:),ZF(:),ZF2(:)
REAL(EB), INTENT(IN) :: XF(:),XF2(:)
REAL(EB) :: WGT
REAL(EB) :: OVLP1,OVLP2,T1_MIN,T1_MAX,T2_MIN,T2_MAX,FACE1,FACE2
! FDS6: клампинг индексов под фактические границы переданных массивов координат
! (граничные ghost-клетки дают индекс-1 = 0 при LBOUND=1)
INTEGER :: IC0,IC1,JC0,JC1,KC0,KC1,I2C0,I2C1,J2C0,J2C1,K2C0,K2C1
IC0  = MAX(I-1  ,LBOUND(XF ,1)); IC1  = MIN(I  ,UBOUND(XF ,1))
JC0  = MAX(J-1  ,LBOUND(YF ,1)); JC1  = MIN(J  ,UBOUND(YF ,1))
KC0  = MAX(K-1  ,LBOUND(ZF ,1)); KC1  = MIN(K  ,UBOUND(ZF ,1))
I2C0 = MAX(IIO-1,LBOUND(XF2,1)); I2C1 = MIN(IIO,UBOUND(XF2,1))
J2C0 = MAX(JJO-1,LBOUND(YF2,1)); J2C1 = MIN(JJO,UBOUND(YF2,1))
K2C0 = MAX(KKO-1,LBOUND(ZF2,1)); K2C1 = MIN(KKO,UBOUND(ZF2,1))

WGT = 0._EB

SELECT CASE(ABS_IOR)
CASE(1)  ! IOR = +/-1: transverse directions are Y and Z
   T1_MIN = MAX(YF(JC0),  YF2(J2C0))
   T1_MAX = MIN(YF(JC1),  YF2(J2C1))
   T2_MIN = MAX(ZF(KC0),  ZF2(K2C0))
   T2_MAX = MIN(ZF(KC1),  ZF2(K2C1))
   OVLP1  = MAX(0._EB, T1_MAX - T1_MIN)
   OVLP2  = MAX(0._EB, T2_MAX - T2_MIN)
   FACE1  = (YF(JC1)-YF(JC0)) * (ZF(KC1)-ZF(KC0))
   FACE2  = OVLP1 * OVLP2
   IF (FACE1 > 0._EB) WGT = FACE2 / FACE1
CASE(2)  ! IOR = +/-2: transverse directions are X and Z
   T1_MIN = MAX(XF(IC0),  XF2(I2C0))
   T1_MAX = MIN(XF(IC1),  XF2(I2C1))
   T2_MIN = MAX(ZF(KC0),  ZF2(K2C0))
   T2_MAX = MIN(ZF(KC1),  ZF2(K2C1))
   OVLP1  = MAX(0._EB, T1_MAX - T1_MIN)
   OVLP2  = MAX(0._EB, T2_MAX - T2_MIN)
   FACE1  = (XF(IC1)-XF(IC0)) * (ZF(KC1)-ZF(KC0))
   FACE2  = OVLP1 * OVLP2
   IF (FACE1 > 0._EB) WGT = FACE2 / FACE1
CASE(3)  ! IOR = +/-3: transverse directions are X and Y
   T1_MIN = MAX(XF(IC0),  XF2(I2C0))
   T1_MAX = MIN(XF(IC1),  XF2(I2C1))
   T2_MIN = MAX(YF(JC0),  YF2(J2C0))
   T2_MAX = MIN(YF(JC1),  YF2(J2C1))
   OVLP1  = MAX(0._EB, T1_MAX - T1_MIN)
   OVLP2  = MAX(0._EB, T2_MAX - T2_MIN)
   FACE1  = (XF(IC1)-XF(IC0)) * (YF(JC1)-YF(JC0))
   FACE2  = OVLP1 * OVLP2
   IF (FACE1 > 0._EB) WGT = FACE2 / FACE1
END SELECT

END FUNCTION INTERP_OVERLAP_WEIGHT
