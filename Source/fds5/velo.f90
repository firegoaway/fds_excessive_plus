   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            ITMP = MAX(0, MIN(4999, INT(TMP(I,J,K))))
...
INTEGER, INTENT(IN) :: NM
REAL(EB) :: MU_OTHER,DP_OTHER,KRES_OTHER,WGT
INTEGER :: IIG,JJG,KKG,II,JJ,KK,IW,IOR,IIO,JJO,KKO,NOM,N_INT_CELLS

CALL POINT_TO_MESH(NM)

! Mirror viscosity into solids and exterior boundary cells

!$OMP PARALLEL DO PRIVATE(IW,II,JJ,KK,IIG,JJG,KKG,IOR,NOM,MU_OTHER,DP_OTHER,KRES_OTHER,KKO,JJO,IIO,N_INT_CELLS,WGT)
WALL_LOOP: DO IW=1,NWC
   !!$ IF ((IW == 1) .AND. DEBUG_OPENMP) WRITE(*,*) 'OpenMP_VISCOSITY_BC'
   ...
   DO KKO=IJKW(12,IW),IJKW(15,IW)
      DO JJO=IJKW(11,IW),IJKW(14,IW)
         DO IIO=IJKW(10,IW),IJKW(13,IW)
            WGT = INTERP_OVERLAP_WEIGHT(IOR,ABS(IOR),II,JJ,KK,IIO,JJO,KKO, &
                 Y,MESHES(NOM)%Y, &
                 Z,MESHES(NOM)%Z, &
                 X,MESHES(NOM)%X)
            MU_OTHER = MU_OTHER + OMESH(NOM)%MU(IIO,JJO,KKO)*WGT
            KRES_OTHER = KRES_OTHER + OMESH(NOM)%KRES(IIO,JJO,KKO)*WGT
            IF (PREDICTOR) THEN
               DP_OTHER = DP_OTHER + OMESH(NOM)%D(IIO,JJO,KKO)*WGT
            ELSE
               DP_OTHER = DP_OTHER + OMESH(NOM)%DS(IIO,JJO,KKO)*WGT
            ENDIF
         ENDDO
      ENDDO
   ENDDO
...
REAL(EB) :: RFODT,H_OTHER,DUUDT,DVVDT,DWWDT,ACF,WGT,DX_OFF,DY_OFF,DZ_OFF,H_CORR
...
NO_SCARC_IF: IF (PRES_METHOD /= 'SCARC') THEN
   !$OMP DO PRIVATE(IW,NOM,II,JJ,KK,H_OTHER,KKO,JJO,IIO,N_INT_CELLS,IOR,WGT, &
   !$OMP            DX_OFF,DY_OFF,DZ_OFF,H_CORR)
   DO IW=1,NEWC
      ...
      DO KKO=IJKW(12,IW),IJKW(15,IW)
         DO JJO=IJKW(11,IW),IJKW(14,IW)
            DO IIO=IJKW(10,IW),IJKW(13,IW)
               ...
               DX_OFF = XC(II) - MESHES(NOM)%XC(IIO)
               DY_OFF = YC(JJ) - MESHES(NOM)%YC(JJO)
               DZ_OFF = ZC(KK) - MESHES(NOM)%ZC(KKO)
               IF (PREDICTOR) THEN
         ...
         ENDDO
      ENDDO
      IF (PREDICTOR) H(II,JJ,KK)  = H_OTHER
      IF (CORRECTOR) HS(II,JJ,KK) = H_OTHER
   ENDDO
   !$OMP END DO
ENDIF NO_SCARC_IF
...
!$OMP DO PRIVATE(IW,NOM,II,JJ,KK,IOR,DUUDT,DVVDT,DWWDT,VT,ACF)
WALL_LOOP: DO IW=1,NWC
            ...
            BOUNDARY_CONDITION: SELECT CASE(VELOCITY_BC_INDEX)
               ...
               CASE (WALL_MODEL) BOUNDARY_CONDITION
                  ...
                  IF ( SOLID(CELL_INDEX(IIGM,JJGM,KKGM)) .OR. SOLID(CELL_INDEX(IIGP,JJGP,KKGP)) ) THEN
                     ...
                     SLIP_COEF=-1._EB
                  ELSE
                     ITMP = MAX(0, MIN(4999, NINT(0.5_EB*(TMP(IIGM,JJGM,KKGM)+TMP(IIGP,JJGP,KKGP)))))
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               ...
               UVW  = MAX(UODX,VODY,WODZ) + CFL_DP_SCALE*ABS(DP(I,J,K))
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               ...
               UVW = UVW + CFL_DP_SCALE*ABS(DP(I,J,K))
               IF (UVW>=P_UVWMAX) THEN
                  P_UVWMAX = UVW
                  P_ICFL=I
                  P_JCFL=J
...
WALL_LOOP: DO IW=1,NWC
   ...
   ELSE
      ITMP = MAX(0, MIN(4999, INT(TMP(IIG,JJG,KKG))))
...
ENDIF
 ...
 CFL = DT*UVWMAX
 ! R23a: декомпозиция CFL-лидера (адвекция vs дивергенция DP) — считается здесь,
 ! на ранге-владельце меша, по финальному лидеру ICFL/JCFL/KCFL; в.dump
 ! WRITE_DIAGNOSTICS (только ранг 0) читает скаляры MESHES(NM)%CFL_ADV/CFL_DP,
 ! пересылаемые EXCHANGE_DIAGNOSTICS. CFL_ADV поглощает и не-адвекционные
 ! слагаемые (wall-CFL, гравитационные волны), если лидер от них
 CFL_DP  = DT*CFL_DP_SCALE*ABS(DP(ICFL,JCFL,KCFL))
 CFL_ADV = MAX(0._EB, CFL-CFL_DP)
...
IF ((CFL<CFL_MAX.AND.VN<VN_MAX) .OR. LOCK_TIME_STEP) THEN
   ...
   IF (CFL<=CFL_MIN .AND. VN<VN_MIN .AND. .NOT.LOCK_TIME_STEP) THEN
      ! Faster recovery: use 1.5x growth factor instead of 1.1x to avoid
      ! prolonged small DT after transient CFL spikes
      IF (     RESTRICT_TIME_STEP) DT_NEXT = MIN(1.5_EB*DT,DT_INIT)
      IF (.NOT.RESTRICT_TIME_STEP) DT_NEXT =     1.5_EB*DT
   ENDIF
ELSE
