...
! Update fire spread outputs (stub for vegetation/terrain compatibility)
...
WRITE(LU_OUTPUT,'(A,L10)')    '   CCC Default Enabled            ',CCC_DEFAULT
WRITE(LU_OUTPUT,'(A,F8.2)')   '   Divergence Limit               ',DIVERGENCE_LIMIT
WRITE(LU_OUTPUT,'(A,F8.2)')   '   Heat Transfer CFL Limit        ',HT_CFL_LIMIT
WRITE(LU_OUTPUT,'(A,F8.2)')   '   Particle Velocity Limit        ',PARTICLE_VELOCITY_LIMIT
WRITE(LU_OUTPUT,'(A,F8.2)')   '   Von Neumann Limit              ',VON_NEUMANN_LIMIT
...
SELECT CASE(PRES_FLAG)
   ...
   CASE(ULMAT_FLAG)
      ...
      END SELECT
   CASE(SOAP_FLAG)
      WRITE(LU_OUTPUT,'(3X,A,13X,A)') 'Solver:', 'SOAP (Smoothness-Based Adaptive Poisson)'
      WRITE(LU_OUTPUT,'(3X,A)') 'NOTE: Adaptively switches between FFT solve and extrapolation'
      WRITE(LU_OUTPUT,'(3X,A)') '      based on RHS smoothness. Recommended for engineering'
      WRITE(LU_OUTPUT,'(3X,A)') '      calculations with CFL 5-15.'
   CASE(LAZY_SOAP_FLAG)
      WRITE(LU_OUTPUT,'(3X,A,13X,A)') 'Solver:', 'LAZY-SOAP (Lazy Smoothness-Based Adaptive Poisson)'
      WRITE(LU_OUTPUT,'(3X,A)') 'NOTE: Optimized adaptive solver with RHS computation skipping'
      WRITE(LU_OUTPUT,'(3X,A)') '      and 2nd order extrapolation. Features:'
      WRITE(LU_OUTPUT,'(3X,A)') '      - Global skip decision across all meshes'
      WRITE(LU_OUTPUT,'(3X,A,I0,A)') '      - Aggressive threshold: ',INT(LAZY_SOAP_THRESHOLD*100),'% RHS change'
      WRITE(LU_OUTPUT,'(3X,A)') '      - 2nd order extrapolation using 3 previous steps'
      WRITE(LU_OUTPUT,'(3X,A)') '      - Divergence error control for FFT fallback'
      WRITE(LU_OUTPUT,'(3X,A)') '      Recommended for fast engineering calculations with CFL 8-20.'
      WRITE(LU_OUTPUT,'(3X,A)') '      EXPECTED SPEEDUP: 3-5x with 90-95% accuracy.'
END SELECT
...
USE CC_SCALARS, ONLY: COPY_UNST_DM_TO_CART,CC_COPY_UNST_DM_TO_CART_W
REAL(EB), INTENT(IN) :: T,DT
REAL(EB) :: STIME
...
IF(CC_IBM) CALL CC_COPY_UNST_DM_TO_CART_W(NM)
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(503)  ! CCC - Combustion Completeness Coefficient
      III = MAX(1,MIN(II,IBAR))
      JJJ = MAX(1,MIN(JJ,JBAR))
      KKK = MAX(1,MIN(KK,KBAR))
      GAS_PHASE_OUTPUT_RES = CCC(III,JJJ,KKK)
   CASE(504)  ! CFL 1
      IF (CELL(CELL_INDEX(II,JJ,KK))%SOLID) THEN
         GAS_PHASE_OUTPUT_RES = 0._EB
      ELSE
