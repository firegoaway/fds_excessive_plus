...
PUBLIC INITIALIZE_WALL_ARRAY,INITIALIZE_ATMOSPHERE,INITIALIZE_MESH_VARIABLES_1,INITIALIZE_MESH_VARIABLES_2,&
       ...
       FIND_WALL_BACK_INDICES,INITIALIZE_SOLID_CELL
...
IF (MLR_BASED_HRR) THEN
    ALLOCATE(M%WALL_MLR_SUM(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
    CALL ChkMemErr('INIT','WALL_MLR_SUM',IZERO)
    M%WALL_MLR_SUM = 0._EB
    ALLOCATE(M%VENT_MLR_SUM(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
    CALL ChkMemErr('INIT','VENT_MLR_SUM',IZERO)
    M%VENT_MLR_SUM = 0._EB
ENDIF
...
ALLOCATE(M%CCC(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)       ; CALL ChkMemErr('INIT','CCC',IZERO)       ; M%CCC       = 1._EB
...
! Allocate cached B1 property arrays for hot paths (avoids 3-level indirection in CHECK_STABILITY)
ALLOCATE(M%WALL_Q_CON_F_CACHE(0:M%N_WALL_CELLS_DIM+M%N_THIN_WALL_CELLS_DIM),STAT=IZERO)
CALL ChkMemErr('INIT','WALL_Q_CON_F_CACHE',IZERO) ; M%WALL_Q_CON_F_CACHE=0._EB
ALLOCATE(M%WALL_RHO_F_CACHE(0:M%N_WALL_CELLS_DIM+M%N_THIN_WALL_CELLS_DIM),STAT=IZERO)
CALL ChkMemErr('INIT','WALL_RHO_F_CACHE',IZERO) ; M%WALL_RHO_F_CACHE=0._EB
ALLOCATE(M%WALL_RDN_CACHE(0:M%N_WALL_CELLS_DIM+M%N_THIN_WALL_CELLS_DIM),STAT=IZERO)
CALL ChkMemErr('INIT','WALL_RDN_CACHE',IZERO) ; M%WALL_RDN_CACHE=0._EB
...
!> \brief Populate SOLID_CELL 3D array from CELL%SOLID for hot-loop performance (TIER 7.2)
!> \param NM Mesh number

SUBROUTINE INITIALIZE_SOLID_CELL(NM)

USE MESH_POINTERS
INTEGER, INTENT(IN) :: NM
INTEGER :: I,J,K,IC,IZERO
TYPE (MESH_TYPE), POINTER :: M

M => MESHES(NM)

! If already allocated (e.g., second call from CREATE_OR_REMOVE_OBSTRUCTIONS), deallocate first
IF (ALLOCATED(M%SOLID_CELL)) DEALLOCATE(M%SOLID_CELL)

ALLOCATE(M%SOLID_CELL(1:M%IBAR,1:M%JBAR,1:M%KBAR),STAT=IZERO)
CALL ChkMemErr('INIT','SOLID_CELL',IZERO)

DO K=1,M%KBAR
   DO J=1,M%JBAR
      DO I=1,M%IBAR
         IC = M%CELL_INDEX(I,J,K)
         M%SOLID_CELL(I,J,K) = M%CELL(IC)%SOLID
      ENDDO
   ENDDO
ENDDO

END SUBROUTINE INITIALIZE_SOLID_CELL
